 ...
 */
function assertLicensesValid({ packages, validLicenses }) {
    const invalidMsgs = packages.reduce((acc, pkg) => {
        ...
        return acc.concat(dedent_1.default `
        ${pkg.name}
          version: ${pkg.version}
    ...
    if (invalidMsgs.length) {
        throw dev_utils_1.createFailError(`Non-conforming licenses:\n${invalidMsgs
            .join('\n')
            .split('\n')
            .map(l => `  ${l}`)
            .join('\n')}`);
...
exports.assertLicensesValid = assertLicensesValid;
