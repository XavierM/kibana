...
function runTypeCheckCli() {
    ...
    const projects = projects_1.filterProjectsByFlag(opts.project).filter(p => !p.disableTypeCheck);
    if (!projects.length) {
        log.error(`Unable to find project at ${opts.project}`);
        process.exit(1);
    }
    exec_in_projects_1.execInProjects(log, projects, process.execPath, project => [
