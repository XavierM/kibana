 ...
 */
const getReportFileName = (folder) => {
    return folder.indexOf('public') > -1 ? 'public' : 'server';
};
const apiExtractorConfig = (folder) => {
    ...
    const config = {
        ...
        projectFolder: path_1.default.resolve('./'),
    ...
    const con = api_extractor_1.ExtractorConfig.prepare({
        configObject: config,
        configObjectFullPath: undefined,
        packageJsonFullPath: path_1.default.resolve('package.json'),
    });
    return con;
};
const runBuildTypes = async () => {
    await execa_1.default('yarn', ['run', 'build:types']);
};
const runApiDocumenter = async (folder) => {
    ...
    await execa_1.default('api-documenter', ['generate', '-i', sourceFolder, '-o', targetFolder], {
        preferLocal: true,
    });
};
const renameExtractedApiPackageName = async (folder) => {
    const fname = getReportFileName(folder);
    const jsonApiFile = `build/${folder}/${fname}.api.json`;
    console.log(`Updating ${jsonApiFile}...`);
    const json = JSON.parse(fs_1.default.readFileSync(jsonApiFile).toString());
    json.name = json.canonicalReference = `kibana-plugin-${folder.replace(/\//g, '-')}`;
    fs_1.default.writeFileSync(jsonApiFile, JSON.stringify(json, null, 2));
};
/**
 * Runs api-extractor with a custom logger in order to extract results from the process
 *
 */
const runApiExtractor = (log, folder, acceptChanges = false) => {
    ...
    const options = {
        ...
        messageCallback: (message) => {
            if (message.messageId === 'console-api-report-not-copied') {
                ...
                log.warning('To accept these changes run `node scripts/check_published_api_changes.js --accept` and then:\n' +
                ...
                log.warning(`You have changed the signature of the ${folder} Core API`);
                log.warning("Please commit the updated API documentation and the API review file: '" +
                    config.reportFilePath);
                message.handled = true;
            }
    ...
    return api_extractor_1.Extractor.invoke(config, options);
};
async function run(folder, { log, opts }) {
...
(async () => {
    ...
    const results = await Promise.all(folders
