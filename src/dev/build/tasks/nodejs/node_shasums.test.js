...
jest.mock('axios', () => ({
    async get(url) {
        expect(url).toBe('https://us-central1-elastic-kibana-184716.cloudfunctions.net/kibana-ci-proxy-cache/dist/v8.9.4/SHASUMS256.txt');
        return {
            status: 200,
            data: mockResponse,
        };
...
const node_shasums_1 = require("./node_shasums");
describe('src/dev/build/tasks/nodejs/node_shasums', () => {
    it('resolves to an object with shasums for node downloads for version', async () => {
        const shasums = await node_shasums_1.getNodeShasums('8.9.4');
        expect(shasums).toEqual(expect.objectContaining({
            'node-v8.9.4.tar.gz': '729b44b32b2f82ecd5befac4f7518de0c4e3add34e8fe878f745740a66cbbc01',
            'node-v8.9.4-win-x64.zip': '48946e99ac4484e071df25741d2300f3a656f476c5ff3f8116a4746c07ebe3b7',
        }));
    });
});
