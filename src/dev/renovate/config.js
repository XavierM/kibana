...
const package_groups_1 = require("./package_groups");
...
exports.RENOVATE_CONFIG = {
    extends: ['config:base'],
    includePaths: package_globs_1.PACKAGE_GLOBS,
    /**
    ...
    npm: {
         ...
         */
        packageRules: [
            ...package_groups_1.RENOVATE_PACKAGE_GROUPS.map(group => ({
                groupSlug: group.name,
                groupName: `${group.name} related packages`,
                packagePatterns: utils_1.maybeMap(group.packageWords, word => utils_1.wordRegExp(word).source),
                packageNames: utils_1.maybeFlatMap(group.packageNames, name => [name, utils_1.getTypePackageName(name)]),
