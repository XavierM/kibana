...
const stub_browser_storage_1 = require("./stub_browser_storage");
describe('StubBrowserStorage', () => {
    describe('#getItem() / #setItem()', () => {
        it('stores items as strings', () => {
            const store = new stub_browser_storage_1.StubBrowserStorage();
            store.setItem('1', '1');
    ...
    describe('#setStubbedSizeLimit', () => {
        it('allows limiting the storage size', () => {
            ...
            }).toThrowErrorMatchingInlineSnapshot(`"something about quota exceeded, browsers are not consistent here"`);
        });
