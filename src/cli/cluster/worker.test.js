...
describe('CLI cluster manager', () => {
    afterEach(async () => {
        ...
        cluster_manager_test_mocks_1.mockCluster.fork.mockClear();
    });
    describe('#onChange', () => {
    ...
    describe('#start', () => {
        describe('when not started', () => {
            test('creates a fork and waits for it to come online', async () => {
                ...
                await worker.start();
                expect(cluster_manager_test_mocks_1.mockCluster.fork).toHaveBeenCalledTimes(1);
            ...
            test('listens for cluster and process "exit" events', async () => {
                ...
                jest.spyOn(cluster_manager_test_mocks_1.mockCluster, 'on');
                await worker.start();
                expect(cluster_manager_test_mocks_1.mockCluster.on).toHaveBeenCalledTimes(1);
                expect(cluster_manager_test_mocks_1.mockCluster.on).toHaveBeenCalledWith('exit', expect.any(Function));
