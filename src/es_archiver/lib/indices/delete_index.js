...
async function deleteIndex(options) {
    ...
    try {
        ...
        if (lodash_1.get(error, 'body.error.type') !== 'index_not_found_exception') {
            throw error;
        }
    }
}
 ...
 * @return {Boolean}
 */
function isDeleteWhileSnapshotInProgressError(error) {
    return lodash_1.get(error, 'body.error.reason', '').startsWith('Cannot delete indices that are being snapshotted');
}
exports.isDeleteWhileSnapshotInProgressError = isDeleteWhileSnapshotInProgressError;
/**
 * Wait for the any snapshot in any repository that is
 * snapshotting this index to complete.
 */
async function waitForSnapshotCompletion(client, index, log) {
...
exports.waitForSnapshotCompletion = waitForSnapshotCompletion;
