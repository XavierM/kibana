 ...
 *  @return {Boolean}
 */
function isMappingFile(path) {
    return path_1.basename(path, '.gz') === 'mappings.json';
}
exports.isMappingFile = isMappingFile;
/**
 ...
 *  @return {Array<String>}
 */
function prioritizeMappings(filenames) {
    return filenames.slice().sort((fa, fb) => {
        if (isMappingFile(fa) === isMappingFile(fb))
            return 0;
        return isMappingFile(fb) ? 1 : -1;
    });
}
exports.prioritizeMappings = prioritizeMappings;
