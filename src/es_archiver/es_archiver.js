...
class EsArchiver {
    constructor({ client, dataDir, log, kibanaUrl, }) {
        ...
        this.log = log;
        this.kbnClient = new dev_utils_1.KbnClient(log, [kibanaUrl]);
    }
    /**
    ...
    async save(name, indices, { raw = false } = {}) {
        return await actions_1.saveAction({
            name,
            indices,
            raw,
            client: this.client,
     ...
     *  @return Promise<Stats>
     */
    async load(name, { skipExisting = false } = {}) {
        return await actions_1.loadAction({
            name,
     ...
     */
    async unload(name) {
        return await actions_1.unloadAction({
            name,
            client: this.client,
            dataDir: this.dataDir,
            log: this.log,
     ...
     */
    async rebuildAll() {
        return await actions_1.rebuildAllAction({
            dataDir: this.dataDir,
            log: this.log,
        });
    }
    ...
    async edit(prefix, handler) {
        return await actions_1.editAction({
            prefix,
            log: this.log,
            dataDir: this.dataDir,
            handler,
    ...
    async emptyKibanaIndex() {
        await actions_1.emptyKibanaIndexAction({
            client: this.client,
            log: this.log,
            kbnClient: this.kbnClient,
        });
