...
function stateMonitor(state, customInitialState) {
    ...
    function setInitialState(innerCustomInitialState) {
        ...
        initialState = lodash_1.cloneDeep(innerCustomInitialState) || lodash_1.cloneDeep(state.toJSON());
    }
    function removeIgnoredProps(innerState) {
        ignoredProps.forEach(path => {
            lodash_1.set(innerState, path, true);
        });
        return innerState;
    }
    function getStatus() {
        // state.toJSON returns a reference, clone so we can mutate it safely
        const currentState = removeIgnoredProps(lodash_1.cloneDeep(state.toJSON()));
        const isClean = lodash_1.isEqual(currentState, initialState);
        return {
            clean: isClean,
            dirty: !isClean,
        };
    ...
    return {
        setInitialState(innerCustomInitialState) {
            if (!lodash_1.isPlainObject(innerCustomInitialState)) {
            ...
            if (!lodash_1.isEqual(previousStatus, getStatus())) {
                dispatchChange();
            }
        },
        ignoreProps(props) {
