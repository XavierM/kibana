...
const sub_url_hooks_utils_1 = require("./sub_url_hooks_utils");
test('false for different hashes', () => {
    ...
    expect(sub_url_hooks_utils_1.areHashesDifferentButDecodedHashesEquals(url1, url2)).toBeFalsy();
});
test('false for same hashes', () => {
    ...
    expect(sub_url_hooks_utils_1.areHashesDifferentButDecodedHashesEquals(url1, url1)).toBeFalsy();
});
test('true for same hashes, but one is encoded', () => {
    const hash = `/dashboard/id?_a=(filters:!(),query:(language:kuery,query:''))&_g=(filters:!(),time:(from:now-120m,to:now))`;
    const url1 = `https://localhost/kibana/#/${hash}`;
    const url2 = `https://localhost/kibana/#/${encodeURIComponent(hash)}`;
    expect(sub_url_hooks_utils_1.areHashesDifferentButDecodedHashesEquals(url1, url2)).toBeTruthy();
});
/**
