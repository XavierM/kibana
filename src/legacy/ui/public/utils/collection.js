...
function move(objs, obj, below, qualifier) {
    const origI = lodash_1.default.isNumber(obj) ? obj : objs.indexOf(obj);
    if (origI === -1) {
        return objs;
    }
    if (lodash_1.default.isNumber(below)) {
        // move to a specific index
        objs.splice(below, 0, objs.splice(origI, 1)[0]);
        return objs;
    }
    below = !!below;
    qualifier = qualifier && lodash_1.default.callback(qualifier);
    const above = !below;
    const finder = below ? lodash_1.default.findIndex : lodash_1.default.findLastIndex;
    ...
    const targetI = finder(objs, (otherAgg, otherI) => {
        ...
        return Boolean(lodash_1.default.isFunction(qualifier) && qualifier(otherAgg, otherI));
    });
    if (targetI === -1) {
        return objs;
    }
