...
class Cluster {
    constructor(clusterClient) {
        ...
        this.errors = elasticsearch_1.errors;
        this.callWithRequest = async (req, endpoint, clientParams, options) => {
            return await this.clusterClient
                .asScoped(req)
                .callAsCurrentUser(endpoint, clientParams, options);
        };
        this.callWithInternalUser = async (endpoint, clientParams, options) => {
