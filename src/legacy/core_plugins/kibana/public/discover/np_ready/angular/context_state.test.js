...
const context_state_1 = require("./context_state");
...
describe('Test Discover Context State', () => {
    ...
    beforeEach(async () => {
        ...
        history.push('/');
        state = await context_state_1.getState({
            defaultStepSize: '4',
            timeFieldName: 'time',
            history,
        });
    ...
    test('getState -> setAppState syncing to url', async () => {
        ...
        state.flushToUrl();
        expect(getCurrentUrl()).toMatchInlineSnapshot(`"/#?_a=(columns:!(_source),filters:!(),predecessorCount:10,sort:!(time,desc),successorCount:4)"`);
    });
    test('getState -> url to appState syncing', async () => {
        history.push('/#?_a=(columns:!(_source),predecessorCount:1,sort:!(time,desc),successorCount:1)');
        expect(state.appState.getState()).toMatchInlineSnapshot(`
      Object {
        "columns": Array [
          "_source",
    ...
    test('getState -> filters', async () => {
        const filterManager = new public_1.FilterManager(setupMock.uiSettings);
        const filterGlobal = {
    ...
    `);
        state.flushToUrl();
        expect(getCurrentUrl()).toMatchInlineSnapshot(`"/#?_g=(filters:!(('$state':(store:globalState),meta:(alias:!n,disabled:!f,index:'logstash-*',key:extension,negate:!f,params:(query:jpg),type:phrase),query:(match:(extension:(query:jpg,type:phrase))))))&_a=(columns:!(_source),filters:!(('$state':(store:appState),meta:(alias:!n,disabled:!f,index:'logstash-*',key:extension,negate:!t,params:(query:png),type:phrase),query:(match:(extension:(query:png,type:phrase))))),predecessorCount:4,sort:!(time,desc),successorCount:4)"`);
    });
});
