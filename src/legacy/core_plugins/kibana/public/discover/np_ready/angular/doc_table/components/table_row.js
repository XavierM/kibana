...
require("../../doc_viewer");
// @ts-ignore
const no_white_space_1 = require("../../../../../../common/utils/no_white_space");
    ...
    return {
        ...
        link: ($scope, $el) => {
            ...
            $scope.getContextAppHref = () => {
                const path = `#/discover/context/${encodeURIComponent($scope.indexPattern.id)}/${encodeURIComponent($scope.row._id)}`;
                ...
                const hash = $httpParamSerializer({
                    _g: rison_node_1.default.encode({
            ...
            function createSummaryRow(row) {
                ...
                const hideTimeColumn = kibana_services_1.getServices().uiSettings.get('doc_table:hideTimeColumn');
                if (indexPattern.timeFieldName && !hideTimeColumn) {
                    newHtmls.push(cellTemplate({
                        timefield: true,
                        formatted: _displayField(row, indexPattern.timeFieldName),
                        filterable: mapping(indexPattern.timeFieldName).filterable && lodash_1.default.isFunction($scope.filter),
                ...
                $scope.columns.forEach(function (column) {
                    const isFilterable = $scope.flattenedRow[column] !== undefined &&
                        ...
                        lodash_1.default.isFunction($scope.filter);
                    newHtmls.push(cellTemplate({
                        timefield: false,
