...
describe('field_filter', function () {
    it('getDefaultFieldFilter should return default filter state', function () {
        expect(field_filter_1.getDefaultFieldFilter()).toMatchInlineSnapshot(`
      Object {
        "aggregatable": null,
        "missing": true,
        "name": "",
    ...
    it('setFieldFilterProp should return allow filter changes', function () {
        const state = field_filter_1.getDefaultFieldFilter();
        const targetState = {
            aggregatable: true,
            missing: true,
            name: 'test',
        ...
        const actualState = Object.entries(targetState).reduce((acc, kv) => {
            return field_filter_1.setFieldFilterProp(acc, kv[0], kv[1]);
        }, state);
    ...
    it('filters a given list', () => {
        const defaultState = field_filter_1.getDefaultFieldFilter();
        const fieldList = [
            {
                name: 'bytes',
                type: 'number',
        ...
        [
            ...
            const filtered = fieldList
                .filter(field => field_filter_1.isFieldFiltered(field, { ...defaultState, ...test.filter }, { bytes: 1, extension: 1 }))
