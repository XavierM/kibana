...
const get_matched_indices_1 = require("./get_matched_indices");
jest.mock('./../constants', () => ({
...
describe('getMatchedIndices', () => {
    it('should return all indices', () => {
        const { allIndices, exactMatchedIndices, partialMatchedIndices, visibleIndices, } = get_matched_indices_1.getMatchedIndices(indices, partialIndices, exactIndices, true);
    ...
    it('should return all indices except for system indices', () => {
        const { allIndices, exactMatchedIndices, partialMatchedIndices, visibleIndices, } = get_matched_indices_1.getMatchedIndices(indices, partialIndices, exactIndices, false);
        expect(allIndices).toEqual([
            { name: 'kibana', tags },
            { name: 'es', tags },
            { name: 'logstash', tags },
    ...
    it('should return partial matches as visible if there are no exact', () => {
        const { visibleIndices } = get_matched_indices_1.getMatchedIndices(indices, partialIndices, [], true);
        expect(visibleIndices).toEqual([
            { name: 'kibana', tags },
            { name: 'es', tags },
            { name: '.kibana', tags },
    ...
    it('should return all indices as visible if there are no exact or partial', () => {
        const { visibleIndices } = get_matched_indices_1.getMatchedIndices(indices, [], [], true);
