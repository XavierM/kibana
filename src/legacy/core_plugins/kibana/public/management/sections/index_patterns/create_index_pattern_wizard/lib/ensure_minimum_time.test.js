...
const ensure_minimum_time_1 = require("./ensure_minimum_time");
describe('ensureMinimumTime', () => {
    it('resolves single promise', async (done) => {
        const promiseA = new Promise(resolve => resolve('a'));
        const a = await ensure_minimum_time_1.ensureMinimumTime(promiseA, 0);
        expect(a).toBe('a');
        done();
    });
    it('resolves multiple promises', async (done) => {
        ...
        const [a, b] = await ensure_minimum_time_1.ensureMinimumTime([promiseA, promiseB], 0);
        expect(a).toBe('a');
        expect(b).toBe('b');
        done();
    });
    it('resolves in the amount of time provided, at minimum', async (done) => {
        ...
        await ensure_minimum_time_1.ensureMinimumTime(promise, 100);
