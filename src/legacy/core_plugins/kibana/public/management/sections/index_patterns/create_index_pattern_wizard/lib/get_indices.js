...
async function getIndices(es, indexPatternCreationType, rawPattern, limit) {
    ...
    try {
        ...
        return lodash_1.sortBy(response.aggregations.indices.buckets
            .map((bucket) => {
            return bucket.key;
        })
            .map((indexName) => {
        ...
        const type = lodash_1.get(err, 'body.error.caused_by.type');
