...
const collect_references_deep_1 = require("./collect_references_deep");
...
test('collects dashboard and all dependencies', async () => {
    const savedObjectClient = mocks_1.savedObjectsClientMock.create();
    savedObjectClient.bulkGet.mockImplementation(objects => {
        if (!objects) {
            throw new Error('Invalid test data');
        }
        return Promise.resolve({
            saved_objects: objects.map((obj) => data.find(row => row.id === obj.id && row.type === obj.type)),
        });
    });
    const objects = await collect_references_deep_1.collectReferencesDeep(savedObjectClient, [{ type: 'dashboard', id: '1' }]);
