...
function getAggParamsToRender({ agg, editorConfig, metricAggs, state, schemas, hideCustomLabel }, aggTypeFieldFilters) {
    const params = {
        basic: [],
        advanced: [],
    };
    ...
    paramsToRender.forEach((param, index) => {
        ...
        if (agg.type.subtype && agg_params_map_1.aggParamsMap[agg.type.subtype]) {
            ...
            paramEditor = lodash_1.get(aggParams, param.name) || lodash_1.get(agg_params_map_1.aggParamsMap, ['common', param.type]);
        }
...
function getAggTypeOptions(aggTypes, agg, indexPattern, groupName, allowedAggs) {
    const aggTypeOptions = public_1.search.aggs.aggTypeFilters.filter(aggTypes[groupName], indexPattern, agg, allowedAggs);
