...
function DefaultEditorSideBar({ isCollapsed, onClickCollapse, optionTabs, uiState, vis, isLinkedSearch, eventEmitter, savedSearch, timeRange, }) {
    ...
    const applyChanges = react_1.useCallback(() => {
        ...
        vis.setState({
            ...vis.serialize(),
            params: state.params,
            data: { aggs: state.data.aggs ? state.data.aggs.aggs.map(agg => agg.toJSON()) : [] },
        });
    ...
    const onSubmit = react_1.useCallback(event => {
        if (event.ctrlKey && event.keyCode === eui_1.keyCodes.ENTER) {
