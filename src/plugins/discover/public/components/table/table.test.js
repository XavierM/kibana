...
indexPattern.flattenHit = public_1.indexPatterns.flattenHitWrapper(indexPattern, indexPattern.metaFields);
describe('DocViewTable at Discover', () => {
    ...
    const component = enzyme_1.mount(react_1.default.createElement(table_1.DocViewTable, Object.assign({}, props)));
    [
        {
            _property: '_index',
        ...
        const rowComponent = test_1.findTestSubject(component, `tableDocViewRow-${check._property}`);
        ...
        [
            ...
            if (typeof elementExist === 'boolean') {
                const btn = test_1.findTestSubject(rowComponent, element);
        ...
        ['noMappingWarning'].forEach(element => {
            const elementExist = check[element];
            if (typeof elementExist === 'boolean') {
                const el = test_1.findTestSubject(rowComponent, element);
                it(`renders ${element} for '${check._property}' correctly`, () => {
...
describe('DocViewTable at Discover Doc', () => {
    ...
    const component = enzyme_1.mount(react_1.default.createElement(table_1.DocViewTable, Object.assign({}, props)));
    const foundLength = test_1.findTestSubject(component, 'addInclusiveFilterButton').length;
...
describe('DocViewTable at Discover Context', () => {
    ...
    it(`renders no toggleColumnButton`, () => {
        const foundLength = test_1.findTestSubject(component, 'toggleColumnButtons').length;
        expect(foundLength).toBe(0);
    });
    it(`renders addInclusiveFilterButton`, () => {
        const row = test_1.findTestSubject(component, `tableDocViewRow-_index`);
        const btn = test_1.findTestSubject(row, 'addInclusiveFilterButton');
        expect(btn.length).toBe(1);
        btn.simulate('click');
        expect(props.filter).toBeCalled();
    });
    it(`renders functional collapse button`, () => {
        const btn = test_1.findTestSubject(component, `collapseBtn`);
