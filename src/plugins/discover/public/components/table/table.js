...
function DocViewTable({ hit, indexPattern, filter, columns, onAddColumn, onRemoveColumn, }) {
    const mapping = indexPattern.fields.getByName;
    const flattened = indexPattern.flattenHit(hit);
    const formatted = indexPattern.formatHit(hit, 'html');
    const [fieldRowOpen, setFieldRowOpen] = react_1.useState({});
    function toggleValueCollapse(field) {
        fieldRowOpen[field] = fieldRowOpen[field] !== true;
        setFieldRowOpen({ ...fieldRowOpen });
    }
    return (react_1.default.createElement("table", { className: "table table-condensed kbnDocViewerTable" },
        react_1.default.createElement("tbody", null, Object.keys(flattened)
            .sort()
            .map(field => {
            const valueRaw = flattened[field];
            const value = table_helper_1.trimAngularSpan(String(formatted[field]));
            const isCollapsible = value.length > COLLAPSE_LINE_LENGTH;
            const isCollapsed = isCollapsible && !fieldRowOpen[field];
            const toggleColumn = onRemoveColumn && onAddColumn && Array.isArray(columns)
                ? () => {
                ...
                : undefined;
            const isArrayOfObjects = Array.isArray(flattened[field]) && table_helper_1.arrayContainsObjects(flattened[field]);
            ...
            const isNestedField = !indexPattern.fields.find(patternField => patternField.name === field) &&
                !!indexPattern.fields.find(patternField => {
                    // We only want to match a full path segment
                    const nestedRootRegex = new RegExp(lodash_1.escapeRegExp(field) + '(\\.|$)');
