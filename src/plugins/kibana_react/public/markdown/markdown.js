 ...
 */
exports.markdownFactory = lodash_1.memoize((whiteListedRules = [], openLinksInNewTab = false) => {
    let markdownIt;
    ...
    if (whiteListedRules && whiteListedRules.length > 0) {
        markdownIt = new markdown_it_1.default('zero', { html: false, linkify: true });
        markdownIt.enable(whiteListedRules);
    }
    else {
        markdownIt = new markdown_it_1.default({ html: false, linkify: true });
    }
    if (openLinksInNewTab) {
        // All links should open in new browser tab.
        // Define custom renderer to add 'target' attribute
        // https://github.com/markdown-it/markdown-it/blob/master/docs/architecture.md#renderer
        const originalLinkRender = markdownIt.renderer.rules.link_open ||
        ...
        markdownIt.renderer.rules.link_open = function (tokens, idx, options, env, self) {
            ...
            const target = '_blank';
            const rel = eui_1.getSecureRelForTarget({ href: href === null ? undefined : href, target });
...
class Markdown extends react_1.PureComponent {
    render() {
        const { className, markdown = '', openLinksInNewTab, whiteListedRules, ...rest } = this.props;
        const classes = classnames_1.default('kbnMarkdown__body', className);
        const markdownRenderer = exports.markdownFactory(whiteListedRules, openLinksInNewTab);
        const renderedMarkdown = markdownRenderer(markdown);
        return (react_1.default.createElement("div", Object.assign({}, rest, { className: classes, 
            /*
