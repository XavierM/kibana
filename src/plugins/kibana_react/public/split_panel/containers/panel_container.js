...
function PanelsContainer({ children, className, onPanelWidthChange, resizerClassName, }) {
    ...
    const handleKeyDown = react_1.useCallback((ev) => {
        const { keyCode } = ev;
        if (keyCode === eui_1.keyCodes.LEFT || keyCode === eui_1.keyCodes.RIGHT) {
            ev.preventDefault();
            const { current: registry } = registryRef;
            const [left, right] = registry.getPanels();
            const leftPercent = left.width - (keyCode === eui_1.keyCodes.LEFT ? 1 : -1);
            const rightPercent = right.width - (keyCode === eui_1.keyCodes.RIGHT ? 1 : -1);
