...
const mockMockErrorRenderFunction = jest.fn((el, error, handlers) => handlers.done());
...
describe('render helper function', () => {
    it('returns ExpressionRenderHandler instance', () => {
        const response = render_1.render(element, {});
        expect(response).toBeInstanceOf(render_1.ExpressionRenderHandler);
    });
});
describe('ExpressionRenderHandler', () => {
    it('constructor creates observers', () => {
        const expressionRenderHandler = new render_1.ExpressionRenderHandler(element);
        expect(expressionRenderHandler.events$).toBeInstanceOf(rxjs_1.Observable);
        expect(expressionRenderHandler.render$).toBeInstanceOf(rxjs_1.Observable);
        expect(expressionRenderHandler.update$).toBeInstanceOf(rxjs_1.Observable);
    });
    ...
    describe('render()', () => {
        ...
        it('in case of error render$ should emit when error renderer is finished', async () => {
            ...
            const promise1 = expressionRenderHandler.render$.pipe(operators_1.first()).toPromise();
            await expect(promise1).resolves.toEqual(1);
            expressionRenderHandler.render(false);
            const promise2 = expressionRenderHandler.render$.pipe(operators_1.first()).toPromise();
        ...
        it('should use custom error handler if provided', async () => {
            ...
            expect(getHandledError().message).toEqual(`invalid data provided to the expression renderer`);
        });
        it('should throw error if the rendering function throws', async () => {
            services_1.getRenderersRegistry.mockReturnValueOnce({ get: () => true });
            services_1.getRenderersRegistry.mockReturnValueOnce({
                get: () => ({
                    render: () => {
        ...
        it('default renderer should use notification service', async () => {
            ...
            expressionRenderHandler.render(false);
            await expect(promise1).resolves.toEqual(1);
            expect(mockNotificationService.toasts.addError).toBeCalledWith(expect.objectContaining({
        ...
        it("doesn't swallow sync render errors", async () => {
            const expressionRenderHandler1 = new render_1.ExpressionRenderHandler(element, {
                onRenderError: mockMockErrorRenderFunction,
            });
            expressionRenderHandler1.render(false);
            const renderPromiseAfterRender = expressionRenderHandler1.render$.pipe(operators_1.first()).toPromise();
            await expect(renderPromiseAfterRender).resolves.toEqual(1);
            expect(getHandledError().message).toEqual('invalid data provided to the expression renderer');
            mockMockErrorRenderFunction.mockClear();
            const expressionRenderHandler2 = new render_1.ExpressionRenderHandler(element, {
                onRenderError: mockMockErrorRenderFunction,
            });
            const renderPromiseBeforeRender = expressionRenderHandler2.render$.pipe(operators_1.first()).toPromise();
        ...
        it('should emit previous render and error results', async () => {
            const expressionRenderHandler = new render_1.ExpressionRenderHandler(element);
            expressionRenderHandler.render(false);
            const renderPromise = expressionRenderHandler.render$.pipe(operators_1.take(2), operators_1.toArray()).toPromise();
            expressionRenderHandler.render(false);
