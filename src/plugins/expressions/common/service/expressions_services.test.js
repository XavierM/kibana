...
const expressions_services_1 = require("./expressions_services");
describe('ExpressionsService', () => {
    test('can instantiate', () => {
        new expressions_services_1.ExpressionsService();
    });
    test('returns expected setup contract', () => {
        const expressions = new expressions_services_1.ExpressionsService();
    ...
    describe('.fork()', () => {
        test('returns a new ExpressionsService instance', () => {
            const service = new expressions_services_1.ExpressionsService();
            const fork = service.fork();
            expect(fork).not.toBe(service);
            expect(fork).toBeInstanceOf(expressions_services_1.ExpressionsService);
        });
        ...
        test('newly registered functions in fork are NOT available in origin', () => {
            ...
            expect(Object.values(fork.executor.state.get().functions)).toHaveLength(Object.values(service.executor.state.get().functions).length + 1);
        });
