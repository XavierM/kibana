...
const run = async (expression = 'foo bar=123', context, input = null) => {
...
describe('Execution', () => {
    ...
    describe('.contract', () => {
        test('is instance of ExecutionContract', () => {
            const execution = createExecution('add val=1');
            expect(execution.contract).toBeInstanceOf(execution_contract_1.ExecutionContract);
        });
    ...
    describe('debug mode', () => {
        ...
        test('can execute expression with sub-expression in debug mode', async () => {
            const execution = createExecution('add val={var_set name=foo value=5 | var name=foo} | add val=10', {}, true);
            execution.start(0);
            const result = await execution.result;
            expect(result).toEqual({
                type: 'num',
        ...
        describe('when functions succeed', () => {
            ...
            test('stores resolved arguments of a function', async () => {
                ...
                expect(chain[0].arguments.val[0].chain[0].debug.args).toEqual({
                    name: 'foo',
                    value: 5,
                });
            });
        ...
        describe('when expression throws', () => {
            ...
            test('stores debug information up until the function that throws', async () => {
                const execution = new execution_1.Execution({
                    executor,
                    ast: ast_1.parseExpression('add val=1 | throws | add val=3'),
                    debug: true,
                });
                execution.start(0);
                await execution.result;
