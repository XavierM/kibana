...
function scaleBounds(bounds) {
    ...
    let latDiff = lodash_1.default.round(Math.abs(topLeft.lat - bottomRight.lat), 5);
    const lonDiff = lodash_1.default.round(Math.abs(bottomRight.lon - topLeft.lon), 5);
    // map height can be zero when vis is first created
    if (latDiff === 0)
        latDiff = lonDiff;
    const latDelta = latDiff * scale;
    let topLeftLat = lodash_1.default.round(topLeft.lat, 5) + latDelta;
    if (topLeftLat > 90)
        topLeftLat = 90;
    let bottomRightLat = lodash_1.default.round(bottomRight.lat, 5) - latDelta;
    if (bottomRightLat < -90)
        bottomRightLat = -90;
    const lonDelta = lonDiff * scale;
    let topLeftLon = lodash_1.default.round(topLeft.lon, 5) - lonDelta;
    if (topLeftLon < -180)
        topLeftLon = -180;
    let bottomRightLon = lodash_1.default.round(bottomRight.lon, 5) + lonDelta;
