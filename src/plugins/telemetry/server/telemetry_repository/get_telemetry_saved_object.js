...
exports.getTelemetrySavedObject = async (repository) => {
    try {
        const { attributes } = await repository.get('telemetry', 'telemetry');
        return attributes;
    }
    catch (error) {
        if (server_1.SavedObjectsErrorHelpers.isNotFoundError(error)) {
            return null;
        }
        ...
        if (server_1.SavedObjectsErrorHelpers.isForbiddenError(error)) {
            return false;
        }
        throw error;
    }
