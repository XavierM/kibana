...
async function getSavedObjectsCounts(callCluster, kibanaIndex // Typically '.kibana'. We might need a way to obtain it from the SavedObjects client (or the SavedObjects client to provide a way to run aggregations?)
) {
    ...
    const allZeros = TYPES.reduce((acc, type) => ({ ...acc, [lodash_1.snakeCase(type)]: { total: 0 } }), {});
    // Add the doc_count from each bucket
    return buckets.reduce((acc, { key, doc_count: total }) => (total ? { ...acc, [lodash_1.snakeCase(key)]: { total } } : acc), allZeros);
}
