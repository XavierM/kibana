...
const telemetry_sender_1 = require("./telemetry_sender");
...
describe('TelemetrySender', () => {
    ...
    describe('constructor', () => {
        it('defaults lastReport if unset', () => {
            const telemetryService = mocks_1.mockTelemetryService();
            const telemetrySender = new telemetry_sender_1.TelemetrySender(telemetryService);
            expect(telemetrySender['lastReported']).toBeUndefined();
            expect(mockLocalStorage.getItem).toBeCalledTimes(1);
            expect(mockLocalStorage.getItem).toHaveBeenCalledWith(constants_1.LOCALSTORAGE_KEY);
        });
    ...
    describe('saveToBrowser', () => {
        it('uses lastReport', () => {
            ...
            expect(mockLocalStorage.setItem).toHaveBeenCalledWith(constants_1.LOCALSTORAGE_KEY, JSON.stringify({ lastReport }));
        });
    });
    describe('shouldSendReport', () => {
        ...
        it('returns true if lastReported passed REPORT_INTERVAL_MS', () => {
            const lastReported = Date.now() - (constants_1.REPORT_INTERVAL_MS + 1000);
            const telemetryService = mocks_1.mockTelemetryService();
            telemetryService.getIsOptedIn = jest.fn().mockReturnValue(true);
            const telemetrySender = new telemetry_sender_1.TelemetrySender(telemetryService);
