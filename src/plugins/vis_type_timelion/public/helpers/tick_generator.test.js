...
const tick_generator_1 = require("./tick_generator");
describe('Tick Generator', function () {
    let generateTicks;
    beforeEach(function () {
        generateTicks = tick_generator_1.generateTicksProvider();
    });
    ...
    describe('generateTicks()', function () {
        ...
        axes.forEach(axis => {
            it(`generates ticks from ${axis.min} to ${axis.max}`, function () {
                ...
                expect(ticks[Math.floor(ticks.length / 2)]).toEqual(axis.min + expectedDelta * Math.floor(ticks.length / 2));
