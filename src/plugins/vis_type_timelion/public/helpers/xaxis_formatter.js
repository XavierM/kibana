...
function xaxisFormatterProvider(config) {
    function getFormat(esInterval) {
        ...
        if (parts === null || parts[1] === null || parts[2] === null) {
            throw new Error(i18n_1.i18n.translate('timelion.panels.timechart.unknownIntervalErrorMessage', {
                defaultMessage: 'Unknown interval',
            }));
        }
        const interval = moment_1.default.duration(Number(parts[1]), parts[2]);
        // Cribbed from Kibana's TimeBuckets class
        const rules = config.get('dateFormat:scaled');
        for (let i = rules.length - 1; i >= 0; i--) {
            const rule = rules[i];
            if (!rule[0] || interval >= moment_1.default.duration(rule[0])) {
                return rule[1];
            }
        }
