...
function validateEsRoute(router) {
    router.get({
        ...
        if (lodash_1.default.has(resp, 'aggregations.maxAgg.value') && lodash_1.default.has(resp, 'aggregations.minAgg.value')) {
            return response.ok({
                body: {
                    ok: true,
                    field: timefield,
                    min: lodash_1.default.get(resp, 'aggregations.minAgg.value'),
                    max: lodash_1.default.get(resp, 'aggregations.maxAgg.value'),
                },
            });
        }
        return response.ok({
