 ...
 */
async function buildContextMenuForActions({ actions, actionContext, closeMenu, }) {
    ...
    return {
        id: 'mainMenu',
        title: i18n_1.i18n.translate('uiActions.actionPanel.title', {
...
async function buildEuiContextMenuPanelItems({ actions, actionContext, closeMenu, }) {
    ...
    const promises = actions.map(async (action) => {
        ...
        items.push(await convertPanelActionToContextMenuItem({
            action,
            actionContext,
            closeMenu,
        }));
