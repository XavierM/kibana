...
class ManagementSection {
    constructor({ id, title, order = 100, euiIconType, icon }, getSections, registerLegacyApp, getLegacyManagementSection, getStartServices) {
        this.id = '';
        this.title = '';
        this.apps = [];
        this.id = id;
    ...
    registerApp({ id, title, order, mount }) {
        if (this.getApp(id)) {
            throw new Error(`Management app already registered - id: ${id}, title: ${title}`);
        }
        const app = new management_app_1.ManagementApp({ id, title, order, mount, basePath: `/management/${this.id}/${id}` }, this.getSections, this.registerLegacyApp, this.getLegacyManagementSection, this.getStartServices);
        this.apps.push(app);
        return app;
    }
    getApp(id) {
