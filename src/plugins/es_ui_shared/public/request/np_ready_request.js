...
exports.sendRequest = async (httpClient, { path, method, body, query }) => {
    try {
...
exports.useRequest = (httpClient, { path, method, query, body, pollIntervalMs, initialData, deserializer = (data) => data, }) => {
    const sendRequestRef = react_1.useRef();
    ...
    const scheduleRequest = () => {
        ...
        if (pollInterval.current) {
            pollIntervalId.current = setTimeout(() => (sendRequestRef.current ?? _sendRequest)(), pollInterval.current);
        }
    };
    ...
    const stringifiedQuery = react_1.useMemo(() => JSON.stringify(query), [query]);
    react_1.useEffect(() => {
        (sendRequestRef.current ?? _sendRequest)();
    ...
    return {
        ...
        error,
        data,
        sendRequest: sendRequestRef.current ?? _sendRequest,
    };
};
