...
function fetchProvider(index) {
    return async (callCluster) => {
        ...
        const hits = lodash_1.get(response, 'hits.hits', []);
        ...
        return hits.reduce((telemetry, hit) => {
            ...
            const dataDate = moment_1.default.utc(hit._source.updated_at);
            if (isSampleDataSetInstalled) {
                const { lastDate, lastSet } = getLast(dataSet, dataDate, telemetry.last_install_set, telemetry.last_install_date);
                return {
                    ...telemetry,
                ...
                const { lastDate, lastSet } = getLast(dataSet, dataDate, telemetry.last_uninstall_set, telemetry.last_uninstall_date);
                return {
                    ...telemetry,
