...
exports.createListRoute = (router, sampleDatasets) => {
    router.get({ path: '/api/sample_data', validate: false }, async (context, req, res) => {
        ...
        const isInstalledPromises = registeredSampleDatasets.map(async (sampleDataset) => {
            for (let i = 0; i < sampleDataset.dataIndices.length; i++) {
                ...
                try {
                    const indexExists = await context.core.elasticsearch.dataClient.callAsCurrentUser('indices.exists', { index });
                    if (!indexExists) {
            ...
            try {
                ...
                if (boom_1.isBoom(err) && err.output.statusCode === 404) {
                    sampleDataset.status = NOT_INSTALLED;
                    return;
                }
                sampleDataset.status = UNKNOWN;
