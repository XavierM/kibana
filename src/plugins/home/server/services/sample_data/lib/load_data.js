...
function loadData(path, bulkInsert) {
    return new Promise((resolve, reject) => {
        ...
        const readStream = fs_1.default.createReadStream(path, { highWaterMark: 1024 * 4 });
        const lineStream = readline_1.default.createInterface({ input: readStream.pipe(zlib_1.createUnzip()) });
        const onClose = async () => {
            if (docs.length > 0) {
                try {
