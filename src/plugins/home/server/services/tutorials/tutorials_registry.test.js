...
const tutorials_registry_1 = require("./tutorials_registry");
const mocks_1 = require("../../../../../core/server/mocks");
const mocks_2 = require("../../../../../core/server/mocks");
const INVALID_TUTORIAL = {
    id: 'test',
    category: 'logging',
    name: '',
    isBeta: false,
...
describe('TutorialsRegistry', () => {
    let mockCoreSetup;
    let testProvider;
    let testScopedTutorialContextFactory;
    describe('GET /api/kibana/home/tutorials', () => {
        ...
        test('has a router that retrieves registered tutorials', () => {
            const mockResponse = mocks_2.httpServerMock.createResponseFactory();
            expect(mockResponse.ok.mock.calls).toMatchInlineSnapshot(`Array []`);
        });
    });
    describe('setup', () => {
        test('exposes proper contract', () => {
            const setup = new tutorials_registry_1.TutorialsRegistry().setup(mockCoreSetup);
        ...
        test('registerTutorial throws when registering a tutorial with an invalid schema', () => {
            ...
            expect(() => setup.registerTutorial(testProvider)).toThrowErrorMatchingInlineSnapshot(`"Unable to register tutorial spec because its invalid. ValidationError: child \\"name\\" fails because [\\"name\\" is not allowed to be empty]"`);
        });
        ...
        test('addScopedTutorialContextFactory throws when given a scopedTutorialContextFactory that is not a function', () => {
            const setup = new tutorials_registry_1.TutorialsRegistry().setup(mockCoreSetup);
            const testItem = {};
            expect(() => setup.addScopedTutorialContextFactory(testItem)).toThrowErrorMatchingInlineSnapshot(`"Unable to add scoped(request) context factory because you did not provide a function"`);
        });
        test('addScopedTutorialContextFactory adds a scopedTutorialContextFactory when given a function', () => {
            ...
            expect(() => setup.addScopedTutorialContextFactory(testScopedTutorialContextFactory)).not.toThrowError();
        });
    });
    describe('start', () => {
        test('exposes proper contract', () => {
            const start = new tutorials_registry_1.TutorialsRegistry().start();
