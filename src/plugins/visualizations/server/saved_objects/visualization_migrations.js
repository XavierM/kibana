...
const migratePercentileRankAggregation = doc => {
    const visStateJSON = lodash_1.get(doc, 'attributes.visState');
    let visState;
    if (visStateJSON) {
        try {
        ...
        if (visState && visState.type === 'metrics') {
            const series = lodash_1.get(visState, 'params.series') || [];
            series.forEach(part => {
                (part.metrics || []).forEach((metric) => {
                    if (metric.type === 'percentile_rank' && lodash_1.has(metric, 'value')) {
...
const migrateOperatorKeyTypo = doc => {
    ...
    if (visStateJSON) {
        ...
        if (visState && visState.type === 'metrics') {
            const gaugeColorRules = lodash_1.get(visState, 'params.gauge_color_rules') || [];
    ...
    if (visStateJSON) {
        ...
        if (visState && visState.aggs) {
            visState.aggs.forEach((agg) => {
                ...
                if (lodash_1.get(agg, 'params.customBucket.type', null) === 'date_histogram' &&
...
const transformFilterStringToQueryObject = (doc, logger) => {
    ...
    if (visStateJSON) {
        ...
        if (visState) {
            const visType = lodash_1.get(visState, 'params.type');
            ...
            const params = lodash_1.get(visState, 'params');
            ...
            const annotations = lodash_1.get(visState, 'params.annotations') || [];
            ...
            series.forEach(item => {
                ...
                if (item.split_filters) {
                    const splitFilters = lodash_1.get(item, 'split_filters') || [];
                    ...
                    if (part.metrics && Array.isArray(part.metrics)) {
                        part.metrics.forEach((metric) => {
                            if (metric.type === 'moving_average') {
                                ...
                                metric.alpha = lodash_1.get(metric, 'settings.alpha', 0.3);
                                metric.beta = lodash_1.get(metric, 'settings.beta', 0.1);
                                metric.gamma = lodash_1.get(metric, 'settings.gamma', 0.3);
                                metric.period = lodash_1.get(metric, 'settings.period', 1);
                                metric.multiplicative = lodash_1.get(metric, 'settings.type') === 'mult';
...
const addDocReferences = doc => ({
    ...doc,
    references: doc.references || [],
});
const migrateSavedSearch = doc => {
    const savedSearchId = lodash_1.get(doc, 'attributes.savedSearchId');
...
const migrateControls = doc => {
    ...
    if (visStateJSON) {
        ...
        if (visState) {
            const controls = lodash_1.get(visState, 'params.controls') || [];
...
const migrateTableSplits = doc => {
    try {
        ...
        if (lodash_1.get(visState, 'type') !== 'table') {
        ...
        visState.aggs = visState.aggs.map((agg) => {
            ...
            agg.params = lodash_1.omit(agg.params, ['row']);
            return agg;
        });
        if (splitCount <= 1) {
...
exports.visualizationSavedObjectTypeMigrations = {
    /**
     ...
     */
    '6.7.2': lodash_1.flow(migrateMatchAllQuery, removeDateHistogramTimeZones),
    '7.0.0': lodash_1.flow(addDocReferences, migrateIndexPattern, migrateSavedSearch, migrateControls, migrateTableSplits),
    '7.0.1': lodash_1.flow(removeDateHistogramTimeZones),
    '7.2.0': lodash_1.flow(migratePercentileRankAggregation, migrateDateHistogramAggregation),
    '7.3.0': lodash_1.flow(migrateGaugeVerticalSplitToAlignment, transformFilterStringToQueryObject, migrateFiltersAggQuery, replaceMovAvgToMovFn),
    '7.3.1': lodash_1.flow(migrateFiltersAggQueryStringQueries),
    '7.4.2': lodash_1.flow(transformSplitFiltersStringToQueryObject),
    '7.7.0': lodash_1.flow(migrateOperatorKeyTypo),
    '7.8.0': lodash_1.flow(migrateTsvbDefaultColorPalettes),
};
