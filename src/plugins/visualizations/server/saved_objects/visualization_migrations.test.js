...
const visualization_migrations_1 = require("./visualization_migrations");
const savedObjectMigrationContext = null;
describe('migration visualization', () => {
    describe('6.7.2', () => {
        const migrate = (doc) => visualization_migrations_1.visualizationSavedObjectTypeMigrations['6.7.2'](doc, savedObjectMigrationContext);
        let doc;
    ...
    describe('7.0.0', () => {
        const migrate = (doc) => visualization_migrations_1.visualizationSavedObjectTypeMigrations['7.0.0'](doc, savedObjectMigrationContext);
    ...
    describe('7.2.0', () => {
        describe('date histogram custom interval removal', () => {
            const migrate = (doc) => visualization_migrations_1.visualizationSavedObjectTypeMigrations['7.2.0'](doc, savedObjectMigrationContext);
            let doc;
            beforeEach(() => {
                doc = {
                    attributes: {
            ...
            it('should remove customInterval from nested aggregations and set interval with customInterval value', () => {
                const migratedDoc = migrate(doc);
                const { aggs } = JSON.parse(migratedDoc.attributes.visState);
                expect(aggs[3].params.customBucket.params.interval).toBe(JSON.parse(doc.attributes.visState).aggs[3].params.customBucket.params.customInterval);
                expect(aggs[3]).not.toHaveProperty('params.customBucket.params.customInterval');
            });
    ...
    describe('7.3.0', () => {
        ...
        const logger = {
            log: {
                warn: (msg) => logMsgArr.push(msg),
            },
        };
        const migrate = (doc) => visualization_migrations_1.visualizationSavedObjectTypeMigrations['7.3.0'](doc, logger);
    ...
    describe('7.3.0 tsvb', () => {
        const migrate = (doc) => visualization_migrations_1.visualizationSavedObjectTypeMigrations['7.3.0'](doc, savedObjectMigrationContext);
        ...
        it('should not change a series item filter string in the object after migration', () => {
            ...
            expect(markdownSeries[0].split_filters[0].filter.query).toBe(JSON.parse(markdownDoc.attributes.visState).params.series[0].split_filters[0].filter);
        });
    ...
    describe('7.3.1', () => {
        const migrate = (doc) => visualization_migrations_1.visualizationSavedObjectTypeMigrations['7.3.1'](doc, savedObjectMigrationContext);
    ...
    describe('7.4.2 tsvb split_filters migration', () => {
        const migrate = (doc) => visualization_migrations_1.visualizationSavedObjectTypeMigrations['7.4.2'](doc, savedObjectMigrationContext);
        const generateDoc = (params) => ({
            attributes: {
                title: 'My Vis',
                description: 'This is my super cool vis.',
        ...
        it('should change series item filters from a string into an object for all filters', () => {
            ...
            const migratedtimeSeriesDoc = migrate(timeSeriesDoc);
            const timeSeriesParams = JSON.parse(migratedtimeSeriesDoc.attributes.visState).params;
            expect(Object.keys(timeSeriesParams.filter)).toEqual(expect.arrayContaining(['query', 'language']));
    ...
    describe('7.7.0 tsvb opperator typo migration', () => {
        const migrate = (doc) => visualization_migrations_1.visualizationSavedObjectTypeMigrations['7.7.0'](doc, savedObjectMigrationContext);
        const generateDoc = (params) => ({
            attributes: {
                title: 'My Vis',
                description: 'This is my super cool vis.',
    ...
    describe('7.8.0 tsvb split_color_mode', () => {
        const migrate = (doc) => visualization_migrations_1.visualizationSavedObjectTypeMigrations['7.8.0'](doc, savedObjectMigrationContext);
        const generateDoc = (params) => ({
            attributes: {
                title: 'My Vis',
                type: 'visualization',
