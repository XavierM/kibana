...
const toPath_1 = tslib_1.__importDefault(require("lodash/internal/toPath"));
...
class PersistedState extends events_1.EventEmitter {
    ...
    reset(path) {
        ...
        if (origValue === undefined) {
            ...
            lodash_1.set(this._mergedState, keyPath, origValue);
        }
    ...
    getIndex(key) {
        if (key === undefined)
            return this._path;
        return [...(this._path || []), ...toPath_1.default(key)];
    }
    getPartialIndex(key) {
    ...
    setValue(key, value, silent = false) {
        ...
        if (initialState) {
            ...
            if (!this.hasPath() && key === undefined)
                ...
                this._defaultState = lodash_1.set({}, keyPath, value);
        }
        if (!initialState) {
            ...
            if (!this.hasPath() && key === undefined) {
                ...
                if (Array.isArray(lodash_1.get(this._mergedState, keyPath))) {
                    if (hasKeyPath) {
                        lodash_1.set(this._mergedState, keyPath, undefined);
                    }
                    else {
                ...
                if (hasKeyPath) {
                    lodash_1.set(this._changedState, keyPath, value);
                }
                else {
        ...
        this._mergedState = lodash_1.merge(targetObj, sourceObj, mergeMethod);
