...
describe('extractReferences', () => {
    test('extracts nothing if savedSearchId is empty', () => {
        ...
        const updatedDoc = saved_visualization_references_1.extractReferences(doc);
...
describe('injectReferences', () => {
    test('injects nothing when savedSearchRefName is null', () => {
        ...
        saved_visualization_references_1.injectReferences(context, []);
...
`);
    ...
    test('injects references into context', () => {
        ...
        saved_visualization_references_1.injectReferences(context, references);
        expect(context).toMatchInlineSnapshot(`
Object {
  "id": "1",
  "savedSearchId": "123",
...
`);
    ...
    test(`fails when it can't find the saved search reference in the array`, () => {
        ...
        expect(() => saved_visualization_references_1.injectReferences(context, [])).toThrowErrorMatchingInlineSnapshot(`"Could not find saved search reference \\"search_0\\""`);
    });
    test(`fails when it can't find the index pattern reference in the array`, () => {
        ...
        expect(() => saved_visualization_references_1.injectReferences(context, [])).toThrowErrorMatchingInlineSnapshot(`"Could not find index pattern reference \\"control_0_index_pattern\\""`);
    });
});
