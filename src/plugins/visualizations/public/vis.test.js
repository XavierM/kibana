...
jest.mock('./services', () => {
    ...
    return {
        ...
        getAggs: () => ({
            createAggConfigs: (indexPattern, cfg) => ({
                aggs: cfg.map((aggConfig) => ({ ...aggConfig, toJSON: () => aggConfig })),
            }),
        }),
    };
});
describe('Vis Class', function () {
    ...
    const stateFixture = {
        ...
        data: {
            ...
            searchSource: {
                getField: (name) => {
                    if (name === 'index') {
                        return stubbed_logstash_index_pattern_1.default();
                    }
                },
                createCopy: jest.fn(),
            },
    ...
    beforeEach(function () {
        vis = new vis_1.Vis('test', stateFixture);
    });
    const verifyVis = function (visToVerify) {
