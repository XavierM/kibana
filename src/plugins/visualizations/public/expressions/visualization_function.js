...
exports.visualization = () => ({
    name: 'visualization',
    type: 'render',
    help: i18n_1.i18n.translate('visualizations.functions.visualization.help', {
    ...
    async fn(input, args, { inspectorAdapters }) {
        ...
        const visType = services_1.getTypes().get(args.type || 'histogram');
        const indexPattern = args.index ? await services_1.getIndexPatterns().get(args.index) : null;
        const uiStateParams = args.uiState ? JSON.parse(args.uiState) : {};
        const uiState = new public_1.PersistedState(uiStateParams);
        if (typeof visType.requestHandler === 'function') {
            input = await visType.requestHandler({
                ...
                uiState,
                inspectorAdapters,
                queryFilter: services_1.getFilterManager(),
                forceFetch: true,
            });
        }
