...
const getWrapper = async (props) => {
    const wrapper = enzyme_1.mount(react_1.default.createElement(react_2.I18nProvider, null,
        react_1.default.createElement(legend_1.VisLegend, Object.assign({ position: "top", vis: vis, vislibVis: vislibVis, visData: visData, uiState: uiState }, props))));
    await wrapper.find(legend_1.VisLegend).instance().refresh();
    wrapper.update();
    return wrapper;
};
const getLegendItems = (wrapper) => wrapper.find('.visLegend__button');
describe('VisLegend Component', () => {
    ...
    describe('Filtering', () => {
        ...
        it('should filter out when clicked', () => {
            ...
            const filterGroup = wrapper.find(eui_1.EuiButtonGroup).first();
    ...
    describe('setColor', () => {
        ...
        it('sets the color in the UI state', () => {
            ...
            expect(colors.A).toBe(models_1.legendColors[0]);
        });
    });
