...
const _add_to_siri_1 = require("./_add_to_siri");
function getSeries(table, chart) {
    ...
    const partGetPoint = lodash_1.partial(_get_point_1.getPoint, table, xAspect, aspects.series);
    ...
    table.rows.forEach((row, rowIndex) => {
        if (!multiY) {
            ...
            if (point) {
                ...
                point.seriesId = id;
                _add_to_siri_1.addToSiri(seriesMap, point, id, point.series, yAspect.format, zAspect && zAspect.format, zAspect && zAspect.title);
            }
            return;
        }
        aspects.y.forEach(function (y) {
            ...
            _add_to_siri_1.addToSiri(seriesMap, point, seriesId, seriesLabel, y.format, zAspect && zAspect.format, zAspect && zAspect.title);
        });
    });
