...
const build_hierarchical_data_1 = require("./build_hierarchical_data");
...
describe('buildHierarchicalData convertTable', () => {
    describe('metric only', () => {
        ...
        it('should set the slices with one child to a consistent label', () => {
            const results = build_hierarchical_data_1.buildHierarchicalData(table, dimensions);
    ...
    describe('threeTermBuckets', () => {
        ...
        it('should set the correct hits attribute for each of the results', () => {
            tables.forEach(t => {
                const results = build_hierarchical_data_1.buildHierarchicalData(t.tables[0], dimensions);
                expect(results).toHaveProperty('hits');
                expect(results.hits).toBe(4);
            });
        });
        it('should set the correct names for each of the results', () => {
            const results0 = build_hierarchical_data_1.buildHierarchicalData(tables[0].tables[0], dimensions);
            expect(results0).toHaveProperty('names');
            expect(results0.names).toHaveLength(5);
            const results1 = build_hierarchical_data_1.buildHierarchicalData(tables[1].tables[0], dimensions);
            expect(results1).toHaveProperty('names');
            expect(results1.names).toHaveLength(5);
            const results2 = build_hierarchical_data_1.buildHierarchicalData(tables[2].tables[0], dimensions);
