...
// @ts-ignore
const response_handler_1 = require("./vislib/response_handler");
...
describe('interpreter/functions#pie', () => {
    const fn = utils_1.functionWrapper(pie_fn_1.createPieVisFn());
    const context = {
    ...
    it('calls response handler with correct values', async () => {
        ...
        expect(response_handler_1.vislibSlicesResponseHandler).toHaveBeenCalledTimes(1);
        expect(response_handler_1.vislibSlicesResponseHandler).toHaveBeenCalledWith(context, visConfig.dimensions);
    });
});
