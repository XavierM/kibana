...
describe('SharePlugin', () => {
    beforeEach(() => {
        plugin_test_mocks_1.managerMock.start.mockClear();
    ...
    describe('setup', () => {
        test('wires up and returns registry', async () => {
            ...
            const setup = await new plugin_1.SharePlugin().setup(coreSetup);
            expect(plugin_test_mocks_1.registryMock.setup).toHaveBeenCalledWith();
            expect(setup.register).toBeDefined();
        });
        test('registers redirect app', async () => {
            const coreSetup = mocks_1.coreMock.createSetup();
            await new plugin_1.SharePlugin().setup(coreSetup);
    ...
    describe('start', () => {
        test('wires up and returns show function, but not registry', async () => {
            const coreSetup = mocks_1.coreMock.createSetup();
            const service = new plugin_1.SharePlugin();
            await service.setup(coreSetup);
            const start = await service.start({});
            expect(plugin_test_mocks_1.registryMock.start).toHaveBeenCalled();
            expect(plugin_test_mocks_1.managerMock.start).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ getShareMenuItems: expect.any(Function) }));
