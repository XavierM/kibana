...
const share_menu_registry_1 = require("./share_menu_registry");
describe('ShareActionsRegistry', () => {
    describe('setup', () => {
        test('throws when registering duplicate id', () => {
            const setup = new share_menu_registry_1.ShareMenuRegistry().setup();
            setup.register({
                id: 'myTest',
                getShareMenuItems: () => [],
            });
            expect(() => setup.register({
                id: 'myTest',
                getShareMenuItems: () => [],
            })).toThrowErrorMatchingInlineSnapshot(`"Share menu provider with id [myTest] has already been registered. Use a unique id."`);
        });
    });
    describe('start', () => {
        describe('getActions', () => {
            test('returns a flat list of actions returned by all providers', () => {
                const service = new share_menu_registry_1.ShareMenuRegistry();
