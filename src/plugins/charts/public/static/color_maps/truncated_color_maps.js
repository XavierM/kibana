...
exports.truncatedColorMaps = {};
const colormaps = color_maps_1.vislibColorMaps;
for (const key in colormaps) {
    if (colormaps.hasOwnProperty(key)) {
        // slice off lightest colors
        // @ts-ignore
        const color = colormaps[key];
        exports.truncatedColorMaps[key] = {
            ...color,
            value: color.value.slice(Math.floor(color.value.length / 4)),
        };
    }
}
exports.truncatedColorSchemas = Object.values(exports.truncatedColorMaps).map(({ id, label }) => ({
    value: id,
    text: label,
}));
