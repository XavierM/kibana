...
class MappedColors {
    ...
    getConfigColorMapping() {
        return lodash_1.default.mapValues(this.uiSettings.get('visualization:colorMapping'), standardizeColor);
    }
    get oldMap() {
        return this._oldMap;
    }
    ...
    mapKeys(keys) {
        ...
        lodash_1.default.each(keys, key => {
            ...
            if (lodash_1.default.contains(configColors, this._mapping[key]))
                keysToMap.push(key);
        ...
        const allColors = lodash_1.default(this._mapping)
            .values()
            .union(configColors)
            .union(oldColors)
            .value();
        ...
        while (keysToMap.length > newColors.length) {
            newColors = newColors.concat(lodash_1.default.sample(allColors, keysToMap.length - newColors.length));
        }
