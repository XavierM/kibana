...
const INTERVAL_STRING_RE = new RegExp('^([0-9\\.]*)\\s*(' + datemath_1.default.units.join('|') + ')$');
function parseInterval(interval) {
    ...
    try {
        ...
        const duration = moment_1.default.duration(value, unit);
        ...
        const selectedUnit = lodash_1.find(datemath_1.default.units, u => Math.abs(duration.as(u)) >= 1);
        ...
        if (datemath_1.default.units.indexOf(selectedUnit) < datemath_1.default.units.indexOf(unit)) {
            return duration;
        }
        return moment_1.default.duration(duration.as(selectedUnit), selectedUnit);
    }
    catch (e) {
        return null;
    }
