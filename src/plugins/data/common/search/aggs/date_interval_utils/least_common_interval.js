...
const least_common_multiple_1 = require("./least_common_multiple");
...
function leastCommonInterval(a, b) {
    const { unitsMap, unitsDesc } = datemath_1.default;
    const aInt = parse_es_interval_1.parseEsInterval(a);
    const bInt = parse_es_interval_1.parseEsInterval(b);
    if (a === b) {
        return a;
    }
    ...
    const lcmMs = least_common_multiple_1.leastCommonMultiple(aMs, bMs);
