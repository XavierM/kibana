...
const mapFilter = (filter, comparators, excludedAttributes) => {
    const cleaned = lodash_1.omit(filter, excludedAttributes);
    ...
    if (comparators.alias)
        cleaned.alias = filter.meta?.alias;
    return cleaned;
};
const mapFilterArray = (filters, comparators, excludedAttributes) => {
    return lodash_1.map(filters, (filter) => mapFilter(filter, comparators, excludedAttributes));
};
/**
 ...
 */
exports.compareFilters = (first, second, comparatorOptions = {}) => {
    if (!first || !second)
        return false;
    let comparators = {};
    const excludedAttributes = ['$$hashKey', 'meta'];
    comparators = lodash_1.defaults(comparatorOptions || {}, {
        state: false,
        negate: false,
        disabled: false,
        alias: false,
    ...
    if (Array.isArray(first) && Array.isArray(second)) {
        return lodash_1.isEqual(mapFilterArray(first, comparators, excludedAttributes), mapFilterArray(second, comparators, excludedAttributes));
    }
    else if (!Array.isArray(first) && !Array.isArray(second)) {
        return lodash_1.isEqual(mapFilter(first, comparators, excludedAttributes), mapFilter(second, comparators, excludedAttributes));
    }
    else {
        return false;
    }
