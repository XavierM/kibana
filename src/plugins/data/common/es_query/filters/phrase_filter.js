...
exports.isPhraseFilter = (filter) => {
    const isMatchPhraseQuery = filter && filter.query && filter.query.match_phrase;
    const isDeprecatedMatchPhraseQuery = filter &&
        filter.query &&
        filter.query.match &&
        Object.values(filter.query.match).find((params) => params.type === 'phrase');
    return Boolean(isMatchPhraseQuery || isDeprecatedMatchPhraseQuery);
};
exports.isScriptedPhraseFilter = (filter) => Boolean(lodash_1.get(filter, 'script.script.params.value'));
exports.getPhraseFilterField = (filter) => {
    const queryConfig = filter.query.match_phrase || filter.query.match;
    return Object.keys(queryConfig)[0];
};
exports.getPhraseFilterValue = (filter) => {
    const queryConfig = filter.query.match_phrase || filter.query.match;
    const queryValue = Object.values(queryConfig)[0];
    return lodash_1.isPlainObject(queryValue) ? queryValue.query : queryValue;
};
exports.buildPhraseFilter = (field, value, indexPattern) => {
    const convertedValue = exports.getConvertedValueForField(field, value);
    if (field.scripted) {
        return {
            meta: { index: indexPattern.id, field: field.name },
            script: exports.getPhraseScript(field, value),
        };
    }
    else {
        return {
...
exports.getPhraseScript = (field, value) => {
    const convertedValue = exports.getConvertedValueForField(field, value);
    const script = exports.buildInlineScriptForPhraseFilter(field);
    return {
        script: {
            source: script,
            lang: field.lang,
...
exports.getConvertedValueForField = (field, value) => {
 ...
 */
exports.buildInlineScriptForPhraseFilter = (scriptedField) => {
