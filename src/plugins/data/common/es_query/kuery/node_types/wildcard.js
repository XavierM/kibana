...
exports.wildcardSymbol = '@kuery-wildcard@';
...
function buildNode(value) {
    if (!value.includes(exports.wildcardSymbol)) {
        return ast_1.fromLiteralExpression(value);
    }
    return {
        type: 'wildcard',
        value,
...
function toElasticsearchQuery(node) {
    const { value } = node;
    return value.split(exports.wildcardSymbol).join('*');
}
...
function hasLeadingWildcard(node) {
    ...
    return value.startsWith(exports.wildcardSymbol) && value.replace(exports.wildcardSymbol, '').length > 0;
}
