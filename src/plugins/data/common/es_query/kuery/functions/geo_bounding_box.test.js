    ...
    describe('geoBoundingBox', () => {
        ...
        describe('buildNodeParams', () => {
            ...
            test('arguments should contain the provided params as named arguments with "lat, lon" string values', () => {
                ...
                args.map((param) => {
                    ...
                    const { lat, lon } = lodash_1.get(params, param.name);
        ...
        describe('toElasticsearchQuery', () => {
            test('should return an ES geo_bounding_box query representing the given node', () => {
                const node = node_types_1.nodeTypes.function.buildNode('geoBoundingBox', 'geo', params);
            ...
            test('should throw an error for scripted fields', () => {
                const node = node_types_1.nodeTypes.function.buildNode('geoBoundingBox', 'script number', params);
                expect(() => geoBoundingBox.toElasticsearchQuery(node, indexPattern)).toThrowError(/Geo bounding box query does not support scripted fields/);
            });
            test('should use a provided nested context to create a full field name', () => {
                const node = node_types_1.nodeTypes.function.buildNode('geoBoundingBox', 'geo', params);
                const result = geoBoundingBox.toElasticsearchQuery(node, indexPattern, {}, { nested: { path: 'nestedField' } });
                expect(result).toHaveProperty('geo_bounding_box');
