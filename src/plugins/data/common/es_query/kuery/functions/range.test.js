    ...
    describe('range', () => {
        ...
        describe('buildNodeParams', () => {
            ...
            test('arguments should contain the provided params as named arguments', () => {
                ...
                params.map((param) => {
                    ...
                    expect(param.value.value).toBe(lodash_1.get(givenParams, param.name));
                });
            });
        });
        describe('toElasticsearchQuery', () => {
            test("should return an ES range query for the node's field and params", () => {
                ...
                const node = node_types_1.nodeTypes.function.buildNode('range', 'bytes', { gt: 1000, lt: 8000 });
            ...
            test('should support wildcard field names', () => {
                ...
                const node = node_types_1.nodeTypes.function.buildNode('range', 'byt*', { gt: 1000, lt: 8000 });
                const result = range.toElasticsearchQuery(node, indexPattern);
                expect(result).toEqual(expected);
            });
            test('should support scripted fields', () => {
                const node = node_types_1.nodeTypes.function.buildNode('range', 'script number', { gt: 1000, lt: 8000 });
                const result = range.toElasticsearchQuery(node, indexPattern);
            ...
            test('should support date fields without a dateFormat provided', () => {
                ...
                const node = node_types_1.nodeTypes.function.buildNode('range', '@timestamp', {
                    gt: '2018-01-03T19:04:17',
                    lt: '2018-04-03T19:04:17',
                });
                const result = range.toElasticsearchQuery(node, indexPattern);
            ...
            test('should use a provided nested context to create a full field name', () => {
                ...
                const node = node_types_1.nodeTypes.function.buildNode('range', 'bytes', { gt: 1000, lt: 8000 });
                const result = range.toElasticsearchQuery(node, indexPattern, {}, { nested: { path: 'nestedField' } });
                expect(result).toEqual(expected);
            });
            test('should automatically add a nested query when a wildcard field name covers a nested field', () => {
                ...
                const node = node_types_1.nodeTypes.function.buildNode('range', '*doublyNested*', {
                    gt: 1000,
                    lt: 8000,
                });
                const result = range.toElasticsearchQuery(node, indexPattern);
