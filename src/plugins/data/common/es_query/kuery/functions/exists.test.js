    ...
    describe('exists', () => {
        ...
        describe('toElasticsearchQuery', () => {
            test('should return an ES exists query', () => {
                ...
                const existsNode = node_types_1.nodeTypes.function.buildNode('exists', 'response');
            ...
            test('should throw an error for scripted fields', () => {
                const existsNode = node_types_1.nodeTypes.function.buildNode('exists', 'script string');
                expect(() => exists.toElasticsearchQuery(existsNode, indexPattern)).toThrowError(/Exists query does not support scripted fields/);
            });
            test('should use a provided nested context to create a full field name', () => {
                ...
                const existsNode = node_types_1.nodeTypes.function.buildNode('exists', 'response');
                const result = exists.toElasticsearchQuery(existsNode, indexPattern, {}, { nested: { path: 'nestedField' } });
                expect(expected).toEqual(result);
            });
        });
    });
