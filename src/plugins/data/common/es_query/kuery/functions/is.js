...
const wildcard = tslib_1.__importStar(require("../node_types/wildcard"));
function buildNodeParams(fieldName, value, isPhrase = false) {
    if (lodash_1.isUndefined(fieldName)) {
...
function toElasticsearchQuery(node, indexPattern, config = {}, context = {}) {
    ...
    const value = !lodash_1.isUndefined(valueArg) ? ast.toElasticsearchQuery(valueArg) : valueArg;
    ...
    const isAllFieldsQuery = (fullFieldNameArg.type === 'wildcard' && fieldName === '*') ||
    ...
    const queries = fields.reduce((accumulator, field) => {
        const wrapWithNestedQuery = (query) => {
            ...
            if (!(fullFieldNameArg.type === 'wildcard') ||
                !lodash_1.get(field, 'subType.nested') ||
                context?.nested) {
                return query;
            }
            else {
        ...
        if (field.scripted) {
            ...
            if (!isExistsQuery) {
                return [
                    ...
                    {
                        script: {
                            ...filters_1.getPhraseScript(field, value),
                        },
                    },
                ];
            }
