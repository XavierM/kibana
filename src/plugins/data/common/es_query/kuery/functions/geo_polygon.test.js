    ...
    describe('geoPolygon', () => {
        ...
        describe('toElasticsearchQuery', () => {
            test('should return an ES geo_polygon query representing the given node', () => {
                const node = node_types_1.nodeTypes.function.buildNode('geoPolygon', 'geo', points);
            ...
            test('should throw an error for scripted fields', () => {
                const node = node_types_1.nodeTypes.function.buildNode('geoPolygon', 'script number', points);
                expect(() => geoPolygon.toElasticsearchQuery(node, indexPattern)).toThrowError(/Geo polygon query does not support scripted fields/);
            });
            test('should use a provided nested context to create a full field name', () => {
                const node = node_types_1.nodeTypes.function.buildNode('geoPolygon', 'geo', points);
                const result = geoPolygon.toElasticsearchQuery(node, indexPattern, {}, { nested: { path: 'nestedField' } });
                expect(result).toHaveProperty('geo_polygon');
