...
const childNode = node_types_1.nodeTypes.function.buildNode('is', 'child', 'foo');
    ...
    describe('nested', () => {
        ...
        describe('toElasticsearchQuery', () => {
            test('should wrap subqueries in an ES nested query', () => {
                const node = node_types_1.nodeTypes.function.buildNode('nested', 'nestedField', childNode);
            ...
            test('should pass the nested path to subqueries so the full field name can be used', () => {
                const node = node_types_1.nodeTypes.function.buildNode('nested', 'nestedField', childNode);
                const result = nested.toElasticsearchQuery(node, indexPattern);
                const expectedSubQuery = ast.toElasticsearchQuery(node_types_1.nodeTypes.function.buildNode('is', 'nestedField.child', 'foo'));
