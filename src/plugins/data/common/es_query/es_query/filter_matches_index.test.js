...
describe('filterMatchesIndex', () => {
    it('should return true if the filter has no meta', () => {
        ...
        expect(filter_matches_index_1.filterMatchesIndex(filter, indexPattern)).toBe(true);
    });
    it('should return true if no index pattern is passed', () => {
        const filter = { meta: { index: 'foo', key: 'bar' } };
        expect(filter_matches_index_1.filterMatchesIndex(filter, undefined)).toBe(true);
    });
    ...
    it('should return false if the filter key does not match a field name', () => {
        ...
        const indexPattern = { id: 'foo', fields: [{ name: 'bar' }] };
        expect(filter_matches_index_1.filterMatchesIndex(filter, indexPattern)).toBe(false);
    });
    it('should return true if the filter has meta without a key', () => {
        const filter = { meta: { index: 'foo' } };
        const indexPattern = { id: 'foo', fields: [{ name: 'bar' }] };
        expect(filter_matches_index_1.filterMatchesIndex(filter, indexPattern)).toBe(true);
    });
});
