...
function isDeprecatedMatchPhraseFilter(filter) {
    ...
    return Boolean(fieldName && lodash_1.get(filter, ['query', 'match', fieldName, 'type']) === 'phrase');
}
function migrateFilter(filter, indexPattern) {
    if (isDeprecatedMatchPhraseFilter(filter)) {
        ...
        const params = lodash_1.get(filter, ['query', 'match', fieldName]);
        if (indexPattern) {
            const field = indexPattern.fields.find(f => f.name === fieldName);
            if (field) {
                params.query = filters_1.getConvertedValueForField(field, params.query);
            }
        }
        return {
            ...filter,
            query: {
                match_phrase: {
                    [fieldName]: lodash_1.omit(params, 'type'),
                },
            },
        };
    }
