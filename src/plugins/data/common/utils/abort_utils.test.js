...
const abort_utils_1 = require("./abort_utils");
jest.useFakeTimers();
...
describe('AbortUtils', () => {
    describe('AbortError', () => {
        test('should preserve `message`', () => {
            ...
            const error = new abort_utils_1.AbortError(message);
            expect(error.message).toBe(message);
        });
        test('should have a name of "AbortError"', () => {
            const error = new abort_utils_1.AbortError();
    ...
    describe('toPromise', () => {
        describe('resolves', () => {
            test('should not resolve if the signal does not abort', async () => {
                ...
                const promise = abort_utils_1.toPromise(controller.signal);
        ...
        describe('rejects', () => {
            test('should not reject if the signal does not abort', async () => {
                const controller = new AbortController();
                const promise = abort_utils_1.toPromise(controller.signal, true);
    ...
    describe('getCombinedSignal', () => {
        test('should return an AbortSignal', () => {
            const signal = abort_utils_1.getCombinedSignal([]);
        ...
        test('should not abort if none of the signals abort', async () => {
            ...
            const signal = abort_utils_1.getCombinedSignal([controller1.signal, controller2.signal]);
