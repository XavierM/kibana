...
class IndexPatternsFetcher {
    ...
    async getFieldsForWildcard(options) {
        ...
        return await lib_1.getFieldCapabilities(this._callDataCluster, pattern, metaFields);
    }
    /**
    ...
    async getFieldsForTimePattern(options) {
        ...
        const { matches } = await lib_1.resolveTimePattern(this._callDataCluster, pattern);
        const indices = matches.slice(0, lookBack);
        if (indices.length === 0) {
            throw lib_1.createNoMatchingIndicesError(pattern);
        }
        return await lib_1.getFieldCapabilities(this._callDataCluster, indices, metaFields);
    }
}
