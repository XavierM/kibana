...
async function getFieldCapabilities(callCluster, indices = [], metaFields = []) {
    ...
    const fieldsFromFieldCapsByName = lodash_1.indexBy(field_caps_response_1.readFieldCapsResponse(esFieldCaps), 'name');
    const allFieldsUnsorted = Object.keys(fieldsFromFieldCapsByName)
        .filter(name => !name.startsWith('_'))
        .concat(metaFields)
        .reduce(concatIfUniq, [])
        .map(name => lodash_1.defaults({}, fieldsFromFieldCapsByName[name], {
        name,
        type: 'string',
        searchable: false,
        aggregatable: false,
