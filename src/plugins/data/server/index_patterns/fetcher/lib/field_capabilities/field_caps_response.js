...
function readFieldCapsResponse(fieldCapsResponse) {
    ...
    const kibanaFormattedCaps = Object.keys(capsByNameThenType).map(fieldName => {
        ...
        const uniqueKibanaTypes = lodash_1.uniq(types.map(common_1.castEsToKbnFieldTypeName));
        ...
        return {
            name: fieldName,
            type: common_1.castEsToKbnFieldTypeName(esType),
            esTypes: types,
            searchable: isSearchable,
            aggregatable: isAggregatable,
            readFromDocValues: should_read_field_from_doc_values_1.shouldReadFieldFromDocValues(isAggregatable, esType),
        };
    });
