...
const get_search_params_1 = require("./get_search_params");
...
describe('getSearchParams', () => {
    test('includes rest_total_hits_as_int', () => {
        ...
        const searchParams = get_search_params_1.getSearchParams(config);
    ...
    test('includes ignore_throttled according to search:includeFrozen', () => {
        ...
        let searchParams = get_search_params_1.getSearchParams(config);
        expect(searchParams.ignore_throttled).toBe(false);
        config = getConfigStub({ 'search:includeFrozen': false });
        searchParams = get_search_params_1.getSearchParams(config);
    ...
    test('includes timeout according to esShardTimeout if greater than 0', () => {
        const config = getConfigStub();
        let searchParams = get_search_params_1.getSearchParams(config, 0);
        expect(searchParams.timeout).toBe(undefined);
        searchParams = get_search_params_1.getSearchParams(config, 100);
