...
const updateTimeBuckets = (agg, timefilter, customBuckets) => {
...
exports.getDateHistogramBucketAgg = ({ uiSettings, query, getInternalStartServices, }) => new bucket_agg_type_1.BucketAggType({
    ...
    getFormat(agg) {
        ...
        const DateFieldFormat = fieldFormats.getType(common_2.FIELD_FORMAT_IDS.DATE);
        if (!DateFieldFormat) {
            throw new Error('Unable to retrieve Date Field Format');
        }
        return new DateFieldFormat({
            pattern: agg.buckets.getScaledDateFormat(),
        }, (key) => uiSettings.get(key));
    },
    params: [
        {
            name: 'field',
            type: 'field',
            filterFieldTypes: common_2.KBN_FIELD_TYPES.DATE,
            default(agg) {
