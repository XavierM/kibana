...
const getNestedAggDSL = (aggNestedDsl, startFromAggId) => {
    ...
    const nestedAggs = lodash_1.values(aggNestedDsl);
    let aggs;
...
const getAggResultBuckets = (aggConfigs, response, aggWithOtherBucket, key) => {
    ...
    for (const i in keyParts) {
        if (keyParts[i]) {
            const responseAggs = lodash_1.values(responseAgg);
            ...
            for (let aggId = 0; aggId < responseAggs.length; aggId++) {
                ...
                const aggKey = lodash_1.keys(responseAgg)[aggId];
                const aggConfig = lodash_1.find(aggConfigs.aggs, agg => agg.id === aggKey);
                if (aggConfig) {
                    const aggResultBucket = lodash_1.find(aggById.buckets, (bucket, bucketObjKey) => {
                        const bucketKey = aggConfig
                            .getKey(bucket, lodash_1.isNumber(bucketObjKey) ? undefined : bucketObjKey)
                            .toString();
...
const getAggConfigResultMissingBuckets = (responseAggs, aggId) => {
    ...
    lodash_1.each(responseAggs, agg => {
        if (agg.buckets) {
            lodash_1.each(agg.buckets, bucket => {
...
exports.buildOtherBucketAgg = (aggConfigs, aggWithOtherBucket, response) => {
    const bucketAggs = aggConfigs.aggs.filter(agg => agg.type.type === agg_groups_1.AggGroupNames.Buckets);
    ...
    const walkBucketTree = (aggIndex, aggregations, aggId, filters, key) => {
        ...
        if (aggIndex < index) {
            lodash_1.each(agg.buckets, (bucket, bucketObjKey) => {
                const bucketKey = currentAgg.getKey(bucket, lodash_1.isNumber(bucketObjKey) ? undefined : bucketObjKey);
                const filter = lodash_1.cloneDeep(bucket.filters) || currentAgg.createFilter(bucketKey);
                const newFilters = lodash_1.flatten([...filters, filter]);
                walkBucketTree(newAggIndex, bucket, newAgg.id, newFilters, `${key}-${bucketKey.toString()}`);
            });
            return;
        }
        if (!aggWithOtherBucket.params.missingBucket ||
            agg.buckets.some((bucket) => bucket.key === '__missing__')) {
            filters.push(common_1.buildExistsFilter(aggWithOtherBucket.params.field, aggWithOtherBucket.params.field.indexPattern));
        }
        ...
        resultAgg.filters.filters[key] = {
            bool: common_1.buildQueryFromFilters(filters, indexPattern),
        };
    };
...
exports.mergeOtherBucketAggResponse = (aggsConfig, response, otherResponse, otherAgg, requestAgg) => {
    const updatedResponse = lodash_1.cloneDeep(response);
    lodash_1.each(otherResponse.aggregations['other-filter'].buckets, (bucket, key) => {
        if (!bucket.doc_count || key === undefined)
            return;
        ...
        const phraseFilter = common_1.buildPhrasesFilter(otherAgg.params.field, requestFilterTerms, otherAgg.params.field.indexPattern);
        ...
        if (aggResultBuckets.some((aggResultBucket) => aggResultBucket.key === '__missing__')) {
            bucket.filters.push(common_1.buildExistsFilter(otherAgg.params.field, otherAgg.params.field.indexPattern));
        }
        aggResultBuckets.push(bucket);
    });
    return updatedResponse;
};
exports.updateMissingBucket = (response, aggConfigs, agg) => {
    const updatedResponse = lodash_1.cloneDeep(response);
