...
const filterByType = filter_1.propFilter('type');
class FieldParamType extends base_1.BaseParamType {
    constructor(config, { getInternalStartServices }) {
        super(config);
        this.required = true;
        ...
        this.getAvailableFields = (aggConfig) => {
            ...
            const filteredFields = fields.filter((field) => {
                ...
                if ((onlyAggregatable && (!field.aggregatable || common_1.isNestedField(field))) ||
        ...
        this.deserialize = (fieldName, aggConfig) => {
            ...
            if (!field) {
                throw new public_1.SavedObjectNotFound('index-pattern-field', fieldName);
            }
            // @ts-ignore
            const validField = this.getAvailableFields(aggConfig).find((f) => f.name === fieldName);
            if (!validField) {
                getInternalStartServices().notifications.toasts.addDanger(i18n_1.i18n.translate('data.search.aggs.paramTypes.field.invalidSavedFieldParameterErrorMessage', {
