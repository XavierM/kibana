...
class AggConfig {
    constructor(aggConfigs, opts, { fieldFormats }) {
        this.subAggs = [];
    ...
    getAggParams() {
        return [...(lodash_1.default.has(this, 'type.params') ? this.type.params : [])];
    }
    getRequestAggs() {
    ...
    fieldOwnFormatter(contentType, defaultFormat) {
        ...
        if (!format)
            format = this.fieldFormats.getDefaultInstance(common_1.KBN_FIELD_TYPES.STRING);
        return format.getConverterFor(contentType);
    }
    fieldName() {
        const field = this.getField();
    ...
    set type(type) {
        if (this.__typeDecorations) {
            lodash_1.default.forOwn(this.__typeDecorations, function (prop, name) {
                // @ts-ignore
                delete this[name];
            }, this);
        }
