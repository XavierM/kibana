...
describe('AggConfigs', () => {
    ...
    describe('constructor', () => {
        ...
        it('attempts to ensure that all states have an id', () => {
            ...
            const spy = jest.spyOn(agg_config_1.AggConfig, 'ensureIds');
            new agg_configs_1.AggConfigs(indexPattern, configStates, { typesRegistry, fieldFormats });
    ...
    describe('#createAggConfig', () => {
        it('accepts a configState which is provided as an AggConfig object', () => {
            ...
            ac.createAggConfig(new agg_config_1.AggConfig(ac, {
                enabled: true,
    ...
    describe('#getRequestAggs', () => {
        it('performs a stable sort, but moves metrics to the bottom', () => {
            ...
            const aggs = lodash_1.indexBy(ac.aggs, agg => agg.type.name);
    ...
    describe('#toDsl', () => {
        beforeEach(() => {
            indexPattern = stubs_1.stubIndexPattern;
            indexPattern.fields.getByName = name => name;
        });
        it('uses the sorted aggs', () => {
            const configStates = [{ enabled: true, type: 'avg', params: { field: 'bytes' } }];
            const ac = new agg_configs_1.AggConfigs(indexPattern, configStates, { typesRegistry, fieldFormats });
            const spy = jest.spyOn(agg_configs_1.AggConfigs.prototype, 'getRequestAggs');
            ac.toDsl();
            expect(spy).toHaveBeenCalledTimes(1);
            spy.mockRestore();
        });
        it('calls aggConfig#toDsl() on each aggConfig and compiles the nested output', () => {
            ...
            const ac = new agg_configs_1.AggConfigs(indexPattern, configStates, {
                typesRegistry,
                fieldFormats,
            });
        ...
        it('adds the parent aggs of nested metrics at every level if the vis is hierarchical', () => {
            ...
            expect(topLevelDsl.aggs['3'].aggs['1'].avg_bucket).toHaveProperty('buckets_path', '1-bucket>_count');
        });
    });
});
