...
describe('SearchSource', () => {
    ...
    let searchSourceDependencies;
    beforeEach(() => {
        const core = mocks_2.coreMock.createStart();
        const data = mocks_1.dataPluginMock.createStartContract();
    ...
    describe('#onRequestStart()', () => {
        test('should be called when starting a request', async () => {
            const searchSource = new search_source_1.SearchSource({ index: indexPattern }, searchSourceDependencies);
            const fn = jest.fn();
        ...
        test('should not be called on parent searchSource', async () => {
            const parent = new search_source_1.SearchSource({}, searchSourceDependencies);
            const searchSource = new search_source_1.SearchSource({ index: indexPattern }, searchSourceDependencies);
            const fn = jest.fn();
            searchSource.onRequestStart(fn);
        ...
        test('should be called on parent searchSource if callParentStartHandlers is true', async () => {
            const parent = new search_source_1.SearchSource({}, searchSourceDependencies);
            const searchSource = new search_source_1.SearchSource({ index: indexPattern }, searchSourceDependencies).setParent(parent, {
    ...
    describe('#legacy fetch()', () => {
        ...
        test('should call msearch', async () => {
            ...
            const options = {};
            await searchSource.fetch(options);
            expect(legacy_1.fetchSoon).toBeCalledTimes(1);
        });
    });
    ...
    describe('#serialize', () => {
        ...
        test('should omit sort and size', () => {
            ...
            searchSource.setField('highlightAll', true);
            searchSource.setField('from', 123456);
            searchSource.setField('sort', { field: __1.SortDirection.asc });
