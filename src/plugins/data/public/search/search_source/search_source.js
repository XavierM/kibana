...
class SearchSource {
    ...
    fetch$(searchRequest, signal) {
        ...
        const searchParams = fetch_1.getSearchParams(uiSettings, esShardTimeout);
        const params = {
    ...
    mergeProp(data, val, key) {
        ...
        switch (key) {
            ...
            case 'sort':
                const sort = normalize_sort_request_1.normalizeSortRequest(val, this.getField('index'), uiSettings.get('sort:options'));
    ...
    flatten() {
        ...
        if (body._source) {
            ...
            const filter = public_1.fieldWildcardFilter(body._source.excludes, uiSettings.get('metaFields'));
        ...
        if (fields) {
            ...
            lodash_1.set(body, '_source.includes', remainingFields);
        }
        ...
        if (highlightAll && body.query) {
            body.highlight = field_formats_1.getHighlightRequest(body.query, uiSettings.get('doc_table:highlight'));
    ...
    serialize() {
        const references = [];
        const { filter: originalFilters, ...searchSourceFields } = lodash_1.omit(this.getFields(), ['sort', 'size']);
