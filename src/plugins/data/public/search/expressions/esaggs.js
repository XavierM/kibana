...
const handleCourierRequest = async ({ searchSource, aggs, timeRange, timeFields, indexPattern, query, filters, forceFetch, partialRows, metricsAtAllLevels, inspectorAdapters, filterManager, abortSignal, }) => {
    ...
    if (timeRange && allTimeFields.length > 0) {
        timeFilterSearchSource.setField('filter', () => {
            return allTimeFields
                .map(fieldName => query_1.getTime(indexPattern, timeRange, { fieldName }))
    ...
    for (const agg of aggs.aggs) {
        if (lodash_1.has(agg, 'type.postFlightRequest')) {
            resp = await agg.type.postFlightRequest(resp, aggs, agg, requestSearchSource, inspectorAdapters, abortSignal);
        }
    }
    searchSource.finalResponse = resp;
    const parsedTimeRange = timeRange ? query_1.calculateBounds(timeRange) : null;
    const tabifyParams = {
        metricsAtAllLevels,
        partialRows,
    ...
    inspectorAdapters.data.setTabularLoader(() => build_tabular_inspector_data_1.buildTabularInspectorData(searchSource.tabifiedResponse, filterManager), { returnsFormattedValues: true });
        ...
        const table = {
            ...
            columns: response.columns.map((column) => {
                ...
                if (args.includeFormatHints) {
                    cleanedColumn.formatHint = common_1.serializeFieldFormat(column.aggConfig);
                }
