...
async function buildTabularInspectorData(table, queryFilter) {
    ...
    const rows = table.rows.map(row => {
        return table.columns.reduce((prev, cur, colIndex) => {
            ...
            prev[`col-${colIndex}-${cur.aggConfig.id}`] = new public_1.FormattedData(value, fieldFormatter(value));
            return prev;
        }, {});
    });
                ...
                ((value) => {
                    ...
                    if (filter) {
                        ...
                        if (Array.isArray(filter)) {
                            filter.forEach(f => lodash_1.set(f, 'meta.negate', notOther && notMissing));
                        }
                        else {
                            lodash_1.set(filter, 'meta.negate', notOther && notMissing);
                        }
                        queryFilter.addFilters(filter);
                    }
                }),
