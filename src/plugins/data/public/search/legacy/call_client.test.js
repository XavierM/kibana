...
const handle_response_1 = require("../fetch/handle_response");
...
jest.mock('./default_search_strategy', () => {
    return {
        defaultSearchStrategy: {
            search: jest.fn(({ searchRequests }) => {
                return {
                    searching: Promise.resolve(searchRequests.map(req => {
                        return {
...
describe('callClient', () => {
    beforeEach(() => {
        handle_response_1.handleResponse.mockClear();
    });
    test('Passes the additional arguments it is given to the search strategy', () => {
        const searchRequests = [{ _searchStrategyId: 0 }];
        const args = { legacySearchService: {}, config: {}, esShardTimeout: 0 };
        call_client_1.callClient(searchRequests, [], args);
        expect(default_search_strategy_1.defaultSearchStrategy.search).toBeCalled();
        expect(default_search_strategy_1.defaultSearchStrategy.search.mock.calls[0][0]).toEqual({
            searchRequests,
            ...args,
        });
    });
    test('Returns the responses in the original order', async () => {
        ...
        const responses = await Promise.all(call_client_1.callClient(searchRequests, [], {}));
    ...
    test('Calls handleResponse with each request and response', async () => {
        ...
        const responses = call_client_1.callClient(searchRequests, [], {});
        await Promise.all(responses);
        expect(handle_response_1.handleResponse).toBeCalledTimes(2);
        expect(handle_response_1.handleResponse).toBeCalledWith(searchRequests[0], {
            id: searchRequests[0]._searchStrategyId,
        });
        expect(handle_response_1.handleResponse).toBeCalledWith(searchRequests[1], {
    ...
    test('If passed an abortSignal, calls abort on the strategy if the signal is aborted', () => {
        ...
        call_client_1.callClient(searchRequests, requestOptions, {});
