...
function tabifyAggResponse(aggConfigs, esResponse, respOpts) {
    ...
    function collectBucket(aggs, write, bucket, key, aggScale) {
        ...
        if (column) {
            ...
            switch (agg.type.type) {
                case aggs_1.AggGroupNames.Buckets:
                    const aggBucket = lodash_1.get(bucket, agg.id);
                    const tabifyBuckets = new buckets_1.TabifyBuckets(aggBucket, agg.params, respOpts?.timeRange);
                    ...
                    break;
                case aggs_1.AggGroupNames.Metrics:
    ...
    const write = new response_writer_1.TabbedAggResponseWriter(aggConfigs, respOpts || {});
