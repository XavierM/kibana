...
describe('Buckets wrapper', () => {
    const check = (aggResp, count, keys) => {
        test('reads the length', () => {
            const buckets = new buckets_1.TabifyBuckets(aggResp);
    ...
    describe('with object style buckets', () => {
        ...
        test('should accept filters agg queries with strings', () => {
            ...
            const buckets = new buckets_1.TabifyBuckets(aggResp, aggParams);
    ...
    describe('drop_partial option', () => {
        ...
        test('drops partial buckets when enabled', () => {
            ...
            const timeRange = {
                from: moment_1.default(150),
                to: moment_1.default(350),
                timeFields: ['date'],
            };
            const buckets = new buckets_1.TabifyBuckets(aggResp, aggParams, timeRange);
            expect(buckets).toHaveLength(1);
        });
        ...
        test('keeps aligned buckets when enabled', () => {
            ...
            const timeRange = {
                from: moment_1.default(100),
                to: moment_1.default(400),
                timeFields: ['date'],
            };
            const buckets = new buckets_1.TabifyBuckets(aggResp, aggParams, timeRange);
        ...
        test('does drop bucket when multiple time fields specified', () => {
            ...
            const buckets = new buckets_1.TabifyBuckets(aggResp, aggParams, timeRange);
            expect(buckets.buckets.map((b) => b.key)).toEqual([100, 200]);
        });
        test('does not drop bucket when no timeFields have been specified', () => {
            ...
            const buckets = new buckets_1.TabifyBuckets(aggResp, aggParams, timeRange);
            expect(buckets.buckets.map((b) => b.key)).toEqual([0, 100, 200, 300]);
        });
    });
});
