...
const createFilter = async (table, columnIndex, rowIndex) => {
    ...
    const aggConfig = expressions_1.deserializeAggConfig({
        type: column.meta.type,
        aggConfigParams: column.meta.aggConfigParams ? column.meta.aggConfigParams : {},
        indexPattern: await services_1.getIndexPatterns().get(column.meta.indexPatternId),
    });
    let filter = [];
...
/** @public */
exports.createFiltersFromValueClickAction = async ({ data, negate, }) => {
    const filters = [];
    await Promise.all(data
        ...
        if (filter) {
            filter.forEach(f => {
                if (negate) {
                    f = public_1.esFilters.toggleFilterNegated(f);
                }
                filters.push(f);
            });
        }
    }));
    return public_1.esFilters.mapAndFlattenFilters(filters);
};
