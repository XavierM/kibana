...
const create_filters_from_range_select_1 = require("./create_filters_from_range_select");
const mocks_1 = require("../../../public/mocks");
const services_1 = require("../../../public/services");
const test_helpers_1 = require("../../../public/search/aggs/test_helpers");
describe('brushEvent', () => {
    ...
    beforeEach(() => {
        ...
        services_1.setIndexPatterns({
            ...mocks_1.dataPluginMock.createStartContract().indexPatterns,
    ...
    test('should be a function', () => {
        expect(typeof create_filters_from_range_select_1.createFiltersFromRangeSelectAction).toBe('function');
    });
    test('ignores event when data.xAxisField not provided', async () => {
        const filter = await create_filters_from_range_select_1.createFiltersFromRangeSelectAction(baseEvent);
    ...
    describe('handles an event when the x-axis field is a date field', () => {
        ...
        describe('date field is not index pattern timefield', () => {
            ...
            test('creates a new range filter', async () => {
                ...
                if (filter.length) {
                    const rangeFilter = filter[0];
                    expect(rangeFilter.range.anotherTimeField.gte).toBe(moment_1.default(rangeBegin).toISOString());
                    expect(rangeFilter.range.anotherTimeField.lt).toBe(moment_1.default(rangeEnd).toISOString());
                    expect(rangeFilter.range.anotherTimeField).toHaveProperty('format', 'strict_date_optional_time');
                }
            });
        });
    });
