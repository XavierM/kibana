...
const create_filters_from_value_click_1 = require("./create_filters_from_value_click");
const mockField = {
    name: 'bytes',
    indexPattern: {
        id: 'logstash-*',
    ...
    filterable: true,
    format: new public_1.fieldFormats.BytesFormat({}, (() => { })),
};
describe('createFiltersFromValueClick', () => {
    ...
    test('ignores event when value for rows is not provided', async () => {
        ...
        const filters = await create_filters_from_value_click_1.createFiltersFromValueClickAction({ data: dataPoints });
    ...
    test('handles an event when aggregations type is not terms', async () => {
        ...
        if (public_1.esFilters.isRangeFilter(rangeFilter)) {
