...
const generate_filters_1 = require("./generate_filters");
...
describe('Generate filters', () => {
    ...
    it('should create exists filter', () => {
        const filters = generate_filters_1.generateFilters(mockFilterManager, EXISTS_FIELD_NAME, FIELD.name, '', INDEX_NAME);
        expect(filters).toHaveLength(1);
        expect(filters[0].meta.index === INDEX_NAME);
        expect(filters[0].meta.negate).toBeFalsy();
        expect(common_1.isExistsFilter(filters[0])).toBeTruthy();
    });
    ...
    it('should update and re-enable EXISTING exists filter', () => {
        const filter = common_1.buildExistsFilter(FIELD, { id: INDEX_NAME });
        filter.meta.disabled = true;
        filtersArray.push(filter);
        const filters = generate_filters_1.generateFilters(mockFilterManager, '_exists_', FIELD.name, '-', INDEX_NAME);
        expect(filters).toHaveLength(1);
        expect(filters[0].meta.index === INDEX_NAME);
        expect(filters[0].meta.negate).toBeTruthy();
    ...
    it('should create phrase filter', () => {
        const filters = generate_filters_1.generateFilters(mockFilterManager, FIELD, PHRASE_VALUE, '', INDEX_NAME);
        expect(filters).toHaveLength(1);
        expect(filters[0].meta.index === INDEX_NAME);
        expect(filters[0].meta.negate).toBeFalsy();
        expect(common_1.isPhraseFilter(filters[0])).toBeTruthy();
    ...
    it('should create multiple phrase filters', () => {
        ...
        const filters = generate_filters_1.generateFilters(mockFilterManager, FIELD, [PHRASE_VALUE, ANOTHER_PHRASE], '', INDEX_NAME);
        ...
        expect(common_1.isPhraseFilter(filters[0])).toBeTruthy();
        expect(common_1.isPhraseFilter(filters[1])).toBeTruthy();
        expect(filters[0].query.match_phrase).toEqual({
            [FIELD.name]: PHRASE_VALUE,
        });
