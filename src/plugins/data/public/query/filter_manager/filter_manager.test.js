...
const get_stub_filter_1 = require("./test_helpers/get_stub_filter");
const get_filters_array_1 = require("./test_helpers/get_filters_array");
const common_1 = require("../../../common");
const mocks_1 = require("../../../../../core/public/mocks");
const setupMock = mocks_1.coreMock.createSetup();
const uiSettingsMock = (pinnedByDefault) => (key) => {
    switch (key) {
...
describe('filter_manager', () => {
    ...
    let readyFilters;
    beforeEach(() => {
        updateListener = sinon_1.default.stub();
        filterManager = new filter_manager_1.FilterManager(setupMock.uiSettings);
        readyFilters = get_filters_array_1.getFiltersArray();
    });
    afterEach(async () => {
    ...
    describe('observing', () => {
        test('should return observable', () => {
            ...
            expect(updateSubscription).toBeInstanceOf(rxjs_1.Subscription);
            expect(fetchSubscription).toBeInstanceOf(rxjs_1.Subscription);
        });
    });
    describe('get \\ set filters', () => {
        ...
        test('global state should be set', async () => {
            updateSubscription = filterManager.getUpdates$().subscribe(updateListener);
            const f1 = get_stub_filter_1.getFilter(common_1.FilterStateStore.GLOBAL_STATE, false, false, 'age', 34);
            filterManager.setFilters([f1]);
            expect(filterManager.getAppFilters()).toHaveLength(0);
        ...
        test('both states should be set', async () => {
            updateSubscription = filterManager.getUpdates$().subscribe(updateListener);
            const f1 = get_stub_filter_1.getFilter(common_1.FilterStateStore.GLOBAL_STATE, false, false, 'age', 34);
            const f2 = get_stub_filter_1.getFilter(common_1.FilterStateStore.APP_STATE, false, false, 'gender', 'FEMALE');
        ...
        test('changing a disabled filter should fire only update event', async function () {
            ...
            const f1 = get_stub_filter_1.getFilter(common_1.FilterStateStore.GLOBAL_STATE, true, false, 'age', 34);
            filterManager.setFilters([f1]);
            ...
            const f2 = lodash_1.default.cloneDeep(f1);
            f2.meta.negate = true;
            filterManager.setFilters([f2]);
        ...
        test('should merge multiple conflicting app filters', async function () {
            ...
            const appFilter1 = lodash_1.default.cloneDeep(readyFilters[1]);
            appFilter1.meta.negate = true;
            appFilter1.$state = {
                store: common_1.FilterStateStore.APP_STATE,
            };
            const appFilter2 = lodash_1.default.cloneDeep(readyFilters[2]);
            ...
            expect(res.filter(function (filter) {
                return filter.$state && filter.$state.store === common_1.FilterStateStore.GLOBAL_STATE;
            }).length).toBe(3);
        });
        ...
        test('should set global filters and remove any duplicated app filters', async function () {
            ...
            const globalFilter1 = lodash_1.default.cloneDeep(readyFilters[1]);
            const globalFilter2 = lodash_1.default.cloneDeep(readyFilters[2]);
        ...
        test('set filter with no state, and force pin', async () => {
            const f1 = get_stub_filter_1.getFilter(common_1.FilterStateStore.GLOBAL_STATE, false, false, 'age', 38);
            f1.$state = undefined;
    ...
    describe('add filters', () => {
        ...
        test('global state should be accept array and preserve order', async () => {
            const f1 = get_stub_filter_1.getFilter(common_1.FilterStateStore.GLOBAL_STATE, false, false, 'age', 34);
            const f2 = get_stub_filter_1.getFilter(common_1.FilterStateStore.GLOBAL_STATE, false, false, 'gender', 'female');
        ...
        test('add same filter to global and app', async () => {
            updateSubscription = filterManager.getUpdates$().subscribe(updateListener);
            const f1 = get_stub_filter_1.getFilter(common_1.FilterStateStore.GLOBAL_STATE, false, false, 'age', 34);
            const f2 = get_stub_filter_1.getFilter(common_1.FilterStateStore.APP_STATE, false, false, 'age', 34);
            filterManager.addFilters([f1, f2]);
        ...
        test('add filter with no state, and force pin', async () => {
            ...
            if (f1Output.$state) {
                expect(f1Output.$state.store).toBe(common_1.FilterStateStore.GLOBAL_STATE);
            }
        });
        test('add filter with no state, and dont force pin', async () => {
            ...
            expect(f1Output.$state).toBeDefined();
            if (f1Output.$state) {
                expect(f1Output.$state.store).toBe(common_1.FilterStateStore.APP_STATE);
            }
        });
        test('should return app and global filters', async function () {
            const filters = get_filters_array_1.getFiltersArray();
            ...
            expect(res[0].$state && res[0].$state.store).toEqual(common_1.FilterStateStore.GLOBAL_STATE);
            expect(res[0].meta.disabled).toEqual(filters[1].meta.disabled);
            expect(res[0].query).toEqual(filters[1].query);
            expect(res[1].$state && res[1].$state.store).toEqual(common_1.FilterStateStore.APP_STATE);
        ...
        test('should skip appStateStub filters that match globalStateStub filters', async function () {
            filterManager.addFilters(readyFilters, true);
            const appFilter = lodash_1.default.cloneDeep(readyFilters[1]);
            filterManager.addFilters(appFilter, false);
            // global filters should be listed first
            const res = filterManager.getFilters();
            expect(res).toHaveLength(3);
            lodash_1.default.each(res, function (filter) {
                expect(filter.$state && filter.$state.store).toBe(common_1.FilterStateStore.GLOBAL_STATE);
            });
        });
        test('should allow overwriting a positive filter by a negated one', async function () {
            // Add negate: false version of the filter
            const filter = lodash_1.default.cloneDeep(readyFilters[0]);
            filter.meta.negate = false;
            filterManager.addFilters(filter);
            expect(filterManager.getFilters()).toHaveLength(1);
            expect(filterManager.getFilters()[0]).toEqual(filter);
            // Add negate: true version of the same filter
            const negatedFilter = lodash_1.default.cloneDeep(readyFilters[0]);
    ...
    describe('filter reconciliation', function () {
        test('should de-dupe app filters being added', async function () {
            const newFilter = lodash_1.default.cloneDeep(readyFilters[1]);
            filterManager.addFilters(readyFilters, false);
        ...
        test('should mutate global filters on appStateStub filter changes', async function () {
            const idx = 1;
            filterManager.addFilters(readyFilters, true);
            const appFilter = lodash_1.default.cloneDeep(readyFilters[idx]);
            ...
            const res = filterManager.getFilters();
            expect(res).toHaveLength(3);
            lodash_1.default.each(res, function (filter, i) {
        ...
        test('should enable disabled filters - global state', async function () {
            ...
            const disabledFilters = lodash_1.default.map(readyFilters, function (filter) {
                const f = lodash_1.default.cloneDeep(filter);
    ...
    describe('remove filters', () => {
        ...
        test('remove non existing filter should do nothing and not fire events', async () => {
            const f1 = get_stub_filter_1.getFilter(common_1.FilterStateStore.GLOBAL_STATE, false, false, 'age', 34);
            const f2 = get_stub_filter_1.getFilter(common_1.FilterStateStore.APP_STATE, false, false, 'gender', 'FEMALE');
            const f3 = get_stub_filter_1.getFilter(common_1.FilterStateStore.APP_STATE, false, false, 'country', 'US');
            filterManager.setFilters([f1, f2]);
            expect(filterManager.getFilters()).toHaveLength(2);
            updateSubscription = filterManager.getUpdates$().subscribe(updateListener);
        ...
        test('should remove matching filters by comparison', async function () {
            filterManager.addFilters([readyFilters[0], readyFilters[1]], true);
            filterManager.addFilters([readyFilters[2]], false);
            filterManager.removeFilter(lodash_1.default.cloneDeep(readyFilters[0]));
            expect(filterManager.getAppFilters()).toHaveLength(1);
            expect(filterManager.getGlobalFilters()).toHaveLength(1);
            filterManager.removeFilter(lodash_1.default.cloneDeep(readyFilters[2]));
            expect(filterManager.getAppFilters()).toHaveLength(0);
            expect(filterManager.getGlobalFilters()).toHaveLength(1);
        });
        test('should do nothing with a non-matching filter', async function () {
            filterManager.addFilters([readyFilters[0], readyFilters[1]], true);
            filterManager.addFilters([readyFilters[2]], false);
            const missedFilter = lodash_1.default.cloneDeep(readyFilters[0]);
