...
describe('PersistedLog', () => {
    ...
    describe('expected API', () => {
        test('has expected methods', () => {
            const log = new persisted_log_1.PersistedLog(historyName, {}, storage);
    ...
    describe('stack options', () => {
        test('should observe the maxLength option', () => {
            ...
            const log = new persisted_log_1.PersistedLog(historyName, { maxLength: historyLimit }, storage);
            log.add(['new array 1']);
        ...
        test('should observe the filterDuplicates option', () => {
            storage.get.mockReturnValue(payload.slice(0));
            const log = new persisted_log_1.PersistedLog(historyName, { filterDuplicates: true }, storage);
        ...
        test('should truncate the list upon initialization if too long', () => {
            storage.get.mockReturnValue(payload.slice(0));
            const log = new persisted_log_1.PersistedLog(historyName, { maxLength: 1 }, storage);
            const items = log.get();
            expect(items.length).toBe(1);
        });
        test('should allow a maxLength of 0', () => {
            storage.get.mockReturnValue(payload.slice(0));
            const log = new persisted_log_1.PersistedLog(historyName, { maxLength: 0 }, storage);
            const items = log.get();
