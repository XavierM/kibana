...
function areRefreshIntervalsDifferent(rangeA, rangeB) {
    if (lodash_1.default.isObject(rangeA) && lodash_1.default.isObject(rangeB)) {
        if (valueOf(rangeA.value) !== valueOf(rangeB.value) ||
            valueOf(rangeA.pause) !== valueOf(rangeB.pause)) {
            return true;
        }
    }
    else {
        return !lodash_1.default.isEqual(rangeA, rangeB);
    }
    return false;
}
exports.areRefreshIntervalsDifferent = areRefreshIntervalsDifferent;
function areTimeRangesDifferent(rangeA, rangeB) {
    if (rangeA && rangeB && lodash_1.default.isObject(rangeA) && lodash_1.default.isObject(rangeB)) {
        if (valueOf(rangeA.to) !== valueOf(rangeB.to) ||
