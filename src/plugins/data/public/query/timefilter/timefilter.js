...
const parse_querystring_1 = require("./lib/parse_querystring");
...
class Timefilter {
    constructor(config, timeHistory) {
        ...
        this.createFilter = (indexPattern, timeRange) => {
            return get_time_1.getTime(indexPattern, timeRange ? timeRange : this._time, {
        ...
        this.getForceNow = () => {
            const forceNow = parse_querystring_1.parseQueryString().forceNow;
            if (!forceNow) {
                return;
            }
    ...
    calculateBounds(timeRange) {
        return get_time_1.calculateBounds(timeRange, { forceNow: this.getForceNow() });
    }
    getActiveBounds() {
