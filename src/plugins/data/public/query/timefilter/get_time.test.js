...
describe('get_time', () => {
    describe('getTime', () => {
        test('build range filter in iso format', () => {
            const clock = sinon_1.default.useFakeTimers(moment_1.default.utc([2000, 1, 1, 0, 0, 0, 0]).valueOf());
            const filter = get_time_1.getTime({
                id: 'test',
                title: 'test',
                timeFieldName: 'date',
                fields: [
        ...
        test('build range filter for non-primary field', () => {
            ...
            }, { from: 'now-60y', to: 'now' }, { fieldName: 'myCustomDate' });
