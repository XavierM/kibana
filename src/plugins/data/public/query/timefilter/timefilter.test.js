...
const timefilterSetupMock = timefilter_service_mock_1.timefilterServiceMock.createSetupContract();
const timefilterConfig = {
    timeDefaults: { from: 'now-15m', to: 'now' },
    refreshIntervalDefaults: { pause: false, value: 0 },
};
const timefilter = new timefilter_1.Timefilter(timefilterConfig, timefilterSetupMock.history);
...
describe('setTime', () => {
    ...
    test('should return strings and not moment objects', () => {
        const from = moment_1.default().subtract(15, 'minutes');
...
describe('setRefreshInterval', () => {
    ...
    beforeEach(() => {
        update = sinon_1.default.spy();
        fetch = sinon_1.default.spy();
        autoRefreshFetch = sinon_1.default.spy();
...
describe('calculateBounds', () => {
    ...
    let clock;
    beforeEach(() => {
        clock = sinon_1.default.useFakeTimers(clockNowTicks);
    });
    afterEach(() => {
        clock.restore();
        clearNowTimeStub();
    ...
    test('uses clock time by default', () => {
        ...
        expect(result.min && result.min.valueOf()).toEqual(clockNowTicks - fifteenMinutesInMilliseconds);
    ...
    test('uses forceNow string', () => {
        ...
        expect(result.min && result.min.valueOf()).toEqual(forceNowTicks - fifteenMinutesInMilliseconds);
