...
exports.createSavedQueryService = (savedObjectsClient) => {
    const saveQuery = async (attributes, { overwrite = false } = {}) => {
        ...
        const query = {
            query: typeof attributes.query.query === 'string'
    ...
    const findSavedQueries = async (searchText = '', perPage = 50, activePage = 1) => {
        const response = await savedObjectsClient.find({
            type: 'query',
            search: searchText,
    ...
    const getSavedQuery = async (id) => {
        const savedObject = await savedObjectsClient.get('query', id);
