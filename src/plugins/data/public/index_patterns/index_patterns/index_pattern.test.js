...
// @ts-ignore
const stubbed_saved_object_index_pattern_1 = require("../../../../../fixtures/stubbed_saved_object_index_pattern");
...
function create(id, payload) {
    const indexPattern = new index_pattern_1.IndexPattern(id, (cfg) => config.get(cfg), savedObjectsClient, apiClient, patternCache);
    setDocsourcePayload(id, payload);
    return indexPattern.init();
}
function setDocsourcePayload(id, providedPayload) {
    object = lodash_1.defaults(providedPayload || {}, stubbed_saved_object_index_pattern_1.stubbedSavedObjectIndexPattern(id));
}
describe('IndexPattern', () => {
    ...
    beforeEach(() => {
        ...
        services_1.setFieldFormats({
            getDefaultInstance: jest.fn(),
            deserialize: jest.fn(),
        });
        return create(indexPatternId).then((pattern) => {
    ...
    describe('init', () => {
        test('should append the found fields', () => {
            ...
            expect(indexPattern.fields).toHaveLength(logstash_fields_1.default().length);
        });
    });
    describe('fields', () => {
    ...
    describe('getScriptedFields', () => {
        test('should return all scripted fields', () => {
            const scriptedNames = logstash_fields_1.default()
                .filter((item) => item.scripted === true)
                .map((item) => item.name);
            const respNames = lodash_1.pluck(indexPattern.getScriptedFields(), 'name');
    ...
    describe('getNonScriptedFields', () => {
        test('should return all non-scripted fields', () => {
            const notScriptedNames = logstash_fields_1.default()
                .filter((item) => item.scripted === false)
                .map((item) => item.name);
            const respNames = lodash_1.pluck(indexPattern.getNonScriptedFields(), 'name');
    ...
    describe('refresh fields', () => {
        ...
        test('should preserve the scripted fields', async () => {
            ...
            expect(indexPattern.getScriptedFields().map(f => f.name)).toEqual(logstash_fields_1.default()
                .filter((f) => f.scripted)
                .map((f) => f.name));
        });
    });
    describe('add and remove scripted fields', () => {
        test('should append the scripted field', async () => {
            ...
            await indexPattern.addScriptedField(scriptedField.name, scriptedField.script, scriptedField.type, 'lang');
            const scriptedFields = indexPattern.getScriptedFields();
            // expect(saveSpy.callCount).to.equal(1);
            expect(scriptedFields).toHaveLength(oldCount + 1);
            expect(indexPattern.fields.getByName(scriptedField.name).name).toEqual(scriptedField.name);
        });
        test('should remove scripted field, by name', async () => {
            // const saveSpy = sinon.spy(indexPattern, 'save');
            const scriptedFields = indexPattern.getScriptedFields();
            const oldCount = scriptedFields.length;
            const scriptedField = lodash_1.last(scriptedFields);
        ...
        test('should not allow duplicate names', async () => {
            ...
            try {
                ...
                expect(e).toBeInstanceOf(public_1.DuplicateField);
            }
        });
    });
    ...
    test('should handle version conflicts', async () => {
        ...
        const pattern = new index_pattern_1.IndexPattern('foo', (cfg) => config.get(cfg), savedObjectsClient, apiClient, patternCache);
        await pattern.init();
        expect(lodash_1.get(pattern, 'version')).toBe('fooa');
        // Create the same one - we're going to handle concurrency
        const samePattern = new index_pattern_1.IndexPattern('foo', (cfg) => config.get(cfg), savedObjectsClient, apiClient, patternCache);
        await samePattern.init();
        expect(lodash_1.get(samePattern, 'version')).toBe('fooaa');
