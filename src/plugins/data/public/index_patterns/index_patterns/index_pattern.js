...
class IndexPattern {
    constructor(id, getConfig, savedObjectsClient, apiClient, patternCache) {
        this.title = '';
        this.originalBody = {};
        this.shortDotsEnable = false;
        this.mapping = public_1.expandShorthand({
            title: common_1.ES_FIELD_TYPES.TEXT,
            timeFieldName: common_1.ES_FIELD_TYPES.KEYWORD,
            intervalName: common_1.ES_FIELD_TYPES.KEYWORD,
            fields: 'json',
            sourceFilters: 'json',
            fieldFormatMap: {
                type: common_1.ES_FIELD_TYPES.TEXT,
                _serialize: (map = {}) => {
            ...
            type: common_1.ES_FIELD_TYPES.KEYWORD,
            typeMeta: 'json',
        });
        this.id = id;
        ...
        this.fields = new fields_1.FieldList(this, [], this.shortDotsEnable);
        this.fieldsFetcher = _fields_fetcher_1.createFieldsFetcher(this, apiClient, this.getConfig('metaFields'));
        this.flattenHit = flatten_hit_1.flattenHitWrapper(this, this.getConfig('metaFields'));
        this.formatHit = format_hit_1.formatHitProvider(this, services_1.getFieldFormats().getDefaultInstance(common_1.KBN_FIELD_TYPES.STRING));
    ...
    deserializeFieldFormatMap(mapping) {
        const FieldFormat = services_1.getFieldFormats().getType(mapping.id);
        return FieldFormat && new FieldFormat(mapping.params, this.getConfig);
    }
    initFields(input) {
        const newValue = input || this.fields;
        this.fields = new fields_1.FieldList(this, newValue, this.shortDotsEnable);
    }
    ...
    updateFromElasticSearch(response, forceFieldRefresh = false) {
        if (!response.found) {
            throw new public_1.SavedObjectNotFound(type, this.id, '#/management/kibana/index_pattern');
        }
        ...
        lodash_1.default.assign(this, response._source);
    ...
    async init(forceFieldRefresh = false) {
        ...
        const response = {
            ...
            _source: lodash_1.default.cloneDeep(savedObject.attributes),
    ...
    async addScriptedField(name, script, fieldType = 'string', lang) {
        ...
        const names = lodash_1.default.pluck(scriptedFields, 'name');
        if (lodash_1.default.contains(names, name)) {
            throw new public_1.DuplicateField(name);
        }
    ...
    getNonScriptedFields() {
        return lodash_1.default.where(this.fields, { scripted: false });
    }
    getScriptedFields() {
        return lodash_1.default.where(this.fields, { scripted: true });
    }
    isTimeBased() {
    ...
    async create(allowOverride = false) {
        const _create = async (duplicateId) => {
            if (duplicateId) {
                const duplicatePattern = new IndexPattern(duplicateId, this.getConfig, this.savedObjectsClient, this.patternCache, this.fieldsFetcher);
    ...
    async save(saveAttempts = 0) {
        ...
        return this.savedObjectsClient
            ...
            if (lodash_1.default.get(err, 'res.status') === 409 &&
                ...
                const samePattern = new IndexPattern(this.id, this.getConfig, this.savedObjectsClient, this.patternCache, this.fieldsFetcher);
                return samePattern.init().then(() => {
                    ...
                    if (unresolvedCollision) {
                        const message = i18n_1.i18n.translate('data.indexPatterns.unableWriteLabel', {
