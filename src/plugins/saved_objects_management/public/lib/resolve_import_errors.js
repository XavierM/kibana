...
async function callResolveImportErrorsApi(http, file, retries) {
...
function mapImportFailureToRetryObject({ failure, overwriteDecisionCache, replaceReferencesCache, state, }) {
    ...
    const isOverwriteGranted = isOverwriteAllChecked ||
...
async function resolveImportErrors({ http, getConflictResolutions, state, }) {
    ...
    while (importFailures.some(failure => ['conflict', 'missing_references'].includes(failure.error.type))) {
        ...
        if (!isOverwriteAllChecked) {
            const result = await getConflictResolutions(importFailures
