...
function addJsonFieldToIndexPattern(target, sourceString, fieldName, indexName) {
    if (sourceString) {
        try {
            ...
            throw new Error(i18n_1.i18n.translate('savedObjectsManagement.parsingFieldErrorMessage', {
...
async function importIndexPattern(doc, indexPatterns, overwriteAll, openConfirm) {
    // TODO: consolidate this is the code in create_index_pattern_wizard.js
    const emptyPattern = await indexPatterns.make();
    const { title, timeFieldName, fields, fieldFormatMap, sourceFilters, type, typeMeta, } = doc._source;
    ...
    if (!newId) {
        ...
        const isConfirmed = await openConfirm(i18n_1.i18n.translate('savedObjectsManagement.indexPattern.confirmOverwriteLabel', {
            values: { title },
            defaultMessage: "Are you sure you want to overwrite '{title}'?",
        }), {
            title: i18n_1.i18n.translate('savedObjectsManagement.indexPattern.confirmOverwriteTitle', {
                defaultMessage: 'Overwrite {type}?',
                values: { type },
            }),
            confirmButtonText: i18n_1.i18n.translate('savedObjectsManagement.indexPattern.confirmOverwriteButton', {
...
async function resolveIndexPatternConflicts(resolutions, conflictedIndexPatterns, overwriteAll, dependencies) {
    let importCount = 0;
    await awaitEachItemInParallel(conflictedIndexPatterns, async ({ obj, doc }) => {
        const serializedSearchSource = JSON.parse(doc._source.kibanaSavedObjectMeta?.searchSourceJSON || '{}');
        ...
        const filter = (serializedSearchSource.filter || []).map((f) => {
        ...
        const replacedReferences = (doc._references || []).map((reference) => {
        ...
        obj.searchSource = await dependencies.search.searchSource.fromJSON(JSON.stringify(serializedSearchSource), replacedReferences);
...
exports.resolveIndexPatternConflicts = resolveIndexPatternConflicts;
async function saveObjects(objs, overwriteAll) {
    let importCount = 0;
...
async function saveObject(obj, overwriteAll) {
    return await obj.save({ confirmOverwrite: !overwriteAll });
}
exports.saveObject = saveObject;
async function resolveSavedSearches(savedSearches, services, indexPatterns, overwriteAll) {
    let importCount = 0;
...
exports.resolveSavedSearches = resolveSavedSearches;
async function resolveSavedObjects(savedObjects, overwriteAll, services, indexPatterns, confirmModalPromise) {
    ...
    await awaitEachItemInParallel(docTypes.indexPatterns, async (indexPatternDoc) => {
        try {
            const importedIndexPatternId = await importIndexPattern(indexPatternDoc, indexPatterns, overwriteAll, confirmModalPromise);
...
exports.resolveSavedObjects = resolveSavedObjects;
