...
test('can set default state', () => {
    ...
    const stateContainer = create_state_container_1.createStateContainer(defaultState);
...
test('can create state container without transitions', () => {
    const state = { foo: 'bar' };
    const stateContainer = create_state_container_1.createStateContainer(state);
...
test('transitions methods are not bound', () => {
    const stateContainer = create_state_container_1.createStateContainer({ value: -3 }, {
        add: (state) => (increment) => ({
            ...state,
...
test('throws when state is modified inline in subscription', () => {
    const container = create_state_container_1.createStateContainer({ a: 'b' }, { set: () => (newState) => newState });
...
describe('selectors', () => {
    test('can specify no selectors, or can skip them', () => {
        create_state_container_1.createStateContainer({});
        create_state_container_1.createStateContainer({}, {});
        create_state_container_1.createStateContainer({}, {}, {});
    });
    test('selector object is available on .selectors key', () => {
        const container1 = create_state_container_1.createStateContainer({}, {}, {});
        const container2 = create_state_container_1.createStateContainer({}, {}, { foo: () => () => 123 });
        const container3 = create_state_container_1.createStateContainer({}, {}, { bar: () => () => 1, baz: () => () => 1 });
    ...
    test('selector without arguments returns correct state slice', () => {
        const container = create_state_container_1.createStateContainer({ name: 'Oleg' }, {
    ...
    test('selector can accept an argument', () => {
        const container = create_state_container_1.createStateContainer({
            users: {
                1: {
                    name: 'Darth',
                },
    ...
    test('selector can accept multiple arguments', () => {
        const container = create_state_container_1.createStateContainer({
            ...
            getName: (state) => (id, which) => state.users[id][which],
        });
