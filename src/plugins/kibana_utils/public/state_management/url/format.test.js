...
describe('format', () => {
    describe('replaceUrlHashQuery', () => {
        it('should add hash query to url without hash', () => {
            ...
            expect(format_1.replaceUrlHashQuery(url, () => ({ test: 'test' }))).toMatchInlineSnapshot(`"http://localhost:5601/oxf/app/kibana#?test=test"`);
        });
        it('should replace hash query', () => {
            const url = 'http://localhost:5601/oxf/app/kibana#?test=test';
            expect(format_1.replaceUrlHashQuery(url, query => ({
                ...query,
                test1: 'test1',
