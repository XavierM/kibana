...
describe('kbn_url_storage', () => {
    describe('getStateFromUrl & setStateToUrl', () => {
        ...
        it('should set expanded state to url', () => {
            let newUrl = kbn_url_storage_1.setStateToKbnUrl('_s', state1, { useHash: false }, url);
            expect(newUrl).toMatchInlineSnapshot(`"http://localhost:5601/oxf/app/kibana#/management/kibana/index_patterns/id?_s=(testArray:!(1,2,()),testNull:!n,testNumber:0,testObj:(test:'123'),testStr:'123')"`);
            const retrievedState1 = kbn_url_storage_1.getStateFromKbnUrl('_s', newUrl);
            expect(retrievedState1).toEqual(state1);
            newUrl = kbn_url_storage_1.setStateToKbnUrl('_s', state2, { useHash: false }, newUrl);
            expect(newUrl).toMatchInlineSnapshot(`"http://localhost:5601/oxf/app/kibana#/management/kibana/index_patterns/id?_s=(test:'123')"`);
            const retrievedState2 = kbn_url_storage_1.getStateFromKbnUrl('_s', newUrl);
            expect(retrievedState2).toEqual(state2);
        });
        it('should set hashed state to url', () => {
            let newUrl = kbn_url_storage_1.setStateToKbnUrl('_s', state1, { useHash: true }, url);
            expect(newUrl).toMatchInlineSnapshot(`"http://localhost:5601/oxf/app/kibana#/management/kibana/index_patterns/id?_s=h@a897fac"`);
            const retrievedState1 = kbn_url_storage_1.getStateFromKbnUrl('_s', newUrl);
            expect(retrievedState1).toEqual(state1);
            newUrl = kbn_url_storage_1.setStateToKbnUrl('_s', state2, { useHash: true }, newUrl);
            expect(newUrl).toMatchInlineSnapshot(`"http://localhost:5601/oxf/app/kibana#/management/kibana/index_patterns/id?_s=h@40f94d5"`);
            const retrievedState2 = kbn_url_storage_1.getStateFromKbnUrl('_s', newUrl);
            expect(retrievedState2).toEqual(state2);
        });
    });
    describe('urlControls', () => {
        ...
        beforeEach(() => {
            ...
            urlControls = kbn_url_storage_1.createKbnUrlControls(history);
            urlControls.update('/', true);
        });
        const getCurrentUrl = () => history_1.createPath(history.location);
    ...
    describe('urlControls - scoped history integration', () => {
        ...
        beforeEach(() => {
            ...
            history = new public_1.ScopedHistory(parentHistory, '/app/kibana/');
            urlControls = kbn_url_storage_1.createKbnUrlControls(history);
        });
        const getCurrentUrl = () => history.createHref(history.location);
        it('should flush async url updates', async () => {
    ...
    describe('getRelativeToHistoryPath', () => {
        it('should extract path relative to browser history without basename', () => {
            ...
            const url = "http://localhost:5601/oxf/app/kibana#/management/kibana/index_patterns/id?_a=(tab:indexedFields)&_b=(f:test,i:'',l:'')";
            const relativePath = kbn_url_storage_1.getRelativeToHistoryPath(url, history);
            expect(relativePath).toEqual("/oxf/app/kibana#/management/kibana/index_patterns/id?_a=(tab:indexedFields)&_b=(f:test,i:'',l:'')");
        });
        it('should extract path relative to browser history with basename', () => {
            const url = "http://localhost:5601/oxf/app/kibana#/management/kibana/index_patterns/id?_a=(tab:indexedFields)&_b=(f:test,i:'',l:'')";
            const history1 = history_1.createBrowserHistory({ basename: '/oxf/app/' });
            const relativePath1 = kbn_url_storage_1.getRelativeToHistoryPath(url, history1);
            expect(relativePath1).toEqual("/kibana#/management/kibana/index_patterns/id?_a=(tab:indexedFields)&_b=(f:test,i:'',l:'')");
            const history2 = history_1.createBrowserHistory({ basename: '/oxf/app/kibana/' });
            const relativePath2 = kbn_url_storage_1.getRelativeToHistoryPath(url, history2);
            expect(relativePath2).toEqual("#/management/kibana/index_patterns/id?_a=(tab:indexedFields)&_b=(f:test,i:'',l:'')");
        });
        it('should extract path relative to browser history with basename from relative url', () => {
            const history = history_1.createBrowserHistory({ basename: '/oxf/app/' });
            const url = "/oxf/app/kibana#/management/kibana/index_patterns/id?_a=(tab:indexedFields)&_b=(f:test,i:'',l:'')";
            const relativePath = kbn_url_storage_1.getRelativeToHistoryPath(url, history);
            expect(relativePath).toEqual("/kibana#/management/kibana/index_patterns/id?_a=(tab:indexedFields)&_b=(f:test,i:'',l:'')");
        });
        it('should extract path relative to hash history without basename', () => {
            const history = history_1.createHashHistory();
            const url = "http://localhost:5601/oxf/app/kibana#/management/kibana/index_patterns/id?_a=(tab:indexedFields)&_b=(f:test,i:'',l:'')";
            const relativePath = kbn_url_storage_1.getRelativeToHistoryPath(url, history);
            expect(relativePath).toEqual("/management/kibana/index_patterns/id?_a=(tab:indexedFields)&_b=(f:test,i:'',l:'')");
        });
        it('should extract path relative to hash history with basename', () => {
            const history = history_1.createHashHistory({ basename: 'management' });
            const url = "http://localhost:5601/oxf/app/kibana#/management/kibana/index_patterns/id?_a=(tab:indexedFields)&_b=(f:test,i:'',l:'')";
            const relativePath = kbn_url_storage_1.getRelativeToHistoryPath(url, history);
            expect(relativePath).toEqual("/kibana/index_patterns/id?_a=(tab:indexedFields)&_b=(f:test,i:'',l:'')");
        });
