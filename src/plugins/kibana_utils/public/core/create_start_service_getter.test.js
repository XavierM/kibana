...
const create_start_service_getter_1 = require("./create_start_service_getter");
...
describe('createStartServicesGetter', () => {
    test('throws if services are accessed before accessor resolves', async () => {
        ...
        const accessor = async () => await future.promise;
        const start = create_start_service_getter_1.createStartServicesGetter(accessor);
        await new Promise(r => setTimeout(r, 1));
        expect(() => start()).toThrowErrorMatchingInlineSnapshot(`"Trying to access start services before start."`);
    });
