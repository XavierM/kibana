...
function makeStateful(prop, stateContainer) {
    ...
    const persistedState = new public_1.PersistedState();
    ...
    const handlerOnChange = (method) => persistedState[method]('change', updateOnChange);
    handlerOnChange('on');
    const unsubscribePersisted = () => handlerOnChange('off');
    // update the stateful object when the app state changes
    const persistOnChange = function (state) {
        if (state[prop]) {
    ...
    if (appState[prop])
        persistedState.setSilent(appState[prop]);
