...
const create_streaming_batched_function_1 = require("./create_streaming_batched_function");
const public_1 = require("../../../kibana_utils/public");
const rxjs_1 = require("rxjs");
const getPromiseState = (promise) => Promise.race([
    new Promise(resolve => promise.then(() => resolve('resolved'), () => resolve('rejected'))),
    new Promise(resolve => resolve()).then(() => 'pending'),
]);
const isPending = (promise) => getPromiseState(promise).then(state => state === 'pending');
const setup = () => {
    const xhr = {};
    const { promise, resolve, reject } = public_1.defer();
...
describe('createStreamingBatchedFunction()', () => {
    test('returns a function', () => {
        ...
        const fn = create_streaming_batched_function_1.createStreamingBatchedFunction({
            url: '/test',
            fetchStreaming,
        });
    ...
    describe('when timeout is reached', () => {
        ...
        test('does nothing is buffer is empty', async () => {
            const { fetchStreaming } = setup();
            create_streaming_batched_function_1.createStreamingBatchedFunction({
                url: '/test',
                fetchStreaming,
                maxItemAge: 5,
                flushOnMaxItems: 3,
    ...
    describe('when receiving results', () => {
        ...
        test('rejects promise on error response', async () => {
            ...
            const [, error] = await public_1.of(promise);
        ...
        test('resolves successful requests even after rejected ones', async () => {
            ...
            const promise1 = public_1.of(fn({ a: '1' }));
            const promise2 = public_1.of(fn({ a: '2' }));
            const promise3 = public_1.of(fn({ a: '3' }));
            await new Promise(r => setTimeout(r, 6));
            stream.next(JSON.stringify({
                id: 2,
                result: { b: '3' },
