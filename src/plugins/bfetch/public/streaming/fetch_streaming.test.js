...
const fetch_streaming_1 = require("./fetch_streaming");
const xhr_1 = require("../test_helpers/xhr");
const tick = () => new Promise(resolve => setTimeout(resolve, 1));
const setup = () => {
    const { xhr, XMLHttpRequest } = xhr_1.mockXMLHttpRequest();
    window.XMLHttpRequest = XMLHttpRequest;
    return { xhr };
};
test('returns XHR request', () => {
    setup();
    const { xhr } = fetch_streaming_1.fetchStreaming({
...
test('returns stream', () => {
    setup();
    const { stream } = fetch_streaming_1.fetchStreaming({
        url: 'http://example.com',
    });
...
test('promise throws when request errors', async () => {
    ...
    expect(spy.mock.calls[0][0].message).toMatchInlineSnapshot(`"Batch request failed with status 400"`);
});
...
test('sets custom headers', async () => {
    const env = setup();
    fetch_streaming_1.fetchStreaming({
        url: 'http://example.com',
        headers: {
