...
function is61Panel(panel) {
    return semver_1.default.satisfies(panel.version, '6.1.x');
}
function is62Panel(panel) {
    return semver_1.default.satisfies(panel.version, '6.2.x');
}
function is63Panel(panel) {
    return semver_1.default.satisfies(panel.version, '6.3.x');
}
function is640To720Panel(panel) {
    return (semver_1.default.satisfies(panel.version, '>6.3') &&
        semver_1.default.satisfies(panel.version, '<7.3'));
}
...
function migratePre61PanelToLatest(panel, version, useMargins, uiState) {
    if (panel.col === undefined || panel.row === undefined) {
        throw new Error(i18n_1.i18n.translate('dashboard.panel.unableToMigratePanelDataForSixOneZeroErrorMessage', {
    ...
    const panelIndex = panel.panelIndex ? panel.panelIndex.toString() : uuid_1.default.v4();
    return {
        ...rest,
        version,
        panelIndex,
...
function migrate610PanelToLatest(panel, version, useMargins, uiState) {
    ['w', 'x', 'h', 'y'].forEach(key => {
        if (panel.gridData[key] === undefined) {
            throw new Error(i18n_1.i18n.translate('dashboard.panel.unableToMigratePanelDataForSixThreeZeroErrorMessage', {
