 ...
 * under the License.
 */
const migrate_to_730_panels_1 = require("./migrate_to_730_panels");
test('6.0 migrates uiState, sort, scales, and gridData', async () => {
    ...
    const newPanels = migrate_to_730_panels_1.migratePanelsTo730(panels, '8.0.0', true, uiState);
    ...
    expect(newPanel.sort).toBeUndefined();
    expect(newPanel.columns).toBeUndefined();
    expect(newPanel.embeddableConfig.sort).toBe('sort');
    expect(newPanel.embeddableConfig.columns).toEqual(['bye']);
    expect(newPanel.embeddableConfig.vis.defaultColors['0+-+100']).toBe('rgb(0,104,55)');
});
test('6.0 migrates even when uiState does not exist', async () => {
    ...
    const newPanels = migrate_to_730_panels_1.migratePanelsTo730(panels, '8.0.0', true);
    const newPanel = newPanels[0];
...
test('6.0 migrates old panel data in the right order', () => {
    const createOldPanelData = (col, id, row, sizeX, sizeY, panelIndex) => {
    ...
    const newPanels = migrate_to_730_panels_1.migratePanelsTo730(panelData, '8.0.0', false, {});
...
test('6.0 migrations keep id and type properties if they exist', () => {
    ...
    const newPanels = migrate_to_730_panels_1.migratePanelsTo730(panels, '8.0.0', false, {});
    expect(newPanels[0].type).toBe('visualization');
    expect(newPanels[0].id).toBe('1');
});
...
test('6.0 migrates old panel data in the right order with margins', () => {
    ...
    const newPanels = migrate_to_730_panels_1.migratePanelsTo730(panelData, '8.0.0', true, {});
    const foo8Panel = newPanels.find(panel => panel.id === 'foo8');
    expect(foo8Panel).toBeDefined();
    expect(foo8Panel.row).toBe(undefined);
...
it('6.1 migrates when uiState={} and panels have sort / column override', () => {
    ...
    expect(() => migrate_to_730_panels_1.migratePanelsTo730(panels, '8.0.0', true, uiState)).not.toThrow();
});
test('6.2 migrates sort and scales', async () => {
    ...
    expect(newPanel.sort).toBeUndefined();
    expect(newPanel.embeddableConfig.sort).toBe('sort');
    expect(newPanel.embeddableConfig.hi).toBe('bye');
});
test('6.3 migrates sort, does not scale', async () => {
    ...
    expect(newPanel.embeddableConfig.hi).toBe('bye');
    expect(newPanel.embeddableConfig.sort).toBe('sort');
    expect(newPanel.embeddableConfig.columns).toEqual(['hi']);
});
