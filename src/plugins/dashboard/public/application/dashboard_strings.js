...
function getDashboardTitle(title, viewMode, isDirty, isNew) {
    const isEditMode = viewMode === embeddable_plugin_1.ViewMode.EDIT;
    let displayTitle;
    const newDashboardTitle = i18n_1.i18n.translate('dashboard.savedDashboard.newDashboardTitle', {
        defaultMessage: 'New Dashboard',
    });
    const dashboardTitle = isNew ? newDashboardTitle : title;
    if (isEditMode && isDirty) {
        displayTitle = i18n_1.i18n.translate('dashboard.strings.dashboardUnsavedEditTitle', {
            defaultMessage: 'Editing {title} (unsaved)',
            values: { title: dashboardTitle },
        });
    }
    else if (isEditMode) {
        displayTitle = i18n_1.i18n.translate('dashboard.strings.dashboardEditTitle', {
