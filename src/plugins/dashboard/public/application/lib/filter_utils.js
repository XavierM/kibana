...
class FilterUtils {
    ...
    static convertTimeToUTCString(time) {
        if (moment_1.default(time).isValid()) {
    ...
    static cleanFiltersForComparison(filters) {
        return lodash_1.default.map(filters, filter => {
            const f = lodash_1.default.omit(filter, ['$$hashKey', '$state']);
            if (f.meta) {
                ...
                return lodash_1.default.omit(f.meta, ['value']);
            }
            return f;
        });
    }
