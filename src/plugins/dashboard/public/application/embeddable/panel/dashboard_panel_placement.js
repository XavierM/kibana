...
function findTopLeftMostOpenSpace({ width, height, currentPanels, }) {
    ...
    for (let y = 0; y < maxY; y++) {
        grid[y] = new Array(__1.DASHBOARD_GRID_COLUMN_COUNT).fill(0);
    }
    currentPanelsArray.forEach(panel => {
        for (let x = panel.gridData.x; x < panel.gridData.x + panel.gridData.w; x++) {
            for (let y = panel.gridData.y; y < panel.gridData.y + panel.gridData.h; y++) {
                const row = grid[y];
                if (row === undefined) {
                    throw new Error(`Attempted to access a row that doesn't exist at ${y} for panel ${JSON.stringify(panel)}`);
                }
                grid[y][x] = 1;
            }
        }
    ...
    for (let y = 0; y < maxY; y++) {
        for (let x = 0; x < __1.DASHBOARD_GRID_COLUMN_COUNT; x++) {
            if (grid[y][x] === 1) {
                // Space is filled
                continue;
            }
                ...
                for (let h = y; h < Math.min(y + height, maxY); h++) {
                    for (let w = x; w < Math.min(x + width, __1.DASHBOARD_GRID_COLUMN_COUNT); w++) {
...
exports.findTopLeftMostOpenSpace = findTopLeftMostOpenSpace;
function placePanelBeside({ width, height, currentPanels, placeBesideId, }) {
    ...
    if (!panelToPlaceBeside) {
        throw new embeddable_plugin_1.PanelNotFoundError();
    }
    const beside = panelToPlaceBeside.gridData;
    const otherPanels = [];
    _.forOwn(currentPanels, (panel, key) => {
        otherPanels.push(panel.gridData);
    });
    const possiblePlacementDirections = [
        { grid: { x: beside.x + beside.w, y: beside.y, w: width, h: height }, fits: true },
        { grid: { x: beside.x - width, y: beside.y, w: width, h: height }, fits: true },
        { grid: { x: beside.x, y: beside.y + beside.h, w: width, h: height }, fits: true },
    ];
    for (const direction of possiblePlacementDirections) {
        if (direction.grid.x >= 0 &&
            direction.grid.x + direction.grid.w <= __1.DASHBOARD_GRID_COLUMN_COUNT &&
            direction.grid.y >= 0) {
            const intersection = otherPanels.some((currentPanelGrid) => {
                return (direction.grid.x + direction.grid.w > currentPanelGrid.x &&
...
exports.placePanelBeside = placePanelBeside;
