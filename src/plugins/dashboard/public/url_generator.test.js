...
const mock_1 = require("../../kibana_utils/public/storage/hashed_item_store/mock");
const public_2 = require("../../data/public");
const APP_BASE_PATH = 'xyz/app/kibana';
const createMockDashboardLoader = (dashboardToFilters = {}) => {
    return {
            ...
            return {
                searchSource: {
                    getField: (field) => {
                        ...
                        throw new Error(`createMockDashboardLoader > searchSource > getField > ${field} is not mocked`);
                    },
                },
            };
        },
...
describe('dashboard url generator', () => {
    beforeEach(() => {
        // @ts-ignore
        public_1.hashedItemStore.storage = mock_1.mockStorage;
    });
    test('creates a link to a saved dashboard', async () => {
        const generator = url_generator_1.createDirectAccessDashboardLinkGenerator(() => Promise.resolve({
    ...
    test('creates a link with global time range set up', async () => {
        ...
        expect(url).toMatchInlineSnapshot(`"xyz/app/kibana#/dashboard?_a=()&_g=(time:(from:now-15m,mode:relative,to:now))"`);
    });
        ...
        const url = await generator.createUrl({
            ...
            filters: [
                ...
                },
                {
                    ...
                    query: { query: 'hi' },
                    $state: {
                        store: public_2.esFilters.FilterStateStore.GLOBAL_STATE,
                    },
                },
            ],
            query: { query: 'bye', language: 'kuery' },
        });
        expect(url).toMatchInlineSnapshot(`"xyz/app/kibana#/dashboard/123?_a=(filters:!((meta:(alias:!n,disabled:!f,negate:!f),query:(query:hi))),query:(language:kuery,query:bye))&_g=(filters:!(('$state':(store:globalState),meta:(alias:!n,disabled:!f,negate:!f),query:(query:hi))),refreshInterval:(pause:!f,value:300),time:(from:now-15m,mode:relative,to:now))"`);
    });
    ...
    describe('preserving saved filters', () => {
        ...
        test('can enforce empty filters', async () => {
            ...
            expect(url).toMatchInlineSnapshot(`"xyz/app/kibana#/dashboard/dashboard1?_a=(filters:!())&_g=(filters:!())"`);
        });
        ...
        test('can turn off preserving filters', async () => {
            ...
            expect(urlWithPreservedFiltersTurnedOff).not.toEqual(expect.stringContaining('query:savedfilter1'));
            expect(urlWithPreservedFiltersTurnedOff).toEqual(expect.stringContaining('query:appliedfilter'));
        });
    });
});
