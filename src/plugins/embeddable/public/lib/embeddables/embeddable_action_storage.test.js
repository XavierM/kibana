...
const embeddable_action_storage_1 = require("./embeddable_action_storage");
const public_1 = require("../../../../kibana_utils/public");
class TestEmbeddable extends embeddable_1.Embeddable {
    constructor() {
        super({ id: 'test', viewMode: types_1.ViewMode.VIEW }, {});
        this.type = 'test';
    }
    reload() { }
}
describe('EmbeddableActionStorage', () => {
    describe('.create()', () => {
        test('method exists', () => {
            ...
            const storage = new embeddable_action_storage_1.EmbeddableActionStorage(embeddable);
        ...
        test('throws when creating an event with the same ID', async () => {
            ...
            await storage.create(event);
            const [, error] = await public_1.of(storage.create(event));
            expect(error).toBeInstanceOf(Error);
            expect(error.message).toMatchInlineSnapshot(`"[EEXIST]: Event with [eventId = EVENT_ID] already exists on [embeddable.id = test, embeddable.title = undefined]."`);
        });
    });
    describe('.update()', () => {
        test('method exists', () => {
        ...
        test('throws when updating event, but storage is empty', async () => {
            ...
            const [, error] = await public_1.of(storage.update(event));
            expect(error).toBeInstanceOf(Error);
            expect(error.message).toMatchInlineSnapshot(`"[ENOENT]: Event with [eventId = EVENT_ID] could not be updated as it does not exist in [embeddable.id = test, embeddable.title = undefined]."`);
        });
        test('throws when updating event with ID that is not stored', async () => {
            ...
            await storage.create(event1);
            const [, error] = await public_1.of(storage.update(event2));
            expect(error).toBeInstanceOf(Error);
            expect(error.message).toMatchInlineSnapshot(`"[ENOENT]: Event with [eventId = EVENT_ID2] could not be updated as it does not exist in [embeddable.id = test, embeddable.title = undefined]."`);
        });
    });
    describe('.remove()', () => {
        test('method exists', () => {
        ...
        test('throws when removing an event from an empty storage', async () => {
            const embeddable = new TestEmbeddable();
            const storage = new embeddable_action_storage_1.EmbeddableActionStorage(embeddable);
            const [, error] = await public_1.of(storage.remove('EVENT_ID'));
            expect(error).toBeInstanceOf(Error);
            expect(error.message).toMatchInlineSnapshot(`"[ENOENT]: Event with [eventId = EVENT_ID] could not be removed as it does not exist in [embeddable.id = test, embeddable.title = undefined]."`);
        });
        test('throws when removing with ID that does not exist in storage', async () => {
            ...
            await storage.create(event);
            const [, error] = await public_1.of(storage.remove('WRONG_ID'));
            await storage.remove(event.eventId);
            expect(error).toBeInstanceOf(Error);
            expect(error.message).toMatchInlineSnapshot(`"[ENOENT]: Event with [eventId = WRONG_ID] could not be removed as it does not exist in [embeddable.id = test, embeddable.title = undefined]."`);
        });
    });
    describe('.read()', () => {
        test('method exists', () => {
        ...
        test('throws when reading from empty storage', async () => {
            const embeddable = new TestEmbeddable();
            const storage = new embeddable_action_storage_1.EmbeddableActionStorage(embeddable);
            const [, error] = await public_1.of(storage.read('EVENT_ID'));
            expect(error).toBeInstanceOf(Error);
            expect(error.message).toMatchInlineSnapshot(`"[ENOENT]: Event with [eventId = EVENT_ID] could not be found in [embeddable.id = test, embeddable.title = undefined]."`);
        });
        test('throws when reading event with ID not existing in storage', async () => {
            ...
            await storage.create(event);
            const [, error] = await public_1.of(storage.read('WRONG_ID'));
            expect(error).toBeInstanceOf(Error);
            expect(error.message).toMatchInlineSnapshot(`"[ENOENT]: Event with [eventId = WRONG_ID] could not be found in [embeddable.id = test, embeddable.title = undefined]."`);
        });
