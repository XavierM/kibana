...
const apply_filter_action_1 = require("./apply_filter_action");
const helpers_1 = require("../../tests/helpers");
test('has ACTION_APPLY_FILTER type and id', () => {
    const action = apply_filter_action_1.createFilterAction();
...
describe('execute()', () => {
    describe('when no filters are given', () => {
        test('throws an error', async () => {
            const action = apply_filter_action_1.createFilterAction();
            const error = await helpers_1.expectErrorAsync(() => action.execute({
