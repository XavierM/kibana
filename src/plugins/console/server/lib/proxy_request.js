...
exports.proxyRequest = ({ method, headers, agent, uri, timeout, payload, rejectUnauthorized, }) => {
    ...
    const timeoutPromise = new Promise((timeoutResolve, timeoutReject) => {
        setTimeout(() => {
            if (!req.aborted && !req.socket)
                req.abort();
            if (!resolved) {
                timeoutReject(boom_1.default.gatewayTimeout('Client request timeout'));
            }
            else {
                timeoutResolve();
            }
