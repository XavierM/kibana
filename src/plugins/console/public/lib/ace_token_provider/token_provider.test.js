 ...
 * under the License.
 */
require("../../application/models/sense_editor/sense_editor.test.mocks");
...
const sense_editor_1 = require("../../application/models/sense_editor");
describe('Ace (legacy) token provider', () => {
    ...
    beforeEach(() => {
        ...
        senseEditor = sense_editor_1.create(document.querySelector('#ConAppEditor'));
        jquery_1.default(senseEditor.getCoreEditor().getContainer()).show();
        senseEditor.autocomplete._test.removeChangeListener();
        tokenProvider = senseEditor.getCoreEditor().getTokenProvider();
    });
    afterEach(async () => {
        jquery_1.default(senseEditor.getCoreEditor().getContainer()).hide();
        senseEditor.autocomplete._test.addChangeListener();
        await senseEditor.update('', true);
    });
    describe('#getTokens', () => {
        const runTest = ({ input, expectedTokens, done, lineNumber = 1, }) => {
...

{
  "wudup": "!"
    ...
    describe('#getTokenAt', () => {
        const runTest = ({ input, expectedToken, done, position, }) => {
            senseEditor.update(input, true).then(() => {
