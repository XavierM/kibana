...
function formatRequestBodyDoc(data, indent) {
    ...
    for (let i = 0; i < data.length; i++) {
        const curDoc = data[i];
        try {
            let newDoc = jsonToString(JSON.parse(public_1.collapseLiteralStrings(curDoc)), indent);
            if (indent) {
                newDoc = public_1.expandLiteralStrings(newDoc);
            }
...
function extractDeprecationMessages(warnings) {
    ...
    return lodash_1.default.map(splitOnUnquotedCommaSpace(warnings), warning => {
