...
const data_migration_1 = require("./data_migration");
const contexts_1 = require("../../contexts");
exports.useDataInit = () => {
    const [error, setError] = react_1.useState(null);
    const [done, setDone] = react_1.useState(false);
    const [retryToken, setRetryToken] = react_1.useState({});
    const retry = react_1.useCallback(() => {
        setRetryToken({});
        setDone(false);
        setError(null);
    }, []);
    const { services: { objectStorageClient, history }, } = contexts_1.useServicesContext();
    const dispatch = contexts_1.useEditorActionContext();
    react_1.useEffect(() => {
        const load = async () => {
            try {
                await data_migration_1.migrateToTextObjects({ history, objectStorageClient });
