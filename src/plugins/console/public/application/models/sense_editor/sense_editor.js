...
const row_parser_1 = tslib_1.__importDefault(require("../../../lib/row_parser"));
...
const autocomplete_1 = tslib_1.__importDefault(require("../../../lib/autocomplete/autocomplete"));
class SenseEditor {
    constructor(coreEditor) {
        this.coreEditor = coreEditor;
        this.prevRequestStart = (rowOrPos) => {
            let curRow;
            if (rowOrPos == null) {
                curRow = this.coreEditor.getCurrentPosition().lineNumber;
            }
            else if (lodash_1.default.isObject(rowOrPos)) {
                curRow = rowOrPos.lineNumber;
            }
            else {
                curRow = rowOrPos;
            }
            while (curRow > 0 && !this.parser.isStartRequestRow(curRow, this.coreEditor))
                curRow--;
            return {
                lineNumber: curRow,
                column: 1,
        ...
        this.nextRequestStart = (rowOrPos) => {
            let curRow;
            if (rowOrPos == null) {
                curRow = this.coreEditor.getCurrentPosition().lineNumber;
            }
        ...
        this.autoIndent = lodash_1.default.debounce(async () => {
        ...
        this.update = async (data, reTokenizeAll = false) => {
            return this.coreEditor.setValue(data, reTokenizeAll);
        };
        this.replaceRequestRange = (newRequest, requestRange) => {
        ...
        this.getRequestRange = async (lineNumber) => {
            await this.coreEditor.waitForLatestTokens();
        ...
        this.expandRangeToRequestEdges = async (range = this.coreEditor.getSelectionRange()) => {
            await this.coreEditor.waitForLatestTokens();
        ...
        this.getRequestInRange = async (range) => {
            await this.coreEditor.waitForLatestTokens();
            if (!range) {
                return null;
            }
            ...
            const tokenIter = factories_1.createTokenIterator({ editor: this.coreEditor, position: pos });
            ...
            let dataEndPos;
            while (bodyStartLineNumber < range.end.lineNumber ||
        ...
        this.getRequestsInRange = async (range = this.coreEditor.getSelectionRange(), includeNonRequestBlocks = false) => {
            await this.coreEditor.waitForLatestTokens();
            if (!range) {
                return [];
            }
        ...
        this.moveToPreviousRequestEdge = async () => {
            await this.coreEditor.waitForLatestTokens();
        ...
        this.moveToNextRequestEdge = async (moveOnlyIfNotOnEdge) => {
            await this.coreEditor.waitForLatestTokens();
            const pos = this.coreEditor.getCurrentPosition();
        ...
        this.highlightCurrentRequestsAndUpdateActionBar = lodash_1.default.debounce(async () => {
            await this.coreEditor.waitForLatestTokens();
        ...
        this.getRequestsAsCURL = async (elasticsearchBaseUrl, range) => {
            const requests = await this.getRequestsInRange(range, true);
            const result = lodash_1.default.map(requests, req => {
                ...
                if (esData && esData.length) {
                    ...
                    const dataAsString = public_1.collapseLiteralStrings(esData.join('\n'));
        ...
        this.currentReqRange = null;
        this.parser = new row_parser_1.default(this.coreEditor);
        this.autocomplete = new autocomplete_1.default({
            coreEditor,
            parser: this.parser,
        });
