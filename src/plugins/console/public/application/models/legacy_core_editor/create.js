...
const legacy_core_editor_1 = require("./legacy_core_editor");
exports.create = (el) => {
    const actions = document.querySelector('#ConAppEditorActions');
    if (!actions) {
        throw new Error('Could not find ConAppEditorActions element!');
    }
    const aceEditor = brace_1.default.edit(el);
    return new legacy_core_editor_1.LegacyCoreEditor(aceEditor, actions);
};
