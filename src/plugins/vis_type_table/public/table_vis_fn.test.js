...
describe('interpreter/functions#table', () => {
    const fn = utils_1.functionWrapper(table_vis_fn_1.createTableVisFn());
    const context = {
    ...
    it('calls response handler with correct values', async () => {
        ...
        expect(table_vis_response_handler_1.tableVisResponseHandler).toHaveBeenCalledTimes(1);
        expect(table_vis_response_handler_1.tableVisResponseHandler).toHaveBeenCalledWith(context, visConfig.dimensions);
    });
});
