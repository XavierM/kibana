    ...
    describe('authRequired', () => {
        describe('optional', () => {
            it('User has access to a route if auth mechanism not registered', async () => {
                ...
                router.get({ path: '/', validate: false, options: { authRequired: 'optional' } }, (context, req, res) => res.ok({
                    body: {
            ...
            it('Authenticated user has access to a route', async () => {
                const { server: innerServer, createRouter, registerAuth, auth } = await server.setup(setupDeps);
                const router = createRouter('/');
            ...
            it('User with invalid credentials cannot access a route', async () => {
                ...
                router.get({ path: '/', validate: false, options: { authRequired: 'optional' } }, (context, req, res) => res.ok({ body: 'ok' }));
                await server.start();
                await supertest_1.default(innerServer.listener)
                    .get('/')
                    .expect(401);
            ...
            it('does not redirect user and allows access to a resource', async () => {
                const { server: innerServer, createRouter, registerAuth, auth } = await server.setup(setupDeps);
                const router = createRouter('/');
                registerAuth((req, res, toolkit) => toolkit.redirected({
        ...
        describe('true', () => {
            it('User has access to a route if auth  interceptor is not registered', async () => {
                const { server: innerServer, createRouter, auth } = await server.setup(setupDeps);
                const router = createRouter('/');
                router.get({ path: '/', validate: false, options: { authRequired: true } }, (context, req, res) => res.ok({
                    body: {
                        httpAuthIsAuthenticated: auth.isAuthenticated(req),
                        requestIsAuthenticated: req.auth.isAuthenticated,
                    },
            ...
            it('User with no credentials cannot access a route', async () => {
                ...
                const router = createRouter('/');
                registerAuth((req, res, toolkit) => toolkit.notHandled());
                router.get({ path: '/', validate: false, options: { authRequired: true } }, (context, req, res) => res.ok({ body: 'ok' }));
                await server.start();
                await supertest_1.default(innerServer.listener)
                    .get('/')
                    .expect(401);
        ...
        describe('false', () => {
            it('does not try to authenticate a user', async () => {
                ...
                router.get({ path: '/', validate: false, options: { authRequired: false } }, (context, req, res) => res.ok({
                    body: {
                        httpAuthIsAuthenticated: auth.isAuthenticated(req),
                        requestIsAuthenticated: req.auth.isAuthenticated,
                    },
...
describe('Handler', () => {
    ...
    it('returns 500 Server error if handler returns unexpected result', async () => {
        const { server: innerServer, createRouter } = await server.setup(setupDeps);
        const router = createRouter('/');
        router.get({ path: '/', validate: false }, (context, req, res) => 'ok');
        await server.start();
        const result = await supertest_1.default(innerServer.listener)
            .get('/')
            .expect(500);
        ...
        router.get({
            ...
            validate: {
                query: config_schema_1.schema.object({
                    page: config_schema_1.schema.number(),
                }),
            },
...
describe('handleLegacyErrors', () => {
    it('properly convert Boom errors', async () => {
        const { server: innerServer, createRouter } = await server.setup(setupDeps);
        const router = createRouter('/');
        router.get({ path: '/', validate: false }, router.handleLegacyErrors((context, req, res) => {
...
describe('Response factory', () => {
    describe('Success', () => {
        ...
        it('throws if given invalid json object as response payload', async () => {
            ...
            expect(logging_service_mock_1.loggingServiceMock.collect(logger).error).toHaveLength(0);
        });
    ...
    describe('Error', () => {
        ...
        it('Custom error response for Boom server error', async () => {
            ...
            router.get({ path: '/', validate: false }, (context, req, res) => {
                const error = new Error('some message');
                return res.customError({
                    body: boom_1.default.boomify(error),
                    statusCode: 500,
                });
            });
            await server.start();
      ...
      `);
        ...
        it('throws an error if statusCode is not specified', async () => {
            ...
            router.get({ path: '/', validate: false }, (context, req, res) => {
                ...
                return res.custom({ body: error }); // options.statusCode is required
            });
            await server.start();
            const result = await supertest_1.default(innerServer.listener)
                .get('/')
