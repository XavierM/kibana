...
const core_service_test_mocks_1 = require("./core_service.test.mocks");
const kbnTestServer = tslib_1.__importStar(require("../../../../test_utils/kbn_server"));
const cookieOptions = {
    name: 'sid',
...
describe('http service', () => {
    describe('auth', () => {
        ...
        describe('#isAuthenticated()', () => {
            it('returns true if has been authenticated', async () => {
                ...
                router.get({ path: '/is-auth', validate: false }, (context, req, res) => res.ok({ body: { isAuthenticated: auth.isAuthenticated(req) } }));
                await root.start();
            ...
            it('returns false if has not been authenticated', async () => {
                ...
                const router = createRouter('');
                router.get({ path: '/is-auth', validate: false, options: { authRequired: false } }, (context, req, res) => res.ok({ body: { isAuthenticated: auth.isAuthenticated(req) } }));
                await root.start();
            ...
            it('returns true if authenticated on a route with "optional" auth', async () => {
                ...
                registerAuth((req, res, toolkit) => toolkit.authenticated());
                const router = createRouter('');
                router.get({ path: '/is-auth', validate: false, options: { authRequired: 'optional' } }, (context, req, res) => res.ok({ body: { isAuthenticated: auth.isAuthenticated(req) } }));
                await root.start();
                await kbnTestServer.request.get(root, '/is-auth').expect(200, { isAuthenticated: true });
            });
        ...
        describe('#get()', () => {
            it('returns authenticated status and allow associate auth state with request', async () => {
                ...
                const router = createRouter('');
                router.get({ path: '/get-auth', validate: false }, (context, req, res) => res.ok({ body: auth.get(req) }));
                await root.start();
            ...
            it('returns correct unauthenticated status', async () => {
                ...
                const router = createRouter('');
                router.get({ path: '/get-auth', validate: false, options: { authRequired: false } }, (context, req, res) => res.ok({ body: auth.get(req) }));
                await root.start();
                await kbnTestServer.request
                    .get(root, '/get-auth')
    ...
    describe('legacy server', () => {
        describe('#registerAuth()', () => {
            ...
            afterEach(async () => {
                core_service_test_mocks_1.clusterClientMock.mockClear();
                await root.shutdown();
            });
            it('runs auth for legacy routes and proxy request to legacy server route handlers', async () => {
                const { http } = await root.setup();
                const sessionStorageFactory = await http.createCookieSessionStorageFactory(cookieOptions);
    ...
    describe('elasticsearch', () => {
        ...
        it('rewrites authorization header via authHeaders to make a request to Elasticsearch', async () => {
            ...
            const [adminClient, dataClient] = core_service_test_mocks_1.clusterClientMock.mock.calls;
