...
describe('OnPreAuth', () => {
    ...
    it('supports request forwarding to specified url', async () => {
        const { registerOnPreAuth, server: innerServer, createRouter } = await server.setup(setupDeps);
        const router = createRouter('/');
        router.get({ path: '/initial', validate: false }, (context, req, res) => res.ok({ body: 'initial' }));
        router.get({ path: '/redirectUrl', validate: false }, (context, req, res) => res.ok({ body: 'redirected' }));
        let urlBeforeForwarding;
        registerOnPreAuth((req, res, t) => {
            urlBeforeForwarding = router_1.ensureRawRequest(req).raw.req.url;
            return t.rewriteUrl('/redirectUrl');
        });
        let urlAfterForwarding;
        registerOnPreAuth((req, res, t) => {
            // used by legacy platform
            urlAfterForwarding = router_1.ensureRawRequest(req).raw.req.url;
            return t.next();
        });
        await server.start();
        await supertest_1.default(innerServer.listener)
    ...
    it('supports redirection from the interceptor', async () => {
        ...
        registerOnPreAuth((req, res, t) => res.redirected({
            headers: {
    ...
    it('supports rejecting request and adjusting response headers', async () => {
        ...
        registerOnPreAuth((req, res, t) => res.unauthorized({
            headers: {
...
describe('OnPostAuth', () => {
    ...
    it('supports redirection from the interceptor', async () => {
        ...
        const redirectUrl = '/redirectUrl';
        router.get({ path: '/initial', validate: false }, (context, req, res) => res.ok());
        registerOnPostAuth((req, res, t) => res.redirected({
            headers: {
                location: redirectUrl,
            },
        }));
    ...
    it('supports rejecting request and adjusting response headers', async () => {
        ...
        registerOnPostAuth((req, res, t) => res.unauthorized({
            headers: {
                'www-authenticate': 'challenge',
            },
        }));
...
describe('Auth', () => {
    ...
    it('registers auth request interceptor only once', async () => {
        ...
        const doRegister = () => registerAuth(() => null);
        doRegister();
    ...
    it('may grant access to a resource', async () => {
        ...
        const router = createRouter('/');
        router.get({ path: '/', validate: false }, (context, req, res) => res.ok({ body: { content: 'ok' } }));
    ...
    it('enables auth for a route by default if registerAuth has been called', async () => {
        const { registerAuth, server: innerServer, createRouter } = await server.setup(setupDeps);
        const router = createRouter('/');
        router.get({ path: '/', validate: false }, (context, req, res) => res.ok({ body: { authRequired: req.route.options.authRequired } }));
    ...
    test('supports disabling auth for a route explicitly', async () => {
        const { registerAuth, server: innerServer, createRouter } = await server.setup(setupDeps);
        const router = createRouter('/');
        router.get({ path: '/', validate: false, options: { authRequired: false } }, (context, req, res) => res.ok({ body: { authRequired: req.route.options.authRequired } }));
    ...
    test('supports enabling auth for a route explicitly', async () => {
        const { registerAuth, server: innerServer, createRouter } = await server.setup(setupDeps);
        const router = createRouter('/');
        router.get({ path: '/', validate: false, options: { authRequired: true } }, (context, req, res) => res.ok({ body: { authRequired: req.route.options.authRequired } }));
        const authenticate = jest.fn().mockImplementation((req, res, t) => t.authenticated({}));
    ...
    it('allows manipulating cookies via cookie session storage', async () => {
        const { createCookieSessionStorageFactory, registerAuth, server: innerServer, createRouter, } = await server.setup(setupDeps);
        const router = createRouter('/');
        router.get({ path: '/', validate: false }, (context, req, res) => res.ok());
        ...
        const sessionCookie = request_1.default.cookie(cookies[0]);
        if (!sessionCookie) {
    ...
    it('allows manipulating cookies from route handler', async () => {
        ...
        await server.start();
        const responseToSetCookie = await supertest_1.default(innerServer.listener)
            .get('/')
            .expect(200);
        expect(responseToSetCookie.header['set-cookie']).toBeDefined();
        const responseToResetCookie = await supertest_1.default(innerServer.listener)
            .get('/with-cookie')
            .expect(200);
    ...
    it.skip('is the only place with access to the authorization header', async () => {
        const { registerOnPreAuth, registerAuth, registerOnPostAuth, server: innerServer, createRouter, } = await server.setup(setupDeps);
        const router = createRouter('/');
...
describe('OnPreResponse', () => {
    it('supports registering response inceptors', async () => {
        const { registerOnPreResponse, server: innerServer, createRouter } = await server.setup(setupDeps);
        const router = createRouter('/');
        router.get({ path: '/', validate: false }, (context, req, res) => res.ok({ body: 'ok' }));
        const callingOrder = [];
