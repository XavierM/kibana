...
describe('BasePath', () => {
    describe('serverBasePath', () => {
        it('defaults to an empty string', () => {
            const basePath = new base_path_service_1.BasePath();
            expect(basePath.serverBasePath).toBe('');
        });
        it('returns the server base path', () => {
            const basePath = new base_path_service_1.BasePath('/server');
    ...
    describe('#get()', () => {
        ...
        it('returns base path associated with an incoming KibanaRequest', () => {
            const request = http_server_mocks_1.httpServerMock.createRawRequest();
            const basePath = new base_path_service_1.BasePath();
            basePath.set(router_1.KibanaRequest.from(request, undefined), '/baz/');
            expect(basePath.get(router_1.KibanaRequest.from(request, undefined))).toBe('/baz/');
        });
        ...
        it('is based on server base path', () => {
            const request = http_server_mocks_1.httpServerMock.createRawRequest();
            const basePath = new base_path_service_1.BasePath('/foo/bar');
            basePath.set(request, '/baz/');
    ...
    describe('#set()', () => {
        it('#set() cannot be set twice for one request', () => {
            ...
            setPath();
            expect(setPath).toThrowErrorMatchingInlineSnapshot(`"Request basePath was previously set. Setting multiple times is not supported."`);
        });
    });
    describe('#prepend()', () => {
    ...
    describe('#remove()', () => {
        ...
        it('ignores urls with hostnames', () => {
            const basePath = new base_path_service_1.BasePath('/foo/bar');
            expect(basePath.remove('http://localhost:5601/foo/bar/a/b')).toBe('http://localhost:5601/foo/bar/a/b');
        });
