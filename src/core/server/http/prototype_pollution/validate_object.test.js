...
const validate_object_1 = require("./validate_object");
test(`fails on circular references`, () => {
    const foo = {};
    foo.myself = foo;
    expect(() => validate_object_1.validateObject({
        payload: foo,
...
[
    ...
    ['headers', 'payload', 'query', 'params'].forEach(property => {
        ...
        test(`can submit ${JSON.stringify(obj)}`, () => {
            expect(() => validate_object_1.validateObject(obj)).not.toThrowError();
        });
    });
});
...
[
    ...
    test(`can't submit ${JSON.stringify(value)}`, () => {
        expect(() => validate_object_1.validateObject(value)).toThrowErrorMatchingSnapshot();
    });
});
