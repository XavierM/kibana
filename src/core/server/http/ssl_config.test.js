...
const ssl_config_test_mocks_1 = require("./ssl_config.test.mocks");
const ssl_config_1 = require("./ssl_config");
describe('#SslConfig', () => {
    const createConfig = (obj) => new ssl_config_1.SslConfig(ssl_config_1.sslSchema.validate(obj));
    beforeEach(() => {
        ssl_config_test_mocks_1.mockReadFileSync.mockReset();
        ssl_config_test_mocks_1.mockReadFileSync.mockImplementation((path) => `content-of-${path}`);
        ssl_config_test_mocks_1.mockReadPkcs12Keystore.mockReset();
        ssl_config_test_mocks_1.mockReadPkcs12Keystore.mockImplementation((path) => ({
            key: `content-of-${path}.key`,
            cert: `content-of-${path}.cert`,
            ca: [`content-of-${path}.ca`],
        }));
        ssl_config_test_mocks_1.mockReadPkcs12Truststore.mockReset();
        ssl_config_test_mocks_1.mockReadPkcs12Truststore.mockImplementation((path) => [`content-of-${path}`]);
    });
    describe('throws when config is invalid', () => {
        beforeEach(() => {
            const realFs = jest.requireActual('fs');
            ssl_config_test_mocks_1.mockReadFileSync.mockImplementation((path) => realFs.readFileSync(path));
            const utils = jest.requireActual('../utils');
            ssl_config_test_mocks_1.mockReadPkcs12Keystore.mockImplementation((path, password) => utils.readPkcs12Keystore(path, password));
            ssl_config_test_mocks_1.mockReadPkcs12Truststore.mockImplementation((path, password) => utils.readPkcs12Truststore(path, password));
        });
        test('throws if `key` is invalid', () => {
            const obj = { key: '/invalid/key', certificate: '/valid/certificate' };
            expect(() => createConfig(obj)).toThrowErrorMatchingInlineSnapshot(`"ENOENT: no such file or directory, open '/invalid/key'"`);
        });
        test('throws if `certificate` is invalid', () => {
            ssl_config_test_mocks_1.mockReadFileSync.mockImplementationOnce((path) => `content-of-${path}`);
            const obj = { key: '/valid/key', certificate: '/invalid/certificate' };
            expect(() => createConfig(obj)).toThrowErrorMatchingInlineSnapshot(`"ENOENT: no such file or directory, open '/invalid/certificate'"`);
        });
        test('throws if `certificateAuthorities` is invalid', () => {
            const obj = { certificateAuthorities: '/invalid/ca' };
            expect(() => createConfig(obj)).toThrowErrorMatchingInlineSnapshot(`"ENOENT: no such file or directory, open '/invalid/ca'"`);
        });
        test('throws if `keystore.path` is invalid', () => {
            const obj = { keystore: { path: '/invalid/keystore' } };
            expect(() => createConfig(obj)).toThrowErrorMatchingInlineSnapshot(`"ENOENT: no such file or directory, open '/invalid/keystore'"`);
        });
        test('throws if `keystore.path` does not contain a private key', () => {
            ssl_config_test_mocks_1.mockReadPkcs12Keystore.mockImplementation((path, password) => ({
                key: undefined,
                certificate: 'foo',
            }));
            const obj = { keystore: { path: 'some-path' } };
            expect(() => createConfig(obj)).toThrowErrorMatchingInlineSnapshot(`"Did not find private key in keystore at [keystore.path]."`);
        });
        test('throws if `keystore.path` does not contain a certificate', () => {
            ...
            const obj = { keystore: { path: 'some-path' } };
            expect(() => createConfig(obj)).toThrowErrorMatchingInlineSnapshot(`"Did not find certificate in keystore at [keystore.path]."`);
        });
        test('throws if `truststore.path` is invalid', () => {
            const obj = { truststore: { path: '/invalid/truststore' } };
            expect(() => createConfig(obj)).toThrowErrorMatchingInlineSnapshot(`"ENOENT: no such file or directory, open '/invalid/truststore'"`);
        });
    });
    describe('reads files', () => {
        it('reads certificate authorities when `keystore.path` is specified', () => {
            ...
            expect(ssl_config_test_mocks_1.mockReadPkcs12Keystore).toHaveBeenCalledTimes(1);
        ...
        it('reads certificate authorities when `truststore.path` is specified', () => {
            ...
            expect(ssl_config_test_mocks_1.mockReadPkcs12Truststore).toHaveBeenCalledTimes(1);
        ...
        it('reads certificate authorities when `certificateAuthorities` is specified', () => {
            ...
            expect(ssl_config_test_mocks_1.mockReadFileSync).toHaveBeenCalledTimes(1);
            expect(configValue.certificateAuthorities).toEqual(['content-of-some-path']);
            ssl_config_test_mocks_1.mockReadFileSync.mockClear();
            ...
            expect(ssl_config_test_mocks_1.mockReadFileSync).toHaveBeenCalledTimes(2);
...
describe('#sslSchema', () => {
    describe('throws when config is invalid', () => {
        test('throws if both `key` and `keystore.path` are specified', () => {
            ...
            expect(() => ssl_config_1.sslSchema.validate(obj)).toThrowErrorMatchingInlineSnapshot(`"cannot use [key] when [keystore.path] is specified"`);
        });
        test('throws if both `certificate` and `keystore.path` are specified', () => {
            ...
            expect(() => ssl_config_1.sslSchema.validate(obj)).toThrowErrorMatchingInlineSnapshot(`"cannot use [certificate] when [keystore.path] is specified"`);
        });
        test('throws if TLS is enabled but `certificate` is specified and `key` is not', () => {
            ...
            expect(() => ssl_config_1.sslSchema.validate(obj)).toThrowErrorMatchingInlineSnapshot(`"must specify [certificate] and [key] -- or [keystore.path] -- when ssl is enabled"`);
        });
        ...
        test('throws if TLS is not enabled but `clientAuthentication` is `optional`', () => {
            ...
            expect(() => ssl_config_1.sslSchema.validate(obj)).toThrowErrorMatchingInlineSnapshot(`"must enable ssl to use [clientAuthentication]"`);
        });
    ...
    describe('#supportedProtocols', () => {
        test('accepts known protocols`', () => {
            ...
            const singleKnownProtocolConfig = ssl_config_1.sslSchema.validate(singleKnownProtocol);
            expect(singleKnownProtocolConfig.supportedProtocols).toEqual(['TLSv1']);
            const allKnownProtocolsConfig = ssl_config_1.sslSchema.validate(allKnownProtocols);
        ...
        test('rejects unknown protocols`', () => {
            ...
            expect(() => ssl_config_1.sslSchema.validate(singleUnknownProtocol)).toThrowErrorMatchingInlineSnapshot(`
...
`);
            expect(() => ssl_config_1.sslSchema.validate(allKnownWithOneUnknownProtocols))
...
`);
    ...
    describe('#clientAuthentication', () => {
        test('can specify `none` client authentication when ssl is not enabled', () => {
            ...
            const configValue = ssl_config_1.sslSchema.validate(obj);
        ...
        test('should properly interpret `none` client authentication when ssl is enabled', () => {
            const sslConfig = new ssl_config_1.SslConfig(ssl_config_1.sslSchema.validate({
                enabled: true,
                key: 'some-key-path',
