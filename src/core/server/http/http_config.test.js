...
test('can specify max payload as string', () => {
    ...
    const configValue = _1.config.schema.validate(obj);
...
test('accepts only valid uuids for server.uuid', () => {
    const httpSchema = _1.config.schema;
    expect(() => httpSchema.validate({ uuid: uuid_1.default.v4() })).not.toThrow();
    expect(() => httpSchema.validate({ uuid: 'not an uuid' })).toThrowErrorMatchingInlineSnapshot(`"[uuid]: must be a valid uuid"`);
});
...
test('throws if xsrf.whitelist element does not start with a slash', () => {
    ...
    expect(() => httpSchema.validate(obj)).toThrowErrorMatchingInlineSnapshot(`"[xsrf.whitelist.1]: must start with a slash"`);
});
describe('with TLS', () => {
...
test('can specify socket timeouts', () => {
    ...
    const { keepaliveTimeout, socketTimeout } = _1.config.schema.validate(obj);
...
describe('with compression', () => {
    test('accepts valid referrer whitelist', () => {
        const { compression: { referrerWhitelist }, } = _1.config.schema.validate({
            compression: {
