...
const createConfig = (partial) => partial;
const forgeRequest = ({ headers = {}, path = '/', method = 'get', kibanaRouteState, }) => {
    return http_server_mocks_1.httpServerMock.createKibanaRequest({ headers, path, method, kibanaRouteState });
};
describe('xsrf post-auth handler', () => {
    let toolkit;
    let responseFactory;
    beforeEach(() => {
        toolkit = http_server_mocks_1.httpServerMock.createToolkit();
        responseFactory = http_server_mocks_1.httpServerMock.createLifecycleResponseFactory();
    });
    describe('non destructive methods', () => {
        it('accepts requests without version or xsrf header', () => {
            ...
            const handler = lifecycle_handlers_1.createXsrfPostAuthHandler(config);
            const request = forgeRequest({ method: 'get', headers: {} });
            toolkit.next.mockReturnValue('next');
    ...
    describe('destructive methods', () => {
        ...
        it('returns a bad request if called without xsrf or version header', () => {
            ...
            responseFactory.badRequest.mockReturnValue('badRequest');
            const result = handler(request, responseFactory, toolkit);
...
describe('versionCheck post-auth handler', () => {
    ...
    it('forward the request to the next interceptor if header matches', () => {
        const handler = lifecycle_handlers_1.createVersionCheckPostAuthHandler('actual-version');
...
describe('customHeaders pre-response handler', () => {
    ...
    it('adds the kbn-name header to the response', () => {
        ...
        const handler = lifecycle_handlers_1.createCustomHeadersPreResponseHandler(config);
        handler({}, {}, toolkit);
        expect(toolkit.next).toHaveBeenCalledTimes(1);
