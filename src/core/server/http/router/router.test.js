...
describe('Router', () => {
    describe('Options', () => {
        it('throws if validation for a route is not defined explicitly', () => {
            ...
            () => router.get({ path: '/' }, (context, req, res) => res.ok({}))).toThrowErrorMatchingInlineSnapshot(`"The [get] at [/] does not have a 'validate' specified. Use 'false' as the value if you want to bypass validation."`);
        });
        it('throws if validation for a route is declared wrong', () => {
            ...
            {
                path: '/',
                validate: { params: { validate: () => 'error' } },
            }, (context, req, res) => res.ok({}))).toThrowErrorMatchingInlineSnapshot(`"Expected a valid validation logic declared with '@kbn/config-schema' package or a RouteValidationFunction at key: [params]."`);
        });
        it('throws if options.body.output is not a valid value', () => {
            ...
            {
                path: '/',
                options: { body: { output: 'file' } },
                validate: { body: config_schema_1.schema.object({}, { unknowns: 'allow' }) },
            }, (context, req, res) => res.ok({}))).toThrowErrorMatchingInlineSnapshot(`"[options.body.output: 'file'] in route POST / is not valid. Only 'data' or 'stream' are valid."`);
        });
        ...
        it('should NOT default `output: "stream" and parse: false` when the user has specified body options (he cares about it)', () => {
            const router = new router_1.Router('', logger, enhanceWithContext);
            router.post({ path: '/', options: { body: { maxBytes: 1 } }, validate: {} }, (context, req, res) => res.ok({}));
            const [route] = router.getRoutes();
