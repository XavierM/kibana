...
class KibanaRequest {
    ...
    getEvents(request) {
        const finish$ = rxjs_1.merge(rxjs_1.fromEvent(request.raw.req, 'end'), // all data consumed
        rxjs_1.fromEvent(request.raw.req, 'close') // connection was closed
        ).pipe(operators_1.shareReplay(1), operators_1.first());
        return {
            aborted$: rxjs_1.fromEvent(request.raw.req, 'aborted').pipe(operators_1.first(), operators_1.takeUntil(finish$)),
        };
    }
