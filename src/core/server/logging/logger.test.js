...
const context = logging_config_1.LoggingConfig.getLoggerContext(['context', 'parent', 'child']);
let appenderMocks;
let logger;
const factory = {
...
beforeEach(() => {
    ...
    logger = new logger_1.BaseLogger(context, log_level_1.LogLevel.All, appenderMocks, factory);
});
afterEach(() => {
    jest.resetAllMocks();
...
test('logger with `Off` level does not pass any records to appenders.', () => {
    const turnedOffLogger = new logger_1.BaseLogger(context, log_level_1.LogLevel.Off, appenderMocks, factory);
...
test('logger with `All` level passes all records to appenders.', () => {
    const catchAllLogger = new logger_1.BaseLogger(context, log_level_1.LogLevel.All, appenderMocks, factory);
...
test('passes log record to appenders only if log level is supported.', () => {
    const warnLogger = new logger_1.BaseLogger(context, log_level_1.LogLevel.Warn, appenderMocks, factory);
