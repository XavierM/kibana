...
test('`configSchema` creates correct schema for `pattern` layout.', () => {
    const layoutsSchema = layouts_1.Layouts.configSchema;
...
test('`create()` creates correct layout.', () => {
    const patternLayout = layouts_1.Layouts.create({
        highlight: false,
        kind: 'pattern',
        pattern: '[%date][%level][%logger] %message',
    });
    expect(patternLayout).toBeInstanceOf(pattern_layout_1.PatternLayout);
    const jsonLayout = layouts_1.Layouts.create({ kind: 'json' });
    expect(jsonLayout).toBeInstanceOf(json_layout_1.JsonLayout);
});
