...
const mockCreateWriteStream = fs_1.createWriteStream;
const _1 = require(".");
let service;
beforeEach(() => {
...
test('uses default memory buffer logger until config is provided', () => {
    const bufferAppendSpy = jest.spyOn(service.bufferAppender, 'append');
...
test('flushes memory buffer logger and switches to real logger once config is provided', () => {
    ...
    service.upgrade(_1.config.schema.validate({
        appenders: { default: { kind: 'console', layout: { kind: 'json' } } },
        root: { level: 'info' },
    }));
    expect(JSON.parse(mockConsoleLog.mock.calls[0][0])).toMatchSnapshot(dynamicProps, 'buffered messages');
...
test('`stop()` disposes all appenders.', async () => {
    ...
    const bufferDisposeSpy = jest.spyOn(service.bufferAppender, 'dispose');
    const consoleDisposeSpy = jest.spyOn(service.appenders.get('default'), 'dispose');
    await service.stop();
