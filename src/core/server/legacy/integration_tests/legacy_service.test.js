...
describe('legacy service', () => {
    describe('http server', () => {
        ...
        it("handles http request in Legacy platform if New platform doesn't handle it", async () => {
            ...
            router.get({ path: '/new-platform', validate: false }, (context, req, res) => res.ok({ body: 'from-new-platform' }));
            await root.start();
        ...
        it('throws error if Legacy and New platforms register handler for the same route', async () => {
            ...
            const rootUrl = '/route';
            const router = http.createRouter(rootUrl);
            router.get({ path: '', validate: false }, (context, req, res) => res.ok({ body: 'from-new-platform' }));
            await root.start();
            const kbnServer = kbnTestServer.getKbnServer(root);
            expect(() => kbnServer.server.route({
                method: 'GET',
                path: rootUrl,
                handler: () => 'ok from legacy server',
