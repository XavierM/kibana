...
const createLegacyExports = ({ uiAppSpecs = [], navLinkSpecs = [], }) => ({
    uiAppSpecs,
    navLinkSpecs,
...
const createPluginSpecs = (...ids) => ids.map(id => ({
    getId: () => id,
}));
describe('getNavLinks', () => {
    describe('generating from uiAppSpecs', () => {
        it('generates navlinks from legacy app specs', () => {
            const navlinks = get_nav_links_1.getNavLinks(createLegacyExports({
                uiAppSpecs: [
                    {
                        id: 'app-a',
                        title: 'AppA',
            ...
            expect(navlinks[0]).toEqual(expect.objectContaining({
                id: 'app-a',
                title: 'AppA',
                url: '/app/app-a',
            }));
            expect(navlinks[1]).toEqual(expect.objectContaining({
                id: 'app-b',
                title: 'AppB',
                url: '/app/app-b',
            }));
        ...
        it('throws if an app reference a missing plugin', () => {
            expect(() => {
                get_nav_links_1.getNavLinks(createLegacyExports({
                    uiAppSpecs: [
                        {
                            id: 'app-a',
                            title: 'AppA',
