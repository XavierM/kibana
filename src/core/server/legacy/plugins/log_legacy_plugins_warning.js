...
exports.logLegacyThirdPartyPluginDeprecationWarning = ({ specs, log, }) => {
    ...
    if (thirdPartySpecs.length > 0) {
        ...
        log.warn(`Some installed third party plugin(s) [${pluginIds.join(', ')}] are using the legacy plugin format and will no longer work in a future Kibana release. ` +
...
const isThirdPartyPluginSpec = (spec) => {
