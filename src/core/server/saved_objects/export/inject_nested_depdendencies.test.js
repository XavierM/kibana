...
describe('getObjectReferencesToFetch()', () => {
    test('works with no saved objects', () => {
        const map = new Map();
        const result = inject_nested_depdendencies_1.getObjectReferencesToFetch(map);
...
describe('injectNestedDependencies', () => {
    ...
    test(`doesn't fetch when no dependencies are missing`, async () => {
        ...
        const result = await inject_nested_depdendencies_1.fetchNestedDependencies(savedObjects, savedObjectsClient);
        expect(result).toMatchInlineSnapshot(`
      Object {
        "missingRefs": Array [],
        "objects": Array [
