...
const get_sorted_objects_for_export_1 = require("./get_sorted_objects_for_export");
...
describe('getSortedObjectsForExport()', () => {
    ...
    test('exports selected types and sorts them', async () => {
        ...
        const exportStream = await get_sorted_objects_for_export_1.exportSavedObjectsToStream({
            savedObjectsClient,
            exportSizeLimit: 500,
    ...
    test('export selected types throws error when exceeding exportSizeLimit', async () => {
        ...
        await expect(get_sorted_objects_for_export_1.exportSavedObjectsToStream({
            savedObjectsClient,
            exportSizeLimit: 1,
            types: ['index-pattern', 'search'],
    ...
    test('export selected objects throws error when exceeding exportSizeLimit', async () => {
        ...
        await expect(get_sorted_objects_for_export_1.exportSavedObjectsToStream(exportOpts)).rejects.toThrowErrorMatchingInlineSnapshot(`"Can't export more than 1 objects"`);
    });
    test('rejects when neither type nor objects paramaters are passed in', () => {
        ...
        expect(get_sorted_objects_for_export_1.exportSavedObjectsToStream(exportOpts)).rejects.toThrowErrorMatchingInlineSnapshot(`"Either \`type\` or \`objects\` are required."`);
    });
    test('rejects when both objects and search are passed in', () => {
        ...
        expect(get_sorted_objects_for_export_1.exportSavedObjectsToStream(exportOpts)).rejects.toThrowErrorMatchingInlineSnapshot(`"Can't specify both \\"search\\" and \\"objects\\" properties when exporting"`);
    });
});
