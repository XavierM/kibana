...
describe('getNonExistingReferenceAsKeys()', () => {
    ...
    test('returns empty response when no objects exist', async () => {
        const result = await validate_references_1.getNonExistingReferenceAsKeys([], savedObjectsClient);
    ...
    test('removes references that exist within savedObjects', async () => {
        ...
        const result = await validate_references_1.getNonExistingReferenceAsKeys(savedObjects, savedObjectsClient);
        expect(result).toEqual([]);
        expect(savedObjectsClient.bulkGet).toHaveBeenCalledTimes(0);
    });
...
describe('validateReferences()', () => {
    ...
    test('returns empty when no objects are passed in', async () => {
        const result = await validate_references_1.validateReferences([], savedObjectsClient);
...
`);
    ...
    test('returns errors when references are missing', async () => {
        ...
        const result = await validate_references_1.validateReferences(savedObjects, savedObjectsClient);
        expect(result).toMatchInlineSnapshot(`
Object {
  "errors": Array [
    Object {
...
`);
    ...
    test('throws when bulkGet fails', async () => {
        ...
        await expect(validate_references_1.validateReferences(savedObjects, savedObjectsClient)).rejects.toThrowErrorMatchingInlineSnapshot(`"Bad Request"`);
    });
});
