...
describe('collectSavedObjects()', () => {
    test('collects nothing when stream is empty', async () => {
        ...
        const result = await collect_saved_objects_1.collectSavedObjects({ readStream, objectLimit: 10, supportedTypes: [] });
...
`);
    ...
    test('collects objects from stream', async () => {
        ...
        const result = await collect_saved_objects_1.collectSavedObjects({
            readStream,
            objectLimit: 1,
            supportedTypes: ['a'],
        });
...
`);
    ...
    test('throws error when object limit is reached', async () => {
        ...
        await expect(collect_saved_objects_1.collectSavedObjects({
            readStream,
            objectLimit: 1,
            supportedTypes: ['a'],
    ...
    test('unsupported types return as import errors', async () => {
        ...
        const result = await collect_saved_objects_1.collectSavedObjects({ readStream, objectLimit: 2, supportedTypes: ['1'] });
        expect(result).toMatchInlineSnapshot(`
Object {
  "collectedObjects": Array [],
  "errors": Array [
