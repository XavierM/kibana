...
describe('includedFields', () => {
    it('returns undefined if fields are not provided', () => {
        expect(included_fields_1.includedFields()).toBe(undefined);
    });
    it('accepts type string', () => {
        const fields = included_fields_1.includedFields('config', 'foo');
        expect(fields).toHaveLength(8);
        expect(fields).toContain('type');
    });
    it('accepts type as string array', () => {
        const fields = included_fields_1.includedFields(['config', 'secret'], 'foo');
...
`);
    ...
    it('accepts fields as an array', () => {
        const fields = included_fields_1.includedFields('config', ['foo', 'bar']);
        expect(fields).toHaveLength(10);
        expect(fields).toContain('config.foo');
        expect(fields).toContain('config.bar');
    });
    it('accepts type as string array and fields as string array', () => {
        const fields = included_fields_1.includedFields(['config', 'secret'], ['foo', 'bar']);
        expect(fields).toMatchInlineSnapshot(`
Array [
  "config.foo",
  "config.bar",
...
`);
    ...
    it('uses wildcard when type is not provided', () => {
        const fields = included_fields_1.includedFields(undefined, 'foo');
        expect(fields).toHaveLength(8);
