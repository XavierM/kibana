...
const retry_call_cluster_1 = require("../../../elasticsearch/retry_call_cluster");
...
class SavedObjectsRepository {
    ...
    static createRepository(migrator, typeRegistry, indexName, callCluster, extraTypes = [], injectedConstructor = SavedObjectsRepository) {
        ...
        const allTypes = Object.keys(mappings_1.getRootPropertiesObjects(mappings));
        ...
        return new injectedConstructor({
            ...
            serializer,
            allowedTypes,
            callCluster: retry_call_cluster_1.retryCallCluster(callCluster),
        });
    }
    /**
     * Persists an object
    ...
    async bulkCreate(objects, options = {}) {
        ...
        return {
            saved_objects: expectedBulkResults.map(expectedResult => {
                ...
                const { error, _id: responseId, _seq_no: seqNo, _primary_term: primaryTerm, } = Object.values(response)[0];
                const { _source: { type, [type]: attributes, references = [], namespaces }, } = rawMigratedDoc;
     ...
     */
    async find({ search, defaultSearchOperator = 'OR', searchFields, hasReference, page = 1, perPage = 20, sortField, sortOrder, fields, namespace, type, filter, }) {
        if (!type) {
