...
const ALL_TYPE_SUBSETS = ALL_TYPES.reduce((subsets, value) => subsets.concat(subsets.map(set => [...set, value])), [[]])
...
describe('#getQueryParams', () => {
    ...
    describe('kueryNode filter clause (query.bool.filter[...]', () => {
        const expectResult = (result, expected) => {
        ...
        describe('`kueryNode` parameter', () => {
            it('does not include the clause when `kueryNode` is not specified', () => {
                const result = query_params_1.getQueryParams({ mappings, registry, kueryNode: undefined });
                expect(result.query.bool.filter).toHaveLength(1);
            });
            it('includes the specified Kuery clause', () => {
                const test = (kueryNode) => {
                    const result = query_params_1.getQueryParams({ mappings, registry, kueryNode });
                    const expected = server_1.esKuery.toElasticsearchQuery(kueryNode);
    ...
    describe('reference filter clause (query.bool.filter[bool.must])', () => {
        describe('`hasReference` parameter', () => {
            const expectResult = (result, expected) => {
                expect(result.query.bool.filter).toEqual(expect.arrayContaining([{ bool: expect.objectContaining({ must: expected }) }]));
            };
            it('does not include the clause when `hasReference` is not specified', () => {
                const result = query_params_1.getQueryParams({
                    mappings,
                    registry,
    ...
    describe('type filter clauses (query.bool.filter[bool.should])', () => {
        describe('`type` parameter', () => {
            const expectResult = (result, ...types) => {
                expect(result.query.bool.filter).toEqual(expect.arrayContaining([
                    {
            ...
            it('searches for all known types when `type` is not specified', () => {
                const result = query_params_1.getQueryParams({ mappings, registry, type: undefined });
                expectResult(result, ...ALL_TYPES);
            });
            it('searches for specified type/s', () => {
                const test = (typeOrTypes) => {
                    const result = query_params_1.getQueryParams({
                        mappings,
                        registry,
                        type: typeOrTypes,
        ...
        describe('`namespace` parameter', () => {
            ...
            const expectResult = (result, ...typeClauses) => {
                expect(result.query.bool.filter).toEqual(expect.arrayContaining([
            ...
            const test = (namespace) => {
                for (const typeOrTypes of ALL_TYPE_SUBSETS) {
                    const result = query_params_1.getQueryParams({ mappings, registry, type: typeOrTypes, namespace });
                    const types = Array.isArray(typeOrTypes) ? typeOrTypes : [typeOrTypes];
                    expectResult(result, ...types.map(x => createTypeClause(x, namespace)));
                }
                // also test with no specified type/s
                const result = query_params_1.getQueryParams({ mappings, registry, type: undefined, namespace });
    ...
    describe('search clause (query.bool.must.simple_query_string)', () => {
        const search = 'foo*';
        const expectResult = (result, sqsClause) => {
        ...
        describe('`searchFields` parameter', () => {
            const getExpectedFields = (searchFields, typeOrTypes) => {
                const types = Array.isArray(typeOrTypes) ? typeOrTypes : [typeOrTypes];
                return searchFields.map(x => types.map(y => `${y}.${x}`)).flat();
            };
            const test = (searchFields) => {
                for (const typeOrTypes of ALL_TYPE_SUBSETS) {
                    const result = query_params_1.getQueryParams({
                        mappings,
                        registry,
