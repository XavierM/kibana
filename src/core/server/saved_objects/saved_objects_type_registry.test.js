...
describe('SavedObjectTypeRegistry', () => {
    let registry;
    beforeEach(() => {
        registry = new saved_objects_type_registry_1.SavedObjectTypeRegistry();
    });
    it('allows to register types', () => {
        ...
        expect(registry
            .getAllTypes()
            .map(type => type.name)
            .sort()).toEqual(['typeA', 'typeB', 'typeC']);
    });
    ...
    describe('#getType', () => {
        ...
        it('forbids to mutate the registered types', () => {
            registry.registerType(createType({
                name: 'typeA',
                mappings: {
                    properties: {
                        someField: { type: 'text' },
    ...
    describe('#isNamespaceAgnostic', () => {
        const expectResult = (expected, schemaDefinition) => {
            registry = new saved_objects_type_registry_1.SavedObjectTypeRegistry();
    ...
    describe('#isImportableAndExportable', () => {
        it('returns correct value for the type', () => {
            registry.registerType(createType({ name: 'typeA', management: { importableAndExportable: true } }));
            registry.registerType(createType({ name: 'typeB', management: { importableAndExportable: false } }));
