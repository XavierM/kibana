...
let typeRegistry = saved_objects_type_registry_mock_1.typeRegistryMock.create();
...
const namespaceAgnosticSerializer = new serializer_1.SavedObjectsSerializer(typeRegistry);
...
typeRegistry.isMultiNamespace.mockReturnValue(false);
const singleNamespaceSerializer = new serializer_1.SavedObjectsSerializer(typeRegistry);
typeRegistry = saved_objects_type_registry_mock_1.typeRegistryMock.create();
typeRegistry.isNamespaceAgnostic.mockReturnValue(false);
typeRegistry.isSingleNamespace.mockReturnValue(false);
typeRegistry.isMultiNamespace.mockReturnValue(true);
const multiNamespaceSerializer = new serializer_1.SavedObjectsSerializer(typeRegistry);
...
const createSampleDoc = (raw, template = sampleTemplate) => lodash_1.default.defaultsDeep(raw, template);
describe('#rawToSavedObject', () => {
    ...
    test('it converts the id and type properties, and retains migrationVersion', () => {
        ...
        const expected = {
            id: 'world',
            type: 'hello',
            version: version_1.encodeVersion(3, 1),
            attributes: {
                a: 'b',
                c: 'd',
            },
    ...
    test(`if specified it encodes _seq_no and _primary_term to version`, () => {
        ...
        expect(actual).toHaveProperty('version', version_1.encodeVersion(4, 1));
    });
    test(`if only _seq_no is specified it throws`, () => {
        expect(() => singleNamespaceSerializer.rawToSavedObject({
            _id: 'foo:bar',
            _seq_no: 4,
            _source: {
                type: 'foo',
    ...
    test('it is complimentary with savedObjectToRaw', () => {
        ...
        expect(singleNamespaceSerializer.savedObjectToRaw(singleNamespaceSerializer.rawToSavedObject(lodash_1.default.cloneDeep(raw)))).toEqual(raw);
    });
...
describe('#savedObjectToRaw', () => {
    ...
    test('it decodes the version property to _seq_no and _primary_term', () => {
        const actual = singleNamespaceSerializer.savedObjectToRaw({
            type: '',
            attributes: {},
            version: version_1.encodeVersion(1, 2),
        });
    ...
    test(`if version invalid it throws`, () => {
        expect(() => singleNamespaceSerializer.savedObjectToRaw({
            type: '',
            attributes: {},
            version: 'foo',
...
describe('#isRawSavedObject', () => {
    describe('single-namespace type without a namespace', () => {
        test('is true if the id is prefixed and the type matches', () => {
            expect(singleNamespaceSerializer.isRawSavedObject({
                _id: 'hello:world',
                _source: {
                    type: 'hello',
                    hello: {},
    ...
    describe('namespace-agnostic type with a namespace', () => {
        test('is true if the id is prefixed with type and the type matches', () => {
            expect(namespaceAgnosticSerializer.isRawSavedObject({
                _id: 'hello:world',
                _source: {
                    type: 'hello',
                    hello: {},
