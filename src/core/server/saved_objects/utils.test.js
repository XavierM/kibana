...
describe('convertLegacyTypes', () => {
    let legacyConfig;
    beforeEach(() => {
        legacyConfig = legacy_service_mock_1.legacyServiceMock.createLegacyConfig();
    });
    it('converts the legacy mappings using default values if no schemas are specified', () => {
        ...
        const converted = utils_1.convertLegacyTypes(uiExports, legacyConfig);
    ...
    it('invokes indexPattern to retrieve the index when it is a function', () => {
        const indexPatternAccessor = jest.fn(config => {
            config.get('foo.bar');
            return 'myIndex';
        });
        const uiExports = {
...
describe('convertTypesToLegacySchema', () => {
    it('converts types to the legacy schema format', () => {
        ...
        expect(utils_1.convertTypesToLegacySchema(types)).toEqual({
            typeA: {
                hidden: false,
                isNamespaceAgnostic: true,
