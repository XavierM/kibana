...
describe('GET /api/saved_objects/_find', () => {
    ...
    it('returns with status 400 when type is missing', async () => {
        const result = await supertest_1.default(httpSetup.server.listener)
            ...
            .expect(400);
        expect(result.body.message).toContain('[request query.type]: expected at least one defined value');
    });
    ...
    it('accepts the query parameter has_reference', async () => {
        const references = querystring_1.default.escape(JSON.stringify({
            id: '1',
            type: 'reference',
        }));
        await supertest_1.default(httpSetup.server.listener)
