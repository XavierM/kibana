...
class SavedObjectsService {
    ...
    async start({ pluginsInitialized = true }, migrationsRetryDelay) {
        ...
        const createRepository = (callCluster, extraTypes = []) => {
            return repository_1.SavedObjectsRepository.createRepository(migrator, this.typeRegistry, kibanaConfig.index, callCluster, extraTypes);
        };
    ...
    createMigrator(kibanaConfig, savedObjectsConfig, migrationsRetryDelay) {
        ...
        return new migrations_1.KibanaMigrator({
            ...
            kibanaConfig,
            callCluster: retry_call_cluster_1.migrationsRetryCallCluster(adminClient.callAsInternalUser, this.logger, migrationsRetryDelay),
        });
    }
}
