...
const build_index_map_1 = require("./build_index_map");
...
test('mappings without index pattern goes to default index', () => {
    const result = build_index_map_1.createIndexMap({
...
test('throws when two scripts are defined for an index pattern', () => {
    ...
    expect(() => build_index_map_1.createIndexMap({
        kibanaIndexName: defaultIndex,
        registry,
        indexMap,
    })).toThrowErrorMatchingInlineSnapshot(`"convertToAliasScript has been defined more than once for index pattern \\".kibana\\""`);
});
