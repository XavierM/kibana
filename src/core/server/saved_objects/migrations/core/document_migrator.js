...
class DocumentMigrator {
     ...
     */
    constructor({ typeRegistry, kibanaVersion, log, validateDoc }) {
        /**
        ...
        this.migrate = (doc) => {
            ...
            const clonedDoc = lodash_clonedeep_1.default(doc);
...
function wrapWithTry(version, type, migrationFn, log) {
    return function tryTransformDoc(doc) {
        try {
            const context = { log: new migration_logger_1.MigrationLogger(log) };
