...
describe('coordinateMigration', () => {
    ...
    test('waits for isMigrated, if there is an index conflict', async () => {
        ...
        await migration_coordinator_1.coordinateMigration({
            log,
            runMigration,
            pollInterval,
            isMigrated,
        ...
        const warnings = log.warning.mock.calls.filter((msg) => /deleting index \.foo/.test(msg));
    ...
    test('does not swallow exceptions', async () => {
        ...
        const isMigrated = jest.fn(() => Promise.resolve(true));
        await expect(migration_coordinator_1.coordinateMigration({
            log,
            runMigration,
            pollInterval,
            isMigrated,
