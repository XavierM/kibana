...
describe('ElasticIndex', () => {
    describe('fetchInfo', () => {
        test('it handles 404', async () => {
            const callCluster = jest
                .fn()
                .mockImplementation(async (path, { ignore, index }) => {
        ...
        test('fails if the index doc type is unsupported', async () => {
            const callCluster = jest.fn(async (path, { index }) => {
                return {
                    [index]: {
                        aliases: { foo: index },
            ...
            await expect(Index.fetchInfo(callCluster, '.baz')).rejects.toThrow(/cannot be automatically migrated/);
        });
        test('fails if there are multiple root types', async () => {
            const callCluster = jest.fn().mockImplementation(async (path, { index }) => {
                return {
                    [index]: {
                        aliases: { foo: index },
                        mappings: {
    ...
    describe('createIndex', () => {
        test('calls indices.create', async () => {
            const callCluster = jest.fn(async (path, { body, index }) => {
            ...
            await Index.createIndex(callCluster, '.abcd', { foo: 'bar' });
    ...
    describe('claimAlias', () => {
        function assertCalled(callCluster) {
        ...
        test('handles unaliased indices', async () => {
            const callCluster = jest.fn(async (path, arg) => {
                switch (path) {
    ...
    describe('convertToAlias', () => {
        test('it creates the destination index, then reindexes to it', async () => {
            ...
            await Index.convertToAlias(callCluster, info, '.muchacha', 10, `ctx._id = ctx._source.type + ':' + ctx._id`);
            expect(callCluster.mock.calls.map(([path]) => path)).toEqual([
                'indices.create',
                'reindex',
                'tasks.get',
        ...
        test('throws error if re-index task fails', async () => {
            ...
            await expect(Index.convertToAlias(callCluster, info, '.muchacha', 10)).rejects.toThrow(/Re-index failed \[search_phase_execution_exception\] all shards failed/);
            expect(callCluster.mock.calls.map(([path]) => path)).toEqual([
                'indices.create',
                'reindex',
                'tasks.get',
    ...
    describe('write', () => {
        ...
        test('fails if any document fails', async () => {
            const index = '.myalias';
            const callCluster = jest.fn(() => Promise.resolve({
    ...
    describe('reader', () => {
        test('returns docs in batches', async () => {
            ...
            callCluster
                ...
                hits: { hits: lodash_1.default.cloneDeep(batch1) },
            })
                .mockResolvedValueOnce({
                _scroll_id: 'y',
                _shards: { success: 1, total: 1 },
                hits: { hits: lodash_1.default.cloneDeep(batch2) },
            })
                .mockResolvedValueOnce({
                _scroll_id: 'z',
                _shards: { success: 1, total: 1 },
        ...
        test('returns all root-level properties', async () => {
            ...
            callCluster
                ...
                _scroll_id: 'x',
                _shards: { success: 1, total: 1 },
                hits: { hits: lodash_1.default.cloneDeep(batch) },
            })
                .mockResolvedValue({
                _scroll_id: 'z',
                _shards: { success: 1, total: 1 },
        ...
        test('handles shards not being returned', async () => {
            ...
            callCluster
                .mockResolvedValueOnce({ _scroll_id: 'x', hits: { hits: lodash_1.default.cloneDeep(batch) } })
    ...
    describe('migrationsUpToDate', () => {
        ...
        async function testMigrationsUpToDate({ index = '.myindex', mappings, count, migrations, }) {
            const callCluster = jest.fn(async (path) => {
                if (path === 'indices.get') {
                    return {
                        [index]: { mappings },
                    };
        ...
        test('counts docs that are out of date', async () => {
            ...
            function shouldClause(type, version) {
                return {
                    bool: {
                        must: [
                            { exists: { field: type } },
