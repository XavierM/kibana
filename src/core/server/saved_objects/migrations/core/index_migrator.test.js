...
const index_migrator_1 = require("./index_migrator");
...
describe('IndexMigrator', () => {
    ...
    beforeEach(() => {
        testOpts = {
            ...
            callCluster: jest.fn(),
            index: '.kibana',
            log: logging_service_mock_1.loggingServiceMock.create().get(),
            mappingProperties: {},
            pollInterval: 1,
            scrollDuration: '1m',
            documentMigrator: {
                migrationVersion: {},
                migrate: lodash_1.default.identity,
            },
            serializer: new serialization_1.SavedObjectsSerializer(new saved_objects_type_registry_1.SavedObjectTypeRegistry()),
        };
    });
    test('creates the index if it does not exist', async () => {
        ...
        await new index_migrator_1.IndexMigrator(testOpts).migrate();
    ...
    test('returns stats about the migration', async () => {
        const { callCluster } = testOpts;
        withIndex(callCluster, { index: { status: 404 }, alias: { status: 404 } });
        const result = await new index_migrator_1.IndexMigrator(testOpts).migrate();
    ...
    test('fails if there are multiple root doc types', async () => {
        ...
        await expect(new index_migrator_1.IndexMigrator(testOpts).migrate()).rejects.toThrow(/use the X-Pack upgrade assistant/);
    });
    ...
    test('transforms all docs from the original index', async () => {
        ...
        const bulkCalls = callCluster.mock.calls.filter(([action]) => action === 'bulk');
...
function withIndex(callCluster, opts = {}) {
    const defaultIndex = {
        '.kibana_1': {
            aliases: { '.kibana': {} },
            mappings: {
    ...
    const { docs = [] } = opts;
    const searchResult = (i) => Promise.resolve({
        _scroll_id: i,
        _shards: {
            successful: 1,
            total: 1,
