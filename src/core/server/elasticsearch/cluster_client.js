...
const callAPI = async (client, endpoint, clientParams = {}, options = { wrap401Errors: true }) => {
    ...
    try {
        ...
        throw errors_1.ElasticsearchErrorHelpers.decorateNotAuthorizedError(err);
    }
};
/**
...
class ClusterClient {
    ...
    asScoped(request) {
        ...
        return new scoped_cluster_client_1.ScopedClusterClient(this.callAsInternalUser, this.callAsCurrentUser, router_1.filterHeaders(this.getHeaders(request), this.config.requestHeadersWhitelist));
    }
    assertIsNotClosed() {
        if (this.isClosed) {
