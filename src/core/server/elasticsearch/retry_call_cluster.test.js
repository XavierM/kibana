...
const retry_call_cluster_1 = require("./retry_call_cluster");
...
describe('retryCallCluster', () => {
    it('retries ES API calls that rejects with NoConnections', () => {
        ...
        const retried = retry_call_cluster_1.retryCallCluster(callEsApi);
...
describe('migrationsRetryCallCluster', () => {
    ...
    const mockLogger = logging_service_mock_1.loggingServiceMock.create();
    beforeEach(() => {
        logging_service_mock_1.loggingServiceMock.clear(mockLogger);
    });
    errors.forEach(errorName => {
        it('retries ES API calls that rejects with ' + errorName, () => {
            ...
            const callEsApi = jest.fn();
            let i = 0;
            const ErrorConstructor = legacyElasticsearch.errors[errorName];
            callEsApi.mockImplementation(() => {
                return i++ <= 2 ? Promise.reject(new ErrorConstructor()) : Promise.resolve('success');
            });
            const retried = retry_call_cluster_1.migrationsRetryCallCluster(callEsApi, mockLogger.get('mock log'), 1);
            return expect(retried('endpoint')).resolves.toMatchInlineSnapshot(`"success"`);
        });
    });
    ...
    it('logs only once for each unique error message', async () => {
        ...
        expect(logging_service_mock_1.loggingServiceMock.collect(mockLogger).warn).toMatchInlineSnapshot(`
      Array [
        Array [
