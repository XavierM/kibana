...
let internalAPICaller;
let scopedAPICaller;
let clusterClient;
beforeEach(() => {
    internalAPICaller = jest.fn();
    scopedAPICaller = jest.fn();
    clusterClient = new scoped_cluster_client_1.ScopedClusterClient(internalAPICaller, scopedAPICaller, { one: '1' });
});
...
describe('#callAsInternalUser', () => {
    test('properly forwards arguments to the API caller and results back from it', async () => {
        ...
        await expect(clusterClient.callAsInternalUser('security.authenticate', { some: 'some' })).resolves.toBe(mockResponse);
        expect(internalAPICaller).toHaveBeenCalledTimes(1);
        expect(internalAPICaller).toHaveBeenCalledWith('security.authenticate', { some: 'some' }, undefined);
        internalAPICaller.mockClear();
        await expect(clusterClient.callAsInternalUser('ping', undefined, { wrap401Errors: true })).resolves.toBe(mockResponse);
        expect(internalAPICaller).toHaveBeenCalledTimes(1);
        expect(internalAPICaller).toHaveBeenCalledWith('ping', {}, { wrap401Errors: true });
        internalAPICaller.mockClear();
        await expect(clusterClient.callAsInternalUser('security.authenticate', { some: 'some' }, { wrap401Errors: true })).resolves.toBe(mockResponse);
        expect(internalAPICaller).toHaveBeenCalledTimes(1);
        expect(internalAPICaller).toHaveBeenCalledWith('security.authenticate', { some: 'some' }, { wrap401Errors: true });
...
describe('#callAsCurrentUser', () => {
    test('properly forwards arguments to the API caller and results back from it', async () => {
        ...
        await expect(clusterClient.callAsCurrentUser('security.authenticate', { some: 'some' })).resolves.toBe(mockResponse);
        expect(scopedAPICaller).toHaveBeenCalledTimes(1);
        expect(scopedAPICaller).toHaveBeenCalledWith('security.authenticate', { some: 'some', headers: { one: '1' } }, undefined);
        scopedAPICaller.mockClear();
        await expect(clusterClient.callAsCurrentUser('security.authenticate', { some: 'some' })).resolves.toBe(mockResponse);
        expect(scopedAPICaller).toHaveBeenCalledTimes(1);
        expect(scopedAPICaller).toHaveBeenCalledWith('security.authenticate', { some: 'some', headers: { one: '1' } }, undefined);
        scopedAPICaller.mockClear();
        await expect(clusterClient.callAsCurrentUser('ping', undefined, { wrap401Errors: true })).resolves.toBe(mockResponse);
        expect(scopedAPICaller).toHaveBeenCalledTimes(1);
        expect(scopedAPICaller).toHaveBeenCalledWith('ping', { headers: { one: '1' } }, { wrap401Errors: true });
        scopedAPICaller.mockClear();
        await expect(clusterClient.callAsCurrentUser('security.authenticate', { some: 'some' }, { wrap401Errors: true })).resolves.toBe(mockResponse);
        expect(scopedAPICaller).toHaveBeenCalledTimes(1);
        expect(scopedAPICaller).toHaveBeenCalledWith('security.authenticate', { some: 'some', headers: { one: '1' } }, { wrap401Errors: true });
        expect(internalAPICaller).not.toHaveBeenCalled();
    });
    test('callAsCurrentUser allows passing additional headers', async () => {
        const mockResponse = { data: 'response' };
        scopedAPICaller.mockResolvedValue(mockResponse);
        await expect(clusterClient.callAsCurrentUser('security.authenticate', {
            some: 'some',
            headers: { additionalHeader: 'Oh Yes!' },
        })).resolves.toBe(mockResponse);
        expect(scopedAPICaller).toHaveBeenCalledTimes(1);
        expect(scopedAPICaller).toHaveBeenCalledWith('security.authenticate', { some: 'some', headers: { one: '1', additionalHeader: 'Oh Yes!' } }, undefined);
    });
    test('callAsCurrentUser cannot override default headers', async () => {
        ...
        const withHeaderOverride = async () => clusterClient.callAsCurrentUser('security.authenticate', { headers: { one: 'OVERRIDE' } });
    ...
    test('does not attach headers to the client params if they are not available', async () => {
        const mockResponse = { data: 'response' };
        scopedAPICaller.mockResolvedValue(mockResponse);
        const clusterClientWithoutHeaders = new scoped_cluster_client_1.ScopedClusterClient(internalAPICaller, scopedAPICaller);
        ...
        scopedAPICaller.mockClear();
        await expect(clusterClientWithoutHeaders.callAsCurrentUser('security.authenticate', { some: 'some' })).resolves.toBe(mockResponse);
        expect(scopedAPICaller).toHaveBeenCalledTimes(1);
        expect(scopedAPICaller).toHaveBeenCalledWith('security.authenticate', { some: 'some' }, undefined);
        expect(internalAPICaller).not.toHaveBeenCalled();
    });
});
