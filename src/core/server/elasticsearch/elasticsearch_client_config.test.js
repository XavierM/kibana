...
test('parses minimally specified config', () => {
    expect(elasticsearch_client_config_1.parseElasticsearchClientConfig({
        apiVersion: 'master',
    ...
    }, logger.get())).toMatchInlineSnapshot(`
Object {
  "apiVersion": "master",
  "hosts": Array [
    Object {
...
test('parses fully specified config', () => {
    ...
    const elasticsearchClientConfig = elasticsearch_client_config_1.parseElasticsearchClientConfig(elasticsearchConfig, logger.get());
...
test('parses config timeouts of moment.Duration type', () => {
    expect(elasticsearch_client_config_1.parseElasticsearchClientConfig({
        ...
        sniffOnStart: false,
        sniffOnConnectionFault: false,
        pingTimeout: moment_1.duration(100, 'ms'),
        requestTimeout: moment_1.duration(30, 's'),
        sniffInterval: moment_1.duration(1, 'minute'),
...
describe('#auth', () => {
    test('is not set if #auth = false even if username and password are provided', () => {
        expect(elasticsearch_client_config_1.parseElasticsearchClientConfig({
            ...
            password: 'changeme',
            requestHeadersWhitelist: [],
        }, logger.get(), { auth: false })).toMatchInlineSnapshot(`
Object {
  "apiVersion": "v7.0.0",
  "hosts": Array [
    Object {
...
`);
    ...
    test('is not set if username is not specified', () => {
        expect(elasticsearch_client_config_1.parseElasticsearchClientConfig({
            ...
            requestHeadersWhitelist: [],
            password: 'changeme',
        }, logger.get(), { auth: true })).toMatchInlineSnapshot(`
Object {
  "apiVersion": "v7.0.0",
  "hosts": Array [
    Object {
...
describe('#log', () => {
    test('default logger with #logQueries = false', () => {
        const parsedConfig = elasticsearch_client_config_1.parseElasticsearchClientConfig({
            apiVersion: 'master',
            customHeaders: { xsrf: 'something' },
            logQueries: false,
            sniffOnStart: false,
        ...
        expect(logging_service_mock_1.loggingServiceMock.collect(logger)).toMatchInlineSnapshot(`
Object {
  "debug": Array [],
  "error": Array [
    Array [
...
`);
    ...
    test('#verificationMode = certificate', () => {
        const clientConfig = elasticsearch_client_config_1.parseElasticsearchClientConfig({
            apiVersion: 'v7.0.0',
            customHeaders: {},
            logQueries: true,
            sniffOnStart: true,
        ...
        expect(clientConfig.ssl.checkServerIdentity('right.com', { subject: { CN: 'wrong.com' } })).toBeUndefined();
...
`);
    ...
    test('#verificationMode is unknown', () => {
        expect(() => elasticsearch_client_config_1.parseElasticsearchClientConfig({
            ...
            hosts: ['https://es.local'],
            requestHeadersWhitelist: [],
            ssl: { verificationMode: 'misspelled' },
        }, logger.get())).toThrowErrorMatchingInlineSnapshot(`"Unknown ssl verificationMode: misspelled"`);
    });
    test('#ignoreCertAndKey = true', () => {
        ...
        }, logger.get(), { ignoreCertAndKey: true })).toMatchInlineSnapshot(`
Object {
  "apiVersion": "v7.0.0",
  "hosts": Array [
    Object {
