...
const create_or_upgrade_saved_config_test_mock_1 = require("./create_or_upgrade_saved_config/create_or_upgrade_saved_config.test.mock");
...
const logger = logging_service_mock_1.loggingServiceMock.create().get();
const TYPE = 'config';
...
describe('ui settings', () => {
    function setup(options = {}) {
        ...
        savedObjectsClient.get.mockReturnValue({ attributes: esDocSource });
        const uiSettings = new ui_settings_client_1.UiSettingsClient({
            type: TYPE,
            id: ID,
            buildNum: BUILD_NUM,
            defaults,
        ...
        const createOrUpgradeSavedConfig = create_or_upgrade_saved_config_test_mock_1.createOrUpgradeSavedConfigMock;
        return {
            uiSettings,
            savedObjectsClient,
    ...
    describe('#setMany()', () => {
        ...
        it('automatically creates the savedConfig if it is missing', async () => {
            ...
            savedObjectsClient.update
                .mockRejectedValueOnce(saved_objects_1.SavedObjectsClient.errors.createGenericNotFoundError())
            ...
            expect(createOrUpgradeSavedConfig).toHaveBeenCalledWith(expect.objectContaining({ handleWriteErrors: false }));
        });
        it('only tried to auto create once and throws NotFound', async () => {
            const { uiSettings, savedObjectsClient, createOrUpgradeSavedConfig } = setup();
            savedObjectsClient.update.mockRejectedValue(saved_objects_1.SavedObjectsClient.errors.createGenericNotFoundError());
            try {
                await uiSettings.setMany({ foo: 'bar' });
                throw new Error('expected setMany to throw a NotFound error');
            }
            catch (error) {
                expect(saved_objects_1.SavedObjectsClient.errors.isNotFoundError(error)).toBe(true);
            }
            expect(savedObjectsClient.update).toHaveBeenCalledTimes(2);
            expect(createOrUpgradeSavedConfig).toHaveBeenCalledTimes(1);
            expect(createOrUpgradeSavedConfig).toHaveBeenCalledWith(expect.objectContaining({ handleWriteErrors: false }));
        ...
        it('throws CannotOverrideError if the key is overridden', async () => {
            ...
            try {
                ...
                expect(error).toBeInstanceOf(ui_settings_errors_1.CannotOverrideError);
                expect(error.message).toBe('Unable to update "foo" because it is overridden');
            }
        });
        it('validates value if a schema presents', async () => {
            const defaults = { foo: { schema: config_schema_1.schema.string() } };
            const { uiSettings, savedObjectsClient } = setup({ defaults });
            await expect(uiSettings.setMany({
                bar: 2,
                foo: 1,
            })).rejects.toMatchInlineSnapshot(`[Error: [validation [foo]]: expected value of type [string] but got [number]]`);
    ...
    describe('#set()', () => {
        ...
        it('validates value if a schema presents', async () => {
            const defaults = { foo: { schema: config_schema_1.schema.string() } };
            const { uiSettings, savedObjectsClient } = setup({ defaults });
            await expect(uiSettings.set('foo', 1)).rejects.toMatchInlineSnapshot(`[Error: [validation [foo]]: expected value of type [string] but got [number]]`);
            expect(savedObjectsClient.update).toHaveBeenCalledTimes(0);
        });
        it('throws CannotOverrideError if the key is overridden', async () => {
            const { uiSettings } = setup({
    ...
    describe('#getRegistered()', () => {
        ...
        it('does not leak validation schema outside', () => {
            const value = chance.word();
            const defaults = { key: { value, schema: config_schema_1.schema.string() } };
            const { uiSettings } = setup({ defaults });
    ...
    describe('#getUserProvided()', () => {
        ...
        it('ignores user-configured value if it fails validation', async () => {
            ...
            const defaults = {
                id: {
                    value: 42,
                    schema: config_schema_1.schema.number(),
                },
            };
      ...
      `);
        ...
        it('automatically creates the savedConfig if it is missing and returns empty object', async () => {
            ...
            expect(createOrUpgradeSavedConfig).toHaveBeenCalledTimes(1);
            expect(createOrUpgradeSavedConfig).toHaveBeenCalledWith(expect.objectContaining({ handleWriteErrors: true }));
        });
        it('returns result of savedConfig creation in case of notFound error', async () => {
            ...
            savedObjectsClient.get.mockRejectedValue(saved_objects_1.SavedObjectsClient.errors.createGenericNotFoundError());
        ...
        it('returns an empty object on Forbidden responses', async () => {
            const { uiSettings, savedObjectsClient, createOrUpgradeSavedConfig } = setup();
            const error = saved_objects_1.SavedObjectsClient.errors.decorateForbiddenError(new Error());
        ...
        it('returns an empty object on EsUnavailable responses', async () => {
            const { uiSettings, savedObjectsClient, createOrUpgradeSavedConfig } = setup();
            const error = saved_objects_1.SavedObjectsClient.errors.decorateEsUnavailableError(new Error());
            savedObjectsClient.get.mockRejectedValue(error);
            expect(await uiSettings.getUserProvided()).toStrictEqual({});
            expect(createOrUpgradeSavedConfig).toHaveBeenCalledTimes(0);
        });
        it('throws Unauthorized errors', async () => {
            const { uiSettings, savedObjectsClient } = setup();
            const error = saved_objects_1.SavedObjectsClient.errors.decorateNotAuthorizedError(new Error());
            savedObjectsClient.get.mockRejectedValue(error);
            try {
                await uiSettings.getUserProvided();
    ...
    describe('#get()', () => {
        ...
        it('returns the overridden value if the document does not exist', async () => {
            ...
            savedObjectsClient.get.mockRejectedValueOnce(saved_objects_1.SavedObjectsClient.errors.createGenericNotFoundError());
            expect(await uiSettings.get('dateFormat')).toBe('foo');
        });
