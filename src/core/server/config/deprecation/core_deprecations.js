...
const configPathDeprecation = (settings, fromPath, log) => {
    if (lodash_1.has(process.env, 'CONFIG_PATH')) {
...
const dataPathDeprecation = (settings, fromPath, log) => {
    if (lodash_1.has(process.env, 'DATA_PATH')) {
...
const xsrfDeprecation = (settings, fromPath, log) => {
    if (lodash_1.has(settings, 'server.xsrf.whitelist') &&
        lodash_1.get(settings, 'server.xsrf.whitelist').length > 0) {
        log('It is not recommended to disable xsrf protections for API endpoints via [server.xsrf.whitelist]. ' +
...
const rewriteBasePathDeprecation = (settings, fromPath, log) => {
    if (lodash_1.has(settings, 'server.basePath') && !lodash_1.has(settings, 'server.rewriteBasePath')) {
        log('You should set server.basePath along with server.rewriteBasePath. Starting in 7.0, Kibana ' +
...
const cspRulesDeprecation = (settings, fromPath, log) => {
    ...
    const rules = lodash_1.get(settings, 'csp.rules');
    if (rules) {
        ...
        settings.csp.rules = [...parsed].map(([policy, sourceList]) => {
            ...
            if (SELF_POLICIES.includes(policy) &&
...
const mapManifestServiceUrlDeprecation = (settings, fromPath, log) => {
    if (lodash_1.has(settings, 'map.manifestServiceUrl')) {
        log('You should no longer use the map.manifestServiceUrl setting in kibana.yml to configure the location ' +
...
exports.coreDeprecationProvider = ({ unusedFromRoot, renameFromRoot, }) => [
