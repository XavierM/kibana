...
const core_deprecations_1 = require("./core_deprecations");
const deprecation_factory_1 = require("./deprecation_factory");
const apply_deprecations_1 = require("./apply_deprecations");
const initialEnv = { ...process.env };
const applyCoreDeprecations = (settings = {}) => {
    const deprecations = core_deprecations_1.coreDeprecationProvider(deprecation_factory_1.configDeprecationFactory);
    const deprecationMessages = [];
    const migrated = apply_deprecations_1.applyDeprecations(settings, deprecations.map(deprecation => ({
        deprecation,
        path: '',
...
describe('core deprecations', () => {
    ...
    describe('cspRulesDeprecation', () => {
        ...
        `);
            ...
            it('replaces a nonce', () => {
                expect(applyCoreDeprecations({ csp: { rules: [`script-src 'nonce-{nonce}'`] } }).migrated.csp
                    .rules).toEqual([`script-src 'self'`]);
                expect(applyCoreDeprecations({ csp: { rules: [`script-src 'unsafe-eval' 'nonce-{nonce}'`] } })
                    .migrated.csp.rules).toEqual([`script-src 'unsafe-eval' 'self'`]);
            });
            it('removes a quoted nonce', () => {
                expect(applyCoreDeprecations({ csp: { rules: [`script-src 'self' 'nonce-{nonce}'`] } }).migrated
                    .csp.rules).toEqual([`script-src 'self'`]);
                expect(applyCoreDeprecations({ csp: { rules: [`script-src 'nonce-{nonce}' 'self'`] } }).migrated
                    .csp.rules).toEqual([`script-src 'self'`]);
            });
            ...
            it('removes a strange nonce', () => {
                expect(applyCoreDeprecations({ csp: { rules: [`script-src 'self' blah-{nonce}-wow`] } }).migrated
                    .csp.rules).toEqual([`script-src 'self'`]);
            });
            it('removes multiple nonces', () => {
                expect(applyCoreDeprecations({
                    csp: {
                        rules: [
                ...
                }).migrated.csp.rules).toEqual([`script-src 'self'`, `style-src 'self'`]);
            });
        });
        ...
        `);
            ...
            it('adds self', () => {
                expect(applyCoreDeprecations({ csp: { rules: [`script-src 'unsafe-eval'`] } }).migrated.csp.rules).toEqual([`script-src 'unsafe-eval' 'self'`]);
            });
        });
        it('does not add self to other policies', () => {
            expect(applyCoreDeprecations({ csp: { rules: [`worker-src blob:`] } }).migrated.csp.rules).toEqual([`worker-src blob:`]);
        });
    });
});
