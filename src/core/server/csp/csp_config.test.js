...
describe('CspConfig', () => {
    test('DEFAULT', () => {
        expect(_1.CspConfig.DEFAULT).toMatchInlineSnapshot(`
      CspConfig {
    ...
    test('defaults from config', () => {
        expect(new _1.CspConfig()).toMatchInlineSnapshot(`
      CspConfig {
        "header": "script-src 'unsafe-eval' 'self'; worker-src blob: 'self'; style-src 'unsafe-inline' 'self'",
        "rules": Array [
          "script-src 'unsafe-eval' 'self'",
    ...
    test('creates from partial config', () => {
        expect(new _1.CspConfig({ strict: false, warnLegacyBrowsers: false })).toMatchInlineSnapshot(`
      CspConfig {
        "header": "script-src 'unsafe-eval' 'self'; worker-src blob: 'self'; style-src 'unsafe-inline' 'self'",
        "rules": Array [
          "script-src 'unsafe-eval' 'self'",
    ...
    test('computes header from rules', () => {
        const cspConfig = new _1.CspConfig({ rules: ['alpha', 'beta', 'gamma'] });
