...
class Root {
    constructor(rawConfigProvider, env, onShutdown) {
        ...
        this.server = new server_1.Server(rawConfigProvider, env, this.logger);
    }
    async setup() {
        try {
    ...
    async setupLogging() {
        ...
        operators_1.switchMap(() => configService.atPath('logging')), operators_1.map(config => this.loggingService.upgrade(config)), 
