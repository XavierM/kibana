...
jest.doMock(path_1.join('plugin-with-initializer-path', 'server'), () => ({ plugin: mockPluginInitializer }), { virtual: true });
jest.doMock(path_1.join('plugin-without-initializer-path', 'server'), () => ({}), {
    virtual: true,
});
jest.doMock(path_1.join('plugin-with-wrong-initializer-path', 'server'), () => ({ plugin: {} }), {
    virtual: true,
});
...
beforeEach(() => {
    ...
    coreContext = { coreId, env, logger, configService: configService };
});
afterEach(() => {
    jest.clearAllMocks();
});
test('`constructor` correctly initializes plugin instance', () => {
    ...
    const plugin = new plugin_1.PluginWrapper({
...
test('`setup` fails if `plugin` initializer is not exported', async () => {
    ...
    await expect(plugin.setup(plugin_context_1.createPluginSetupContext(coreContext, setupDeps, plugin), {})).rejects.toMatchInlineSnapshot(`[Error: Plugin "some-plugin-id" does not export "plugin" definition (plugin-without-initializer-path).]`);
});
test('`setup` fails if plugin initializer is not a function', async () => {
    ...
    await expect(plugin.setup(plugin_context_1.createPluginSetupContext(coreContext, setupDeps, plugin), {})).rejects.toMatchInlineSnapshot(`[Error: Definition of plugin "some-plugin-id" should be a function (plugin-with-wrong-initializer-path).]`);
});
test('`setup` fails if initializer does not return object', async () => {
    ...
    await expect(plugin.setup(plugin_context_1.createPluginSetupContext(coreContext, setupDeps, plugin), {})).rejects.toMatchInlineSnapshot(`[Error: Initializer for plugin "some-plugin-id" is expected to return plugin instance, but returned "null".]`);
});
test('`setup` fails if object returned from initializer does not define `setup` function', async () => {
    ...
    await expect(plugin.setup(plugin_context_1.createPluginSetupContext(coreContext, setupDeps, plugin), {})).rejects.toMatchInlineSnapshot(`[Error: Instance of plugin "some-plugin-id" does not define "setup" function.]`);
});
test('`setup` initializes plugin and calls appropriate lifecycle hook', async () => {
    const manifest = createPluginManifest();
    const opaqueId = Symbol();
    const initializerContext = plugin_context_1.createPluginInitializerContext(coreContext, opaqueId, manifest);
    const plugin = new plugin_1.PluginWrapper({
        path: 'plugin-with-initializer-path',
        manifest,
        opaqueId,
    ...
    mockPluginInitializer.mockReturnValue(mockPluginInstance);
    const setupContext = plugin_context_1.createPluginSetupContext(coreContext, setupDeps, plugin);
...
test('`start` fails if setup is not called first', async () => {
    ...
    await expect(plugin.start({}, {})).rejects.toThrowErrorMatchingInlineSnapshot(`"Plugin \\"some-plugin-id\\" can't be started since it isn't set up."`);
});
...
test('`stop` fails if plugin is not set up', async () => {
    ...
    mockPluginInitializer.mockReturnValue(mockPluginInstance);
    await expect(plugin.stop()).rejects.toMatchInlineSnapshot(`[Error: Plugin "some-plugin-id" can't be stopped since it isn't set up.]`);
...
test('`stop` does nothing if plugin does not define `stop` function', async () => {
    ...
    await plugin.setup(plugin_context_1.createPluginSetupContext(coreContext, setupDeps, plugin), {});
...
describe('#getConfigSchema()', () => {
    it('reads config schema from plugin', () => {
        const pluginSchema = config_schema_1.schema.any();
        const configDescriptor = {
            schema: pluginSchema,
        };
        jest.doMock('plugin-with-schema/server', () => ({
    ...
    it('throws if plugin contains invalid schema', () => {
        jest.doMock('plugin-invalid-schema/server', () => ({
            config: {
                schema: {
                    validate: () => null,
                },
        ...
        expect(() => plugin.getConfigDescriptor()).toThrowErrorMatchingInlineSnapshot(`"Configuration schema expected to be an instance of Type"`);
    });
});
