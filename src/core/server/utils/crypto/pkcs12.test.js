...
const readPem = (file) => {
    const raw = fs_1.readFileSync(file, 'utf8');
...
let pemKey;
beforeAll(() => {
    pemCA = readPem(dev_utils_1.CA_CERT_PATH);
    pemCert = readPem(dev_utils_1.ES_CERT_PATH);
    pemKey = readPem(dev_utils_1.ES_KEY_PATH);
});
describe('#readPkcs12Keystore', () => {
    const expectKey = (pkcs12ReadResult) => {
        const result = reformatPem(pkcs12ReadResult.key);
        expect(result).toEqual(pemKey);
    };
    const expectCert = (pkcs12ReadResult) => {
        const result = reformatPem(pkcs12ReadResult.cert);
        expect(result).toEqual(pemCert);
    };
    const expectCA = (pkcs12ReadResult, ca = [pemCA]) => {
    ...
    describe('Succeeds when the correct password is used', () => {
        let pkcs12ReadResult;
        beforeAll(() => {
            // this is expensive, just do it once
            pkcs12ReadResult = index_1.readPkcs12Keystore(dev_utils_1.ES_P12_PATH, dev_utils_1.ES_P12_PASSWORD);
        });
    ...
    describe('Succeeds on a key store with an empty password', () => {
        ...
        beforeAll(() => {
            // this is expensive, just do it once
            pkcs12ReadResult = index_1.readPkcs12Keystore(dev_utils_1.ES_EMPTYPASSWORD_P12_PATH, '');
        });
        it('Extracts the PEM key', () => {
            expectKey(pkcs12ReadResult);
        });
    ...
    describe('Succeeds on a key store with no password', () => {
        ...
        beforeAll(() => {
            // this is expensive, just do it once
            pkcs12ReadResult = index_1.readPkcs12Keystore(dev_utils_1.ES_NOPASSWORD_P12_PATH);
        });
        it('Extracts the PEM key', () => {
            expectKey(pkcs12ReadResult);
        });
    ...
    describe('Handles other key store permutations', () => {
        it('Succeeds with no key', () => {
            const pkcs12ReadResult = index_1.readPkcs12Keystore(__fixtures__1.NO_KEY_PATH, '');
        ...
        it('Succeeds with no instance certificate', () => {
            const pkcs12ReadResult = index_1.readPkcs12Keystore(__fixtures__1.NO_CERT_PATH, '');
            expectCA(pkcs12ReadResult);
            expect(pkcs12ReadResult.cert).toBeUndefined();
            expectKey(pkcs12ReadResult);
        });
        it('Succeeds with no CA certificate', () => {
            const pkcs12ReadResult = index_1.readPkcs12Keystore(__fixtures__1.NO_CA_PATH, '');
            expect(pkcs12ReadResult.ca).toBeUndefined();
            expectCert(pkcs12ReadResult);
            expectKey(pkcs12ReadResult);
        });
        it('Succeeds with two CA certificates', () => {
            const pkcs12ReadResult = index_1.readPkcs12Keystore(__fixtures__1.TWO_CAS_PATH, '');
    ...
    describe('Throws errors', () => {
        const expectError = (password) => {
            expect(() => index_1.readPkcs12Keystore(dev_utils_1.ES_P12_PATH, password)).toThrowError('PKCS#12 MAC could not be verified. Invalid password?');
        };
        ...
        it('When an invalid file path is used', () => {
            ...
            expect(() => index_1.readPkcs12Keystore(path)).toThrowError(`ENOENT: no such file or directory, open '${path}'`);
        });
        it('When two keys are present', () => {
            expect(() => index_1.readPkcs12Keystore(__fixtures__1.TWO_KEYS_PATH, '')).toThrowError('Keystore contains multiple private keys.');
        });
    });
});
describe('#readPkcs12Truststore', () => {
    it('reads all certificates into one CA array and discards any certificates that have keys', () => {
        const ca = index_1.readPkcs12Truststore(dev_utils_1.ES_P12_PATH, dev_utils_1.ES_P12_PASSWORD);
