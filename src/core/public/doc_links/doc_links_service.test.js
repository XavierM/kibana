...
const doc_links_service_1 = require("./doc_links_service");
...
describe('DocLinksService#start()', () => {
    it('templates the doc links with the branch information from injectedMetadata', () => {
        const injectedMetadata = injected_metadata_service_mock_1.injectedMetadataServiceMock.createStartContract();
        injectedMetadata.getKibanaBranch.mockReturnValue('test-branch');
        const service = new doc_links_service_1.DocLinksService();
        const start = service.start({ injectedMetadata });
        expect(start.DOC_LINK_VERSION).toEqual('test-branch');
        expect(start.links.kibana).toEqual('https://www.elastic.co/guide/en/kibana/test-branch/index.html');
    });
});
