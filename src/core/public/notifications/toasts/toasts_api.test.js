...
async function getCurrentToasts(toasts) {
    return await toasts
        .get$()
...
function uiSettingsMock() {
    const mock = ui_settings_service_mock_1.uiSettingsServiceMock.createSetupContract();
    mock.get.mockImplementation(() => (config) => {
        switch (config) {
...
function startDeps() {
    return { overlays: {}, i18n: i18n_service_mock_1.i18nServiceMock.createStartContract() };
}
describe('#get$()', () => {
    it('returns observable that emits NEW toast list when something added or removed', () => {
        const toasts = new toasts_api_1.ToastsApi(toastDeps());
