...
const saved_objects_client_1 = require("./saved_objects_client");
...
describe('SavedObjectsClient', () => {
    ...
    beforeEach(() => {
        savedObjectsClient = new saved_objects_client_1.SavedObjectsClient(http);
    ...
    describe('#get', () => {
        ...
        test('rejects if `type` parameter is undefined', () => {
            return expect(savedObjectsClient.get(undefined, undefined)).rejects.toMatchInlineSnapshot(`[Error: requires type and id]`);
        });
        test('rejects if `id` parameter is undefined', () => {
            return expect(savedObjectsClient.get('index-pattern', undefined)).rejects.toMatchInlineSnapshot(`[Error: requires type and id]`);
        });
        test('rejects when HTTP call fails', () => {
            http.fetch.mockRejectedValue(new Error('Request failed'));
            return expect(savedObjectsClient.get(doc.type, doc.id)).rejects.toMatchInlineSnapshot(`[Error: Request failed]`);
        });
      ...
      `);
        ...
        test('resolves with SimpleSavedObject instance', async () => {
            ...
            await expect(response).resolves.toBeInstanceOf(simple_saved_object_1.SimpleSavedObject);
    ...
    describe('#delete', () => {
        ...
        test('rejects if `type` parameter is undefined', async () => {
            expect(savedObjectsClient.delete(undefined, undefined)).rejects.toMatchInlineSnapshot(`[Error: requires type and id]`);
        });
        test('throws if `id` parameter is undefined', async () => {
            expect(savedObjectsClient.delete('index-pattern', undefined)).rejects.toMatchInlineSnapshot(`[Error: requires type and id]`);
        });
        test('makes HTTP call', async () => {
    ...
    describe('#update', () => {
        ...
        test('rejects if `type` is undefined', async () => {
            expect(savedObjectsClient.update(undefined, undefined, undefined)).rejects.toMatchInlineSnapshot(`[Error: requires type, id and attributes]`);
        });
        test('rejects if `id` is undefined', async () => {
            expect(savedObjectsClient.update('index-pattern', undefined, undefined)).rejects.toMatchInlineSnapshot(`[Error: requires type, id and attributes]`);
        });
        test('rejects if `attributes` is undefined', async () => {
            expect(savedObjectsClient.update('index-pattern', 'logstash-*', undefined)).rejects.toMatchInlineSnapshot(`[Error: requires type, id and attributes]`);
        });
      ...
      `);
        ...
        test('rejects when HTTP call fails', async () => {
            ...
            await expect(savedObjectsClient.update('index-pattern', 'logstash-*', attributes, options)).rejects.toMatchInlineSnapshot(`[Error: Request failed]`);
        });
        test('resolves with SimpleSavedObject instance', async () => {
            const response = savedObjectsClient.update('index-pattern', 'logstash-*', attributes, options);
            await expect(response).resolves.toBeInstanceOf(simple_saved_object_1.SimpleSavedObject);
            const result = await response;
    ...
    describe('#create', () => {
        ...
        test('rejects if `type` is undefined', async () => {
            await expect(savedObjectsClient.create(undefined, undefined)).rejects.toMatchInlineSnapshot(`[Error: requires type and attributes]`);
        });
        test('resolves with SimpleSavedObject instance', async () => {
      ...
      `);
        ...
        test('rejects when HTTP call fails', async () => {
            http.fetch.mockRejectedValueOnce(new Error('Request failed'));
            await expect(savedObjectsClient.create('index-pattern', attributes, { id: 'myId' })).rejects.toMatchInlineSnapshot(`[Error: Request failed]`);
        });
    });
    describe('#bulk_create', () => {
        ...
        test('resolves with array of SimpleSavedObject instances', async () => {
            ...
            expect(result.savedObjects[0]).toBeInstanceOf(simple_saved_object_1.SimpleSavedObject);
        });
        test('makes HTTP call', async () => {
