...
class LegacyPlatformService {
    ...
    setup({ core, plugins }) {
        ...
        core.injectedMetadata.getLegacyMetadata().nav.forEach((navLink) => core.application.registerLegacyApp({
            id: navLink.id,
            order: navLink.order,
            title: navLink.title,
    ...
    start({ core, targetDomElement, plugins, lastSubUrlStorage = window.sessionStorage, }) {
    ...
    stop() {
        ...
        const angularRoot = angular_1.default.element(this.targetDomElement);
...
const notSupported = (methodName) => (...args) => {
