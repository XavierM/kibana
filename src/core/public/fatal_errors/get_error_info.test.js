...
class StubEsError extends Error {
    constructor(resp) {
...
it('should prepend the `source` to the message', () => {
    expect(get_error_info_1.getErrorInfo('error message', 'unit_test')).toEqual({
        message: 'unit_test: error message',
    });
});
it('should handle a simple string', () => {
    expect(get_error_info_1.getErrorInfo('error message')).toEqual({
        message: 'error message',
    });
});
it('reads the message and stack from an Error object', () => {
    const err = new Error('error message');
    expect(get_error_info_1.getErrorInfo(err)).toEqual({
        message: 'error message',
...
it('reads the root cause reason from elasticsearch errors', () => {
    ...
    expect(get_error_info_1.getErrorInfo(err, 'foo')).toEqual({
...
it('should prepend the stack with the error message if it is not already there', () => {
    ...
    expect(get_error_info_1.getErrorInfo(error)).toEqual({
        message: 'Foo',
        stack: 'Error: Foo\nbar.js:1:1\nbaz.js:2:1\n',
    });
});
