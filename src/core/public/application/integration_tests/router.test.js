...
describe('AppContainer', () => {
    ...
    let appStatuses$;
    let update;
    let scopedAppHistory;
    const navigate = (path) => {
        globalHistory.push(path);
        return update();
    };
    const mockMountersToMounters = () => new Map([...mounters].map(([appId, { mounter }]) => [appId, mounter]));
    const setAppLeaveHandlerMock = () => undefined;
    const mountersToAppStatus$ = () => {
        return new rxjs_1.BehaviorSubject(new Map([...mounters.keys()].map(id => [
            id,
            id.startsWith('disabled') ? types_1.AppStatus.inaccessible : types_1.AppStatus.accessible,
        ])));
    };
    beforeEach(() => {
        mounters = new Map([
            utils_1.createAppMounter({ appId: 'app1', html: '<span>App 1</span>' }),
            utils_1.createLegacyAppMounter('legacyApp1', jest.fn()),
            utils_1.createAppMounter({ appId: 'app2', html: '<div>App 2</div>' }),
            utils_1.createLegacyAppMounter('baseApp:legacyApp2', jest.fn()),
            utils_1.createAppMounter({
                appId: 'app3',
            ...
            utils_1.createAppMounter({ appId: 'disabledApp', html: '<div>Disabled app</div>' }),
            utils_1.createLegacyAppMounter('disabledLegacyApp', jest.fn()),
            utils_1.createAppMounter({
                appId: 'scopedApp',
        ...
        globalHistory = history_1.createMemoryHistory();
        appStatuses$ = mountersToAppStatus$();
        update = utils_1.createRenderer(react_1.default.createElement(ui_1.AppRouter, { history: globalHistory, mounters: mockMountersToMounters(), "appStatuses$": appStatuses$, setAppLeaveHandler: setAppLeaveHandlerMock }));
    });
    ...
    `);
        const app1Unmount = await utils_1.getUnmounter(app1);
    ...
    `);
        const standardAppUnmount = await utils_1.getUnmounter(standardApp);
    ...
    `);
        const chromelessAppUnmount = await utils_1.getUnmounter(standardApp);
    ...
    `);
        const chromelessAppAUnmount = await utils_1.getUnmounter(chromelessAppA);
    ...
    `);
        const chromelessAppBUnmount = await utils_1.getUnmounter(chromelessAppB);
        dom = await navigate('/chromeless-a/path');
    ...
    it('should not mount when partial route path matches', async () => {
        mounters.set(...utils_1.createAppMounter({
            appId: 'spaces',
        ...
        globalHistory = history_1.createMemoryHistory();
        update = utils_1.createRenderer(react_1.default.createElement(ui_1.AppRouter, { history: globalHistory, mounters: mockMountersToMounters(), "appStatuses$": mountersToAppStatus$(), setAppLeaveHandler: setAppLeaveHandlerMock }));
    ...
    it('should not remount when when changing pages within app using hash history', async () => {
        globalHistory = history_1.createHashHistory();
        update = utils_1.createRenderer(react_1.default.createElement(ui_1.AppRouter, { history: globalHistory, mounters: mockMountersToMounters(), "appStatuses$": mountersToAppStatus$(), setAppLeaveHandler: setAppLeaveHandlerMock }));
        const { mounter, unmount } = mounters.get('app1');
        await navigate('/app/app1/page1');
        expect(mounter.mount).toHaveBeenCalledTimes(1);
    ...
    it('calls legacy mount handler', async () => {
        ...
        expect(mounters.get('legacyApp1').mounter.mount.mock.calls[0][0]).toMatchObject({
            ...
            history: expect.any(scoped_history_1.ScopedHistory),
        });
    });
    ...
    it('displays error page if no app is found', async () => {
        ...
        expect(dom?.exists(ui_1.AppNotFound)).toBe(true);
    });
