...
describe('ScopedHistory', () => {
    describe('construction', () => {
        it('succeeds if current location matches basePath', () => {
            const gh = history_1.createMemoryHistory();
            gh.push('/app/wow');
            expect(() => new scoped_history_1.ScopedHistory(gh, '/app/wow')).not.toThrow();
            gh.push('/app/wow/');
            expect(() => new scoped_history_1.ScopedHistory(gh, '/app/wow')).not.toThrow();
        ...
        it('fails if current location does not match basePath', () => {
            const gh = history_1.createMemoryHistory();
            gh.push('/app/other');
            expect(() => new scoped_history_1.ScopedHistory(gh, '/app/wow')).toThrowErrorMatchingInlineSnapshot(`"Browser location [/app/other] is not currently in expected basePath [/app/wow]"`);
        });
    });
    describe('navigation', () => {
        it('supports push', () => {
            ...
            const h = new scoped_history_1.ScopedHistory(gh, '/app/wow');
    ...
    describe('teardown behavior', () => {
        it('throws exceptions after falling out of scope', () => {
            ...
            const h = new scoped_history_1.ScopedHistory(gh, '/app/wow');
            gh.push('/app/other');
            expect(() => h.location).toThrowErrorMatchingInlineSnapshot(`"ScopedHistory instance has fell out of navigation scope for basePath: /app/wow"`);
    ...
    describe('createHref', () => {
        it('creates scoped hrefs', () => {
            ...
            expect(h.createHref({ pathname: '/new-page', search: '?alpha=true' })).toEqual(`/app/wow/new-page?alpha=true`);
        });
        ...
        it('skips the scoped history path when `prependBasePath` is false', () => {
            ...
            expect(h.createHref({ pathname: '/new-page', search: '?alpha=true' }, { prependBasePath: false })).toEqual(`/new-page?alpha=true`);
        });
    });
    describe('action', () => {
    ...
    describe('createSubHistory', () => {
        it('supports push', () => {
            ...
            const h1 = new scoped_history_1.ScopedHistory(gh, '/app/wow');
            h1.push('/new-page');
