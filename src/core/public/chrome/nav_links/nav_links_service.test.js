...
const nav_links_service_1 = require("./nav_links_service");
...
describe('NavLinksService', () => {
    ...
    let start;
    beforeEach(() => {
        service = new nav_links_service_1.NavLinksService();
        mockAppService = {
            applications$: new rxjs_1.BehaviorSubject(availableApps),
        };
    ...
    describe('#getNavLinks$()', () => {
        it('does not include `chromeless` applications', async () => {
            expect(await start
                .getNavLinks$()
                .pipe(operators_1.take(1), operators_1.map(links => links.map(l => l.id)))
                .toPromise()).not.toContain('chromelessApp');
        });
        it('sorts navlinks by `order` property', async () => {
            expect(await start
                .getNavLinks$()
                .pipe(operators_1.take(1), operators_1.map(links => links.map(l => l.id)))
                .toPromise()).toEqual(['app2', 'legacyApp2', 'app1', 'legacyApp1', 'legacyApp3']);
        });
        it('emits multiple values', async () => {
            const navLinkIds$ = start.getNavLinks$().pipe(operators_1.map(links => links.map(l => l.id)));
            const emittedLinks = [];
    ...
    describe('#showOnly()', () => {
        ...
        it('still removes all other links when availableApps are re-emitted', async () => {
            ...
            expect(await start
                .getNavLinks$()
                .pipe(operators_1.take(1), operators_1.map(links => links.map(l => l.id)))
                .toPromise()).toEqual(['legacyApp2']);
        });
    });
    describe('#update()', () => {
        it('updates the navlinks and returns the updated link', async () => {
            expect(start.update('legacyApp1', { hidden: true })).toEqual(expect.objectContaining({
                appUrl: '/app1',
                disabled: false,
                hidden: true,
                icon: 'legacyApp1',
            ...
            const hiddenLinkIds = await start
                .getNavLinks$()
                .pipe(operators_1.take(1), operators_1.map(links => links.filter(l => l.hidden).map(l => l.id)))
                .toPromise();
    ...
    describe('#enableForcedAppSwitcherNavigation()', () => {
        it('flips #getForceAppSwitcherNavigation$()', async () => {
            await expect(start
                ...
                .toPromise()).resolves.toBe(false);
            start.enableForcedAppSwitcherNavigation();
            await expect(start
                .getForceAppSwitcherNavigation$()
                .pipe(operators_1.take(1))
                .toPromise()).resolves.toBe(true);
        });
    });
});
