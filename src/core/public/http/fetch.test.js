...
describe('Fetch', () => {
    const fetchInstance = new fetch_1.Fetch({
        basePath: new base_path_1.BasePath(BASE_PATH),
    ...
    describe('getRequestCount$', () => {
        ...
        it('should increase and decrease when request fails', async () => {
            client_1.default.get('*', Promise.reject('Network!'));
            const fetchResponse = fetchInstance.fetch('/path');
            expect(await getCurrentRequestCount()).toEqual(1);
            await expect(fetchResponse).rejects.toThrow();
            expect(await getCurrentRequestCount()).toEqual(0);
        ...
        it('should change for multiple requests', async () => {
            client_1.default.get(`${BASE_PATH}/success`, 200);
            client_1.default.get(`${BASE_PATH}/fail`, 400);
            client_1.default.get(`${BASE_PATH}/network-fail`, Promise.reject('Network!'));
            const requestCounts = [];
    ...
    describe('http requests', () => {
        it('should fail with invalid arguments', async () => {
            ...
            await expect(fetchInstance.fetch(
            // @ts-ignore
            { path: '/', headers: { hello: 'world' } }, { headers: { hello: 'mars' } })).rejects.toThrowErrorMatchingInlineSnapshot(`"Invalid fetch arguments, must either be (string, object) or (object, undefined), received (object, object)"`);
        });
        it('should use supplied request method', async () => {
            client_1.default.post('*', {});
            await fetchInstance.fetch('/my/path', { method: 'POST' });
            expect(client_1.default.lastOptions().method).toBe('POST');
        });
        it('should use supplied Content-Type', async () => {
            client_1.default.get('*', {});
            await fetchInstance.fetch('/my/path', { headers: { 'Content-Type': 'CustomContentType' } });
            expect(client_1.default.lastOptions().headers).toMatchObject({
        ...
        it('should use supplied pathname and querystring', async () => {
            ...
            expect(client_1.default.lastUrl()).toBe('http://localhost/myBase/my/path?a=b');
        });
        ...
        it('should not allow overwriting of kbn-version header', async () => {
            client_1.default.get('*', {});
            await expect(fetchInstance.fetch('/my/path', {
                headers: { myHeader: 'foo', 'kbn-version': 'CUSTOM!' },
            })).rejects.toThrowErrorMatchingInlineSnapshot(`"Invalid fetch headers, headers beginning with \\"kbn-\\" are not allowed: [kbn-version]"`);
        });
        it('should not set kbn-system-request header by default', async () => {
            ...
            expect(client_1.default.lastOptions().headers['kbn-system-request']).toBeUndefined();
        });
        ...
        it('should not allow overwriting of kbn-system-request when asSystemRequest: true', async () => {
            ...
            await expect(fetchInstance.fetch('/my/path', {
                ...
                asSystemRequest: true,
            })).rejects.toThrowErrorMatchingInlineSnapshot(`"Invalid fetch headers, headers beginning with \\"kbn-\\" are not allowed: [kbn-system-request]"`);
        });
        ...
        it('should not allow overwriting of kbn-system-api when asSystemRequest: true', async () => {
            ...
            await expect(fetchInstance.fetch('/my/path', {
                ...
                asSystemRequest: true,
            })).rejects.toThrowErrorMatchingInlineSnapshot(`"Invalid fetch headers, headers beginning with \\"kbn-\\" are not allowed: [kbn-system-api]"`);
        });
        // Deprecated header used by legacy platform pre-7.7. Remove in 8.x.
        ...
        it('should make request with defaults', async () => {
            client_1.default.get('*', {});
            await fetchInstance.fetch('/my/path');
            const lastCall = client_1.default.lastCall();
        ...
        it('should reject on network error', async () => {
            expect.assertions(1);
            client_1.default.get('*', { status: 500 });
        ...
        it('should contain error message when throwing response', async () => {
            client_1.default.get('*', { status: 404, body: { foo: 'bar' } });
        ...
        it('preserves the name of the original error', async () => {
            ...
            client_1.default.get('*', Promise.reject(abortError));
        ...
        it('exposes the request to the interceptors in case of aborted request', async () => {
            ...
            expect(interceptedResponse.request).toEqual(expect.objectContaining({
                method: 'GET',
                url: 'http://localhost/myBase/my/path',
            }));
        ...
        it('should support get() helper', async () => {
            ...
            expect(client_1.default.lastOptions().method).toBe('GET');
        });
        it('should support head() helper', async () => {
            client_1.default.head('*', {});
            await fetchInstance.head('/my/path', { method: 'GET' });
            expect(client_1.default.lastOptions().method).toBe('HEAD');
        });
        ...
        it('should support put() helper', async () => {
            client_1.default.put('*', {});
            await fetchInstance.put('/my/path', { method: 'GET', body: '{}' });
            expect(client_1.default.lastOptions().method).toBe('PUT');
        });
        it('should support patch() helper', async () => {
            client_1.default.patch('*', {});
            await fetchInstance.patch('/my/path', { method: 'GET', body: '{}' });
            expect(client_1.default.lastOptions().method).toBe('PATCH');
        });
        it('should support delete() helper', async () => {
            client_1.default.delete('*', {});
            await fetchInstance.delete('/my/path', { method: 'GET' });
            expect(client_1.default.lastOptions().method).toBe('DELETE');
        });
        it('should support options() helper', async () => {
            client_1.default.mock('*', { method: 'OPTIONS' });
            await fetchInstance.options('/my/path', { method: 'GET' });
            expect(client_1.default.lastOptions().method).toBe('OPTIONS');
        });
        it('should make requests for NDJSON content', async () => {
            const content = fs_1.readFileSync(path_1.join(__dirname, '_import_objects.ndjson'), {
                encoding: 'utf-8',
            });
    ...
    describe('interception', () => {
        ...
        it('should make request and receive response', async () => {
            ...
            expect(client_1.default.called()).toBe(true);
        ...
        it('should be able to manipulate request instance', async () => {
            ...
            expect(client_1.default.lastUrl()).toBe('http://localhost/myBase/my/route');
        });
        ...
        it('should skip remaining interceptors when controller halts during request', async () => {
            ...
            expect(client_1.default.called()).toBe(false);
        });
        ...
        it('should accumulate request information', async () => {
            ...
            const createRequest = jest.fn((options) => ({
            ...
            expect(client_1.default.lastCall().request.url).toContain('/api/gamma');
        });
        it('should accumulate response information', async () => {
            const bodies = ['alpha', 'beta', 'gamma'];
            const createResponse = jest.fn((httpResponse) => ({
                body: bodies.shift(),
            }));
            fetchInstance.intercept({
