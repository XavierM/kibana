...
describe('#setup()', () => {
    describe('#register', () => {
        it(`allows paths that don't start with /`, () => {
            const basePath = new base_path_1.BasePath('/foo');
            const anonymousPaths = new anonymous_paths_service_1.AnonymousPathsService().setup({ basePath });
    ...
    describe('#isAnonymous', () => {
        ...
        it('when there is no basePath and calling "isAnonymous" without a starting slash, returns true for paths registered with a starting slash', () => {
            const basePath = new base_path_1.BasePath('/');
            const anonymousPaths = new anonymous_paths_service_1.AnonymousPathsService().setup({ basePath });
            anonymousPaths.register('/bar');
