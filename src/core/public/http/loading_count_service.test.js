...
const loading_count_service_1 = require("./loading_count_service");
describe('LoadingCountService', () => {
    const setup = () => {
        const fatalErrors = fatal_errors_service_mock_1.fatalErrorsServiceMock.createSetupContract();
        const service = new loading_count_service_1.LoadingCountService();
    ...
    describe('addLoadingCountSource()', () => {
        it('subscribes to passed in sources, unsubscribes on stop', () => {
            ...
            loadingCount.addLoadingCountSource(new rxjs_1.Observable(subA));
            expect(subA).toHaveBeenCalledTimes(1);
            expect(unsubA).not.toHaveBeenCalled();
            const unsubB = jest.fn();
            const subB = jest.fn().mockReturnValue(unsubB);
            loadingCount.addLoadingCountSource(new rxjs_1.Observable(subB));
        ...
        it('adds a fatal error if source observables emit an error', () => {
            ...
            loadingCount.addLoadingCountSource(rxjs_1.throwError(new Error('foo bar')));
      ...
      `);
        ...
        it('adds a fatal error if source observable emits a negative number', () => {
            const { loadingCount, fatalErrors } = setup();
            loadingCount.addLoadingCountSource(rxjs_1.of(1, 2, 3, 4, -9));
            expect(fatalErrors.add.mock.calls).toMatchInlineSnapshot(`
        Array [
          Array [
    ...
    describe('getLoadingCount$()', () => {
        it('emits 0 initially, the right count when sources emit their own count, and ends with zero', async () => {
            const { service, loadingCount } = setup();
            const countA$ = new rxjs_1.Subject();
            const countB$ = new rxjs_1.Subject();
            const countC$ = new rxjs_1.Subject();
      ...
      `);
        ...
        it('only emits when loading count changes', async () => {
            const { service, loadingCount } = setup();
            const count$ = new rxjs_1.Subject();
            const promise = loadingCount
                .getLoadingCount$()
                .pipe(operators_1.toArray())
                .toPromise();
