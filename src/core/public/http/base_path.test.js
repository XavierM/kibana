...
describe('BasePath', () => {
    describe('#get()', () => {
        it('returns an empty string if no basePath not provided', () => {
            expect(new base_path_1.BasePath().get()).toBe('');
        });
        it('returns basePath value if provided', () => {
            expect(new base_path_1.BasePath('/foo').get()).toBe('/foo');
        });
        describe('#prepend()', () => {
            it('adds the base path to the path if it is relative and starts with a slash', () => {
                const basePath = new base_path_1.BasePath('/foo/bar');
    ...
    describe('serverBasePath', () => {
        it('defaults to basePath', () => {
            expect(new base_path_1.BasePath('/foo/bar').serverBasePath).toEqual('/foo/bar');
        });
        it('returns value when passed into constructor', () => {
            expect(new base_path_1.BasePath('/foo/bar', '/foo').serverBasePath).toEqual('/foo');
        });
    });
});
