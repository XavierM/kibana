...
const map_utils_1 = require("./map_utils");
describe('groupIntoMap', () => {
    it('returns an empty map when there are no items to map', () => {
        const groupBy = jest.fn();
        expect(map_utils_1.groupIntoMap([], groupBy)).toEqual(new Map());
        expect(groupBy).not.toHaveBeenCalled();
    });
    it('calls groupBy for each item in the collection', () => {
        const groupBy = jest.fn();
        map_utils_1.groupIntoMap([{ id: 1 }, { id: 2 }, { id: 3 }], groupBy);
    ...
    it('returns each item in the key returned by groupBy', () => {
        const groupBy = (item) => item.id;
        expect(map_utils_1.groupIntoMap([{ id: 1 }, { id: 2 }, { id: 3 }], groupBy)).toEqual(new Map([
            [1, [{ id: 1 }]],
            [2, [{ id: 2 }]],
            [3, [{ id: 3 }]],
        ]));
    ...
    it('groups items under the same key returned by groupBy', () => {
        const groupBy = (item) => (item.id % 2 === 0 ? 'even' : 'odd');
        const expectedResult = new Map();
        expectedResult.set('even', [{ id: 2 }]);
        expectedResult.set('odd', [{ id: 1 }, { id: 3 }]);
        expect(map_utils_1.groupIntoMap([{ id: 1 }, { id: 2 }, { id: 3 }], groupBy)).toEqual(expectedResult);
    });
...
describe('mapValuesOfMap', () => {
    it('applys the mapper to each value in a map', () => {
        ...
        map.set(even, 2);
        map.set(odd, 1);
        map_utils_1.mapValuesOfMap(map, mapper);
    ...
    it('returns a new map with each value mapped to the value returned by the mapper', () => {
        ...
        map.set(even, 2);
        map.set(odd, 1);
        expect(map_utils_1.mapValuesOfMap(map, mapper)).toEqual(new Map([
            [even, 6],
            [odd, 3],
        ]));
